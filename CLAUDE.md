...
- **Godmode marketing batch (NEW, WIRED in marketing.html)**: `/api/marketing/chatbot` (client-KB FAQ + lead-capture bot — `chatbot.py`, website/WA widget brain) · `/sentiment` (`sentiment.py` — reviews mood+themes) · `/hashtags` (`hashtags.py` — trending + best-time) · `/brand-logo` (AI logo, `ai_image.logo_url`). **Routes ~232; marketing.html ab 27 tabs** (+ Scheduler; pehle 26 — AI Image/Complete-Post/Variations/Chatbot/Sentiment/Hashtags/Logo etc. — JS `node --check` verified). VPS pe ab node v18 installed (`scripts/check_marketing_js.py`).
