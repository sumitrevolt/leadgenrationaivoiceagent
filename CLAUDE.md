...
- 2026-07-07: **ADR-031 Final 3 ledger events** — `lead_captured`/`followup_sent`/`weekly_report_generated`, closing the full 14-event vocabulary. `advisor()` caught pre-implementation that `lead_captured` was about to be wired to a partial-coverage caller instead of the real shared chokepoint (`inquiry_hooks.run_after_inquiry()`).
