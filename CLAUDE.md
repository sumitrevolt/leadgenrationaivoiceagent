...
**Next action:** Self-improve ETA-skew fix SHIPPED 2026-07-11 (`76b538d`, VPS image `2e1172ad78c`): live consecutive ticks 08:23 sales_deepdive → 08:26 harvest_leads prove chain continuity; daily audit loop-alive green, budget 4%. New evidence: harvest task hit asyncpg pooled-connection cleanup on a closed per-task event loop (nonfatal but real P1), and INFO HTTP logs can expose query-string credentials (rotate affected key + redact URL logging = P0). Redis `dlq:dead` still has 4 preserved QA `TimeLimitExceeded(600)` entries. Priorities: (1) secret log-redaction + affected-key rotation (2) Celery async-engine loop lifecycle (3) QA timeout/DLQ (4) `/upi/pending` collision (5) full suite. `HOT_QUEUE_BRIEF_DAILY`/`CELERY_VIDEO_QUEUE` remain OFF.
