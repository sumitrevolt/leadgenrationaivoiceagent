...
**Next action:** UPI 404 shipped (`4aaf8040`). **ADR-075 bhi SHIPPED 2026-07-10 (user-auth "ship bhi karo"): commits `aa84a74` (control-plane code+tests, 10 files) + `ac06096` (memory ledger), pathspec-only, pushed, VPS rebuilt+recreated, 3x `/health = production healthy` + Docker health flip + external domain check; `POST /api/dev-tasks/claim-next` live (401 auth-gate, flags OFF = 503 behind auth).** Remaining uncommitted on shared tree: ADR-074 Hot Queue files + other sessions' concurrent edits (~22 files) — user-authorized surgical commits only, never `git add -A`. Then verify worker/scheduler and only explicitly enable `HOT_QUEUE_BRIEF_DAILY`. Owed: full `run_tests.bat` under project image, NVIDIA key rotation, `/upi/pending` duplicate-route collision cleanup (admin_ops.py:522 vs upi_payments.py:87, pre-existing). Keep `EMAIL_WARMUP`, WhatsApp auto-send, voice-close WhatsApp, and platform dial OFF.

**Ops facts (hot):** Company inboxes = `admin@leadsgenai.in`, `sunny@leadsgenai.in` only · `SOCIAL_PREFS_HONOR` default OFF (draft prefs dormant until operator flip) · Scheduler = Celery durable (`RUN_IN_PROCESS_SCHEDULER=0`, rollback = `=1`+`WEB_CONCURRENCY=1`) · 24 staff jobs + dead-man trio alive · WhatsApp auto-send OFF (`WHATSAPP_AUTO_SEND=0`, `VOICE_CLOSE_WHATSAPP=0`; 1-click only) · UPI pay-info ARMED + `/api/upi/submit` LIVE (401 with auth = correct, 404 fixed 2026-07-10 `4aaf8040`) · NOTIFY_EMAIL set · Sentry ARMED · offsite backup LIVE (restore proven) · MCP `/mcp` gated (`FASTAPI_MCP_TOKEN`/allowlist) · 250 skills in skill_pack · Slash commands: `/verify` `/ship` `/checkpoint` `/learn` `/compact-check` `/optimize` `/test-expand`.
