...
**Ops facts (hot):** **Prod `/health`=`9bfc2d6f`** (2026-08-01T11:00Z deploy via PR #205, canonical deploy_vps.sh; direct HTTPS probe, never browser). `origin/main` = `9bfc2d6f` (EQUAL to prod). All 5 app-image services on `9bfc2d6f` — **no per-container skew** (re-probed 2026-08-01). Rollback ref = `9bfc2d6f` until the next deploy · **sales autopilot EXECUTING hourly, DRY-RUN** (2026-08-01 probe, both containers): `SALES_AUTOPILOT_ENABLED=1` · `SALES_AUTOPILOT_DRY_RUN=1` · `SALES_AUTOPILOT_EMAIL_ENABLED=0` · `SALES_AUTOPILOT_WHATSAPP_ENABLED=0` · `WHATSAPP_AUTO_SEND=0` · do NOT flip DRY_RUN or channels without owner decision · **platform_dial TEST-MODE**: `PLATFORM_DIAL_DAILY=10`, `data/platform_dial.json enabled:true limit:10` (owner re-enabled 2026-07-31, allowlist + bot/IVR detection; NO live call fired) · **CREATIVE_OS_ENABLED=1** (was "OFF" in memory — probe 2026-08-01; state file absent = likely inert, confirm intent) · **31 agents** · calling HARD OFF except test-mode dial · self-improve loop ALIVE (120/day cap, hit daily) · DLQ=0, celery queue=0 · revenue snapshot MRR ₹1,999 / 1 active / 0 churn.
