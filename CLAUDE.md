...
- 2026-07-12: **Production cleanup — repo hygiene + deploy `acc3e8d`.** Committed 53 pending Unity scaffold files (`5d764f8`), deleted 27 local + 19 remote stale branches (claude/*/salvage/*/dependabot/*/feat/*/fix/*), cleared 5 stashes, pruned remote refs. Repo now: single `main` branch, clean working tree. Gates: prod_check ALL PASSED (1099 routes), check_secrets clean, 36/36 tests passed. Deployed `5d764f8` then `acc3e8d` to VPS; `/health` = production, healthy, 0 restarts.
- 2026-07-12: **ADR-076 Unity Blueprint Virtual Office — COMMITTED + DEPLOYED, sab INERT.** 53 files committed as `5d764f8` (previously untracked): 9 docs, `frontend/office_blueprint.html` shell, `app/main.py` +mode-routing, `automation_flags` +2 UNITY_* flags (OFF), `tests/test_office_blueprint_shell.py` (13 tests — all passed), `unity/LeadGenVirtualOffice/` scaffold. BLOCKER: Unity Editor not installed (WebGL build impossible until user installs Unity Hub + 2022.3 LTS).
- 2026-07-12: **Loop 30 SHIPPED + LIVE — a11y focus-trap on 3 new modals (Round-3 HIGH-1).** `_focusTrap(container, onEscape)` helper added to both frontend files. Commit `e7e34d2`. Round-3 backlog now 6 HIGH / 10 MED / 14 LOW.
**Blockers / USER-action pending (env-unset = dormant, graceful skip):**
- Unity program (ADR-076): Unity Hub + 2022.3 LTS + WebGL module install (USER) — code committed + deployed, tests passing, just needs Unity Editor for WebGL build.
...
**Next action:** (1) API.md endpoint index refresh (`scripts/sync_api_docs.py`) (2) secret log-redaction + affected-key rotation (P0) (3) Celery async-engine loop lifecycle (P1) (4) QA timeout/DLQ (5) GTM sprint — Hot Queue / dialer for mid-funnel conversion. `HOT_QUEUE_BRIEF_DAILY`/`CELERY_VIDEO_QUEUE` remain OFF.
