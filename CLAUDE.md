...
**Ops facts (hot):** **Prod `/health`=`dd193a69`** (2026-07-28T02:40:07Z, direct HTTPS — **never a browser**, a service worker served a 5-day-old `/health` and swallowed the cache-buster; PR #147 merge). `origin/main` = `6a504321` and is AHEAD of prod — they are NOT equal; per-container skew NOT re-probed. Previous entry here said `441cf37a`: un-re-verified for 3 days, and demonstrably wrong since at least 2026-07-27 (when `dd193a69` merged) — "stale" and "wrong" are different durations, don't merge them. Rollback ref = `dd193a69` until the next deploy · ⚠️ **sales autopilot + Creative OS now ON DISK in prod but INERT** — `SALES_AUTOPILOT_ENABLED` unset in app+scheduler, `sales_autopilot` in `RUN_DUE_EXCLUDE`; do NOT call them live · **31 agents** · Boss/OpenClaw ≠ 32nd agent · Owner OS sole OpenClaw authority · OpenClaw Stage A GREEN-only · calling HARD OFF · `PLATFORM_DIAL_DAILY=0` · `WHATSAPP_AUTO_SEND=0` · customer review/WhatsApp review/social publish/video scheduler OFF (**flag values here are ASSUMED — `/health` returns no flags, so none of them were re-probed on 2026-07-28; re-probe before relying on one**) · Master Blueprint canonical graph = **L0 48 / L1 5 / L2 1 = 54 nodes**, 52 edges, 11 flows, 18 domains, 9 layers (`app/platform/blueprint_graph.py` + `blueprint_detail_nodes.py`, ONE registry) · shared context = `docs/context/` · Swara/voice FROZEN this wave.
