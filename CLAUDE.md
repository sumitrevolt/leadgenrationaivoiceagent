...
**Next action:** Owner signs in through the handed-off Admin Login tab; owner-managed config enables only customer-review + Jiya allowlist; then one authenticated read-only Jiya Preview canary. Keep WhatsApp/publish/scheduler/calls OFF. Parallel: GTM Hot Queue → 2nd paying customer. Detail = `docs/context/ACTIVE_WORK.md`.

**Ops facts (hot):** **Prod `/health`=`510ed7bc`** (re-probe before future claims) · deploy run `30002538121` successful · all five app-image containers exact-SHA, running, restart=0 · queues celery=0/failed=0/dead=0/resolved=9 · customer review/WhatsApp review/social publish/video scheduler OFF · `PLATFORM_DIAL_DAILY=0` · shared context = `docs/context/` · Swara/voice FROZEN this wave.
