...
- **Automation/marketing helpers (installed on VPS, opt-in/defensive)**: `app/llm/structured.py` (Instructor → typed LLM JSON, auto-retry), `app/marketing/seo_tools.py` (advertools SEM keywords + RSA ad split), `app/lead_scraper/web_extract.py` (trafilatura clean text + email/phone). Dep/key missing → fallback, koi code abhi import nahi karta (wire when needed). Doc: `docs/Automation_Marketing_Repos.md`.
