...
- 2026-07-08: **ADR-051 CustomerDeliverable DB read-path readiness audit shipped.** Added `customer_deliverable_db_audit()` and surfaced it as additive `db_audit` inside existing `/api/admin/delivery-cockpit` payload (no customer-facing read-path swap). It compares current derived deliverable statuses against latest `customer_deliverables` rows and reports `missing_rows`, `stale_db_rows`, `ahead_db_rows`, sample mismatches, and `read_path_ready`. Gate: 42 targeted tests green, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`e9116df`): app/worker/worker-heavy/scheduler healthy, queue 0, public/internal `/health`=`environment:production`, cockpit remains 401 auth-gated, no fresh real errors.
- 2026-07-08: **ADR-050 CustomerDeliverable DB status/proof sync now wired from real actions.** Added never-raise `sync_customer_deliverable_status()` that updates existing rows only (no FK-risk row creation on jsonl-only customers). Real writers now update DB sidecar rows: content generation marks `social_posts`/`branded_posters` pending approval, approve marks `social_posts` approved, posted/manual-publish marks `proof` delivered, monthly-report action marks `monthly_report` delivered, and failures can mark rows failed. Gate: 40 targeted tests green (`customer_deliverable_db`, Product-One, publish-outcome, post-lifecycle), `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`22d3763`): app/worker/worker-heavy/scheduler healthy, queue 0, public/internal `/health`=`environment:production`, public pay-info 200, no fresh real errors.
- 2026-07-08: **ADR-049 CustomerDeliverable DB taxonomy now matches semantic delivery ids.** `ProductOnePlanDeliverables["starter"]` now seeds the same 10 ids as `DELIVERABLES` (`business_profile`, `brand_kit`, `branded_posters`, `social_posts`, `festival_ideas`, `gbp_suggestions`, `whatsapp_pack`, `review_replies`, `monthly_report`, `proof`) instead of the old DB-only vocabulary. `initialize_deliverables_for_client()` now normalizes legacy rows in place and seeds missing semantic rows, with `business_profile` waiting on customer and the rest not-started; removed the misleading auto-delivered invoice row. Gate: `tests/test_customer_deliverable_db.py` + `tests/test_product_one_delivery.py` = 23 passed, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`b428766`): VPS app/worker/worker-heavy/scheduler healthy, queue 0, public/internal `/health`=`environment:production`, no fresh real errors.
...
**Next action:** No verified uncommitted production-ready batch is pending; latest live code commit is `e9116df` (DB read-path readiness audit in delivery cockpit). Stale/dirty Claude worktrees still exist, but council decision remains **do not merge blindly**: several are hundreds of commits behind or would reintroduce deleted Twilio/Exotel/customer-dashboard forks and remove deployed Delivery-OS work; treat them as parked audit material until rebased file-by-file. DB read-path migration should wait until `db_audit.read_path_ready` is clean on real customers. Standing GTM item unchanged and now highest leverage: hot lead **+917498797259 = Relook Clinic (Nashik, hair_transplant, 5.0⭐/178rev)** — value-first FREE-audit WhatsApp draft ready (human-send, NOT auto); dialer sprint triaged (354→230 service-SMB ICP). User-gated deferrals: `AUTO_DELIVER_VALUE` flip after WAHA QR + test-send. Deliverability ~2000-email/0-reply root = separate ops sprint.
