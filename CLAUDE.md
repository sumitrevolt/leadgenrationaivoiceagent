...
**Next action:** UPI 404 + ADR-074 + ADR-075 + route inspection + import fixes ALL SHIPPED as of 2026-07-10. VPS HEAD `7db1fab8` on `main`. Shared tree now clean of the ADR ships — remaining uncommitted = 3 untracked docs (`docs/superpowers/plans|specs/2026-07-10-product-one-video-creative-pipeline-*`, other-session WIP) + `.commandcode/` (unclear artefact). Big session-lesson: sandbox bash mount can systematically lie about file contents (73 "syntax broken" + 14 "null-byte" reports were all mount corruption, not real disk state); trust Windows Read/PowerShell as truth, verify with Windows Python `ast.parse` before ANY commit. ~~(1) rebuild worker+scheduler before flag flip~~ — **DONE 2026-07-10 (2nd ship pass):** image rebuilt at HEAD `7db1fab8`, `worker`+`worker-heavy`+`scheduler` recreated, ALL 4 containers verified same new image `7cd2652df071`, celery queue=0, all healthy, 2x internal + external `/health = production healthy`. `HOT_QUEUE_BRIEF_DAILY` flip ab sirf user ke explicit bolne pe. Baaki priority: (2) NVIDIA key rotation (3) `/upi/pending` duplicate-route collision cleanup (admin_ops.py:522 vs upi_payments.py:87, pre-existing) (4) full `run_tests.bat` under project image. Keep `EMAIL_WARMUP`, WhatsApp auto-send, voice-close WhatsApp, and platform dial OFF.
