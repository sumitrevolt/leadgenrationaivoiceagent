...
- **NEXT SESSION (P3 build order)**: (1) docs.vobiz.ai/concepts/sip-trunking padho — trunk provisioning + termination URI; (2) FreeSWITCH docker-compose `app/telephony/freeswitch/` (ports 5060/5080 + RTP range, Mumbai VPS pe); (3) Vobiz gateway XML + dialplan → ESL socket; (4) `app/voice_agent/pipecat_pipeline.py` (pipecat install — Windows venv + VPS); (5) e2e: outbound test call user ke number pe (DLT abhi pending hai — test calls transactional/own-number tak limited rakhna jab tak DLT na ho); (6) 140-DID Vobiz console se jab DLT clear ho.
