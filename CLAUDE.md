...
- **Pehli test call ke liye chahiye**: (1) Vobiz balance (₹25 credit hai, recharge karo), (2) user ka own number (DLT pending — sirf own/known numbers), (3) fir: POST /api/telephony/vobiz/test-call {to} YA `docker exec leadgen-freeswitch fs_cli -x 'originate sofia/gateway/vobiz/+91XXXX &echo'`. Caller-id: VOBIZ_CALLER_ID .env me set karna hoga (DID kharidne ke baad).
