...
- 2026-07-17: **ADR-119 Hybrid Agentic RAG + OKF** — OKF (`knowledge/`) curated; Qdrant Hybrid RAG keep/upgrade (never OKF-replace); Postgres live truth; Graphify relations; Redis TTL. Target BGE-M3+sparse/RRF+reranker behind flags; prod still e5-small. `SOCIAL_PREFS_HONOR=1` LIVE.
- 2026-07-17: **ADR-117/118 LIVE `95a5aecc`** — Postiz tenant isolation + `social_drain` + prefs consent honesty.
- 2026-07-16: **ADR-112/113/114** enterprise wiring honesty on `15001321`. (Older ADRs → `memory/decisions.md`.)
