...
- 2026-07-18: **ADR-121 Billing containment** — autouse test-store isolation + accountant-safe invoice VOID + prospect time-budget; ops plan `docs/plans/2026-07-18-billing-containment-ops.md` (deploy/void/UPI flip USER-pending).
- 2026-07-17: **ADR-119 Hybrid Agentic RAG + OKF** — OKF curated; Qdrant Hybrid keep; prod still e5-small. `SOCIAL_PREFS_HONOR=1` LIVE.
- 2026-07-17: **ADR-117/118 LIVE `95a5aecc`** — Postiz tenant isolation + `social_drain`. (Older → `memory/decisions.md`.)
**Blockers / USER-action pending (env-unset = dormant, graceful skip):**
- 🚨 **Billing ledger contaminated** (2026-07-18 forensic): INV/0002 disposable + INV/0003–0013 synthetic `cli_*` from VPS pytest; only INV/0001 (Jiya) real. Code containment READY locally; ledger dirty until user deploys + voids. ✅ `UPI_AUTO_ACTIVATE=0` prod LIVE (2026-07-18 11:56 UTC operator flip, backup `.env.bak-upiflip-20260718_115606`, app recreated `1803f819` healthy; rollback `=1`+recreate). `dlq:dead=7` prospect SoftTimeLimits preserved — purge only after deploy+successful prospect.
- **3 voice regressions FIXED locally, deploy pending** (2026-07-18 loop, `memory/incidents.md`): greeting `"hi"` substring-fire on romanized Hindi; stream guard-reject ab reply() pe girta (script nahi); `_script_fallback` hard post-close guard (audit-resell block). Same commit me billing containment ke saath bundle karo.
...
**Next action:** (0) L2 control-center graph RESOLVED on `5b392253`; ADR-104 follow-up RESOLVED. (1) **Postiz registration LOCKED** on VPS (`POSTIZ_DISABLE_REGISTRATION=true`, 2026-07-16 verify). (2) **Own-brand social e2e PROVEN** — jobs `7ff911ed`/`46d14cc3` drained with real Postiz `post_id` (2026-07-15); status API ab `publish_proven` field carry karta. (3) ✅ YouTube OAuth app **PUBLISHED to production** (2026-07-18, Chrome-verified — LeadsGenAI/nomadic-pipe-501417-e3); ✅ Postiz YT reconnect bhi DONE (2026-07-18, Chrome MCP — same integration id `cmr6oak91…`, brand account "sandeep maheswari" ke under LeadsGenAI channel; token ab permanent). Social stack fully self-running. (4) GTM: Hot Queue → 2nd paying customer. (5) Monitor email auto-reply + Sentry triage. Chrome smoke, WAHA, Hot Queue noise, Explorer sync, DLT, monitoring, disk, delivery-log PII = RESOLVED.

**Ops facts (hot):** Company inboxes = `admin@leadsgenai.in`, `sunny@leadsgenai.in` only · `reply_auto_send=enabled_all` in Redis (`REPLY_AUTO_SEND_HARD_OFF` rollback; batch 3/day cap 5 defaults) · `SOCIAL_PREFS_HONOR=1` prod LIVE (2026-07-17 operator flip; rollback=`=0`) · Scheduler = Celery durable (`RUN_IN_PROCESS_SCHEDULER=0`, rollback = `=1`+`WEB_CONCURRENCY=1`) · 25 staff jobs (reply triage hourly :20) + dead-man trio alive · WhatsApp auto-send OFF (`WHATSAPP_AUTO_SEND=0`, `VOICE_CLOSE_WHATSAPP=0`; 1-click only) · UPI pay-info ARMED + `/api/upi/submit` LIVE (route POST-only — GET pe 404 normal hai, `POST`+auth = 401 correct) · **`HOT_QUEUE_BRIEF_DAILY=1` prod me LIVE** · `POSTIZ_INTEGRATIONS` = 4 ids FB/IG/X/YT (2026-07-18 fix — env drift se sirf FB+IG the; text-only posts pe code IG/YT khud skip karta) · `VOICE_GEMINI_PRIMARY=0` prod me · NOTIFY_EMAIL set · Sentry ARMED · offsite backup LIVE (restore proven) · MCP `/mcp` gated (`FASTAPI_MCP_TOKEN`/allowlist) · 250 skills in skill_pack · Slash commands: `/verify` `/ship` `/checkpoint` `/learn` `/compact-check` `/optimize` `/test-expand`.
