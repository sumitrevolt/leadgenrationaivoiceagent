...
- **Festival marketing calendar (NEW, AdBanao/Predis-style)**: `app/marketing/festival_calendar.py` — curated India festival table (2026 H2 + 2027 H1, 20 tyohaar; lunar ±1din indicative, fixed-date exact; ambiguous Janmashtami/Karva Chauth OMIT). `upcoming(months_ahead)` + `autoschedule(business, niche, months_ahead)` → upcoming festivals ko content_schedule me dup-safe queue → daily run_due auto-prepares. `GET /api/marketing/festivals` · `POST /festival-autoschedule`. Verified: 20 upcoming, autoschedule(3mo)=3 queued, re-run dup-safe=0.
