...
- 2026-07-10: **ADR-074 Health-gated Hot Queue Revenue Brief, default-OFF — SHIPPED.** Council ranked health-gated revenue brief above brief-only and watchdog-only. Reuses `office_briefing` + `/app/inbox` at 08:15 IST; no duplicate queue or auto-contact channel. `HOT_QUEUE_BRIEF_DAILY` fail-closed on unknown queue/recent job failure, cross-process idempotent, retry/DLQ + dual boot-grace + Celery/in-process parity wired. FastAPI 0.139 lazy-route gate fixed (85→1064 routes, 628→0 wiring gaps). UPI submit is auth-enforced (401, not 404). 2026-07-11 sync verification: app + worker + worker-heavy + new worker-video + scheduler all run HEAD `f2ced876` image `43e877f0e457`, healthy/restarts=0; all queues/DLQ=0. Flag remains OFF in prod env.
...
**Next action:** Production synchronized 2026-07-11 at VPS/origin HEAD `f2ced876`: app + worker + worker-heavy + worker-video + scheduler same image `43e877f0e457`, healthy/restarts=0; controlled jobs/routes/health green. Natural-cycle audit then found self-improve's only requeue chain could die on sub-second Celery ETA vs Redis timestamp skew; bounded 2s tolerance is TDD-green locally and pending scoped ship/live consecutive-tick proof. Redis `dlq:dead` has 4 preserved QA `TimeLimitExceeded(600)` entries (Jul 8-10), root cause pending—do not delete before diagnosis. `HOT_QUEUE_BRIEF_DAILY` and `CELERY_VIDEO_QUEUE` remain OFF. Baaki priority: (1) QA timeout root cause (2) `/upi/pending` duplicate collision (3) NVIDIA key rotation (4) full `run_tests.bat`.
