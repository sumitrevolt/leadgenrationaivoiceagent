...
- 2026-07-19: **ADR-124 LIVE `ca98ece4`** — GBP + review-reply generators (`auto_content.generate_gbp_pack` type=gbp / `generate_review_reply_pack` type=review_reply, self-guarding, wired into `generate_content`). Jiya operate: gbp_suggestions + review_replies = done, **pct 60→80** (both ids).
- 2026-07-19: **ADR-123 LIVE `670f5793`** — Jiya client-identity split-brain fix: portal + `customer_delivery_status` ab billing/login id (`d79d690f61b3`) ko marketing id (`jiya-makeover`) pe canonicalize karte hain (billing/invoice RAW). Portal 10%→60% visible.
- 2026-07-18: **ADR-121 LIVE `f8a5f6e9`** — billing containment + voice fallback hardening + CI lock pins; ops plan B done, voids C still USER-pending. (Older → `memory/decisions.md`.)
...
**Next action:** (0) ✅ ADR-124 LIVE `ca98ece4` — Jiya gbp+review generators, pct 60→80 (both ids). (1) Jiya delivery finish (80→100): `branded_posters` 2→4 top-up + `proof` (published/scheduled), phir drafts customer approval (stage=approval_pending). (2) **USER: void INV/0002–0013** via `/app/automation` Void (keep INV/0001 Jiya). (3) GTM: Hot Queue → 2nd paying customer. (4) Monitor email auto-reply + Sentry triage. Social/Postiz/YT = self-running.
