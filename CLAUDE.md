...
- 2026-07-19: **ADR-126 LOCAL (INERT, not deployed)** — canonical Agent Runtime Contract registry `app/platform/agent_registry.py` (derive-from-STAFF+JOB_META; autonomy L0-L4 · lane 🟢🟡🔴 · budgets · kill-switch · escalation · heartbeat + useful-work). 31 canonical (Aditi NOT invented — delivery-assurance already owned by `customer_delivery`; Agent-OS = 32nd control-plane worker `CONTROL_PLANE`). 14 tests green, `validate_registry()==[]`. Phase-A foundation of master Agent-OS mandate; §5 gates encoded as data (swara/ananya RED+hard_off).
- 2026-07-19: **ADR-125 LIVE `716bed84`** — `auto_content.generate_poster_pack(target=4)` branded_posters top-up (real SVG, self-guarding, wired into `generate_content`). Jiya poster 2→4, branded_posters=done, **pct 80→90** (both ids). `proof` = honest last 10% (needs real publish — blocked, NOT faked).
- 2026-07-19: **ADR-124 LIVE `ca98ece4`** — GBP + review-reply generators wired into `generate_content`; Jiya gbp+review=done, pct 60→80. (ADR-123 & older → `memory/decisions.md`.)
...
**Next action:** (0) **Agent-OS Phase-A foundation LOCAL-VERIFIED** (ADR-126 — `agent_registry.py` + 14 tests, INERT/not-deployed) — USER decide: deploy-scope (§8: build-only vs commit vs push vs prod) + kya registry ko owner_os/scheduler ka source-of-truth banana hai (Phase A rest). (0b) ✅ ADR-125 LIVE `716bed84` — Jiya poster 2→4, branded_posters done, **pct 90** (9/10, both ids). (1) `proof` = last 10%, HONEST-blocked (NOT faked): real publish/schedule to Jiya's OWN social chahiye (customer-page Meta review + channel-connect pending) ya admin 1-click manual-publish proof + customer approval of `approval_pending` drafts. (2) **USER: void INV/0002–0013** via `/app/automation` Void (keep INV/0001 Jiya). (3) GTM: Hot Queue → 2nd paying customer. (4) Monitor email auto-reply + Sentry triage.

**Ops facts (hot):** Company inboxes = `admin@leadsgenai.in`, `sunny@leadsgenai.in` only · `reply_auto_send=enabled_all` in Redis (`REPLY_AUTO_SEND_HARD_OFF` rollback; batch 3/day cap 5 defaults) · `SOCIAL_PREFS_HONOR=1` prod LIVE (2026-07-17 operator flip; rollback=`=0`) · Scheduler = Celery durable (`RUN_IN_PROCESS_SCHEDULER=0`, rollback = `=1`+`WEB_CONCURRENCY=1`) · 25 staff jobs (reply triage hourly :20) + dead-man trio alive · WhatsApp auto-send OFF (`WHATSAPP_AUTO_SEND=0`, `VOICE_CLOSE_WHATSAPP=0`; 1-click only) · UPI pay-info ARMED + `/api/upi/submit` LIVE (route POST-only — GET pe 404 normal hai, `POST`+auth = 401 correct) · **`HOT_QUEUE_BRIEF_DAILY=1` prod me LIVE** · `POSTIZ_INTEGRATIONS` = **5 ids FB/IG/X/YT/Pinterest** (2026-07-18; pinterest id `cmrqfc9ff…`, keys `deploy/postiz/.env`; text-only posts pe code IG/YT/Pinterest khud skip karta) · `VOICE_GEMINI_PRIMARY=0` prod me · NOTIFY_EMAIL set · Sentry ARMED · offsite backup LIVE (restore proven) · MCP `/mcp` gated (`FASTAPI_MCP_TOKEN`/allowlist) · 250 skills in skill_pack · Slash commands: `/verify` `/ship` `/checkpoint` `/learn` `/compact-check` `/optimize` `/test-expand`.
