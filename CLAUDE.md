...
- 2026-07-09: **ADR-057 URL-fragment/scraper-garbage emails are rejected before dashboard/sendable counts.** Remaining 134 unique pending recipients still included hard malformed artifacts that syntax-only dashboard counts accepted: `support@pw.lie` (truncated `pw.live`), `id@r93.ful`, `a5%@bfe0r.vl`, `%20tkiblr1@...`. Tightened central `email_verify.is_obvious_false_positive()` to reject `%`/URL-encoded locals and observed scraper-garbage TLDs (`lie`/`ful`/`vl`) while preserving the company inbox truth (`admin@leadsgenai.in`, `sunny@leadsgenai.in`). Gate: 29 targeted tests green, ruff clean, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`ad3a03e`): live `with_email=2090`, `pending_total=134`, `pending_sendable=130`, `duplicate_pending_recipients=4`, queue 0, `/app/inbox` 200, public `/health`=`environment:production`.
- 2026-07-09: **ADR-056 Outreach now dedupes recipients per run.** Live post-ADR-055 audit found 138 pending rows but only 134 unique recipient emails (`biz@digchefs.com` x2, `girjassalon@gmail.com` x2, `info@novaivffertility.com` x3). `run_email_outreach()` and `run_email_followups()` now skip duplicate recipient addresses per run and report `duplicate_recipients`; dashboard stats now show `pending_total`, unique `pending_sendable`, and `duplicate_pending_recipients`. Gate: 42 targeted tests green, ruff clean, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`d095ee4`): live `pending_total=138`, `pending_sendable=134`, `duplicate_pending_recipients=4`, queue 0, `/app/inbox` 200, public/internal `/health`=`environment:production`.
- 2026-07-09: **ADR-055 Asset/placeholder false-positive emails are rejected before outreach/dashboard counts.** Live audit of 151 sendable pending prospects found 13 hard junk emails (`flags@2x.webp`, `group-...@*.webp`, `ecom-swiper@11.0.5.js`, `info@domainname.com`, `example@mysite.com`, `john@company.com`, etc.). Added central `email_verify.is_obvious_false_positive()` and reused it in `auto_outreach._valid_email()`. User confirmed company inboxes are only `admin@leadsgenai.in` and `sunny@leadsgenai.in`; tests preserve both as valid syntax. Gate: 39 targeted tests green, ruff clean, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`08b0666`): live `with_email` 2148→2116 and sendable pending 151→138.
...
**Next action:** No verified uncommitted production-ready batch is pending; latest live code commit is `ad3a03e` (URL-fragment email-artifact gate). Stale/dirty Claude worktrees still exist, but council decision remains **do not merge blindly**. Keep `EMAIL_WARMUP` paused; do not resume from `/api/growth/outreach/warmup/resume` until complaint-source/list-quality cleanup is done. Next high-leverage loop: audit remaining 130 unique sendable recipients for precise business-fit/source rules; avoid broad free-mail or broad category blocks.
