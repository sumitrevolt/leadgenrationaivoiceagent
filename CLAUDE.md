...
- 2026-07-08: **ADR-052 Hot Queue web surfaces now open WhatsApp with the AI reply draft prefilled (human-send only).** `reply_agent.hot_queue()` now emits a safe `wa_link` for phone-backed hot replies using the existing draft text; `/app/inbox` and `/app/office` show an explicit "WhatsApp draft" action while keeping copy/email/call/done controls. This closes the web-surface parity gap: ntfy push actions already had prefilled WA, but the operator inbox still opened a blank chat. Gate: 15 hot-queue tests + 15 Office frontend tests green, inline script syntax check green, `prod_check.py` PASS, secrets clean, duplicate hot-queue route grep clean. **COMMITTED + DEPLOYED to production** (`62fe0e1`): app/worker/worker-heavy/scheduler healthy, `/app/inbox` 200, hot-queue API 401 auth-gated, live HTML marker present, queue 0, public/internal `/health`=`environment:production`, no fresh real errors.
...
**Next action:** No verified uncommitted production-ready batch is pending; latest live code commit is `62fe0e1` (Hot Queue WhatsApp draft links). Stale/dirty Claude worktrees still exist, but council decision remains **do not merge blindly**: several are hundreds of commits behind or would reintroduce deleted Twilio/Exotel/customer-dashboard forks and remove deployed Delivery-OS work; treat them as parked audit material until rebased file-by-file. DB read-path migration should wait until `db_audit.read_path_ready` is clean on real customers. Standing GTM item now sharper: work `/app/inbox` Hot Queue top replies from the new "WhatsApp draft" button, starting with **+917498797259 = Relook Clinic (Nashik, hair_transplant, 5.0⭐/178rev)** — value-first FREE-audit message, human-send only. User-gated deferrals: `AUTO_DELIVER_VALUE` flip after WAHA QR + test-send. Deliverability ~2000-email/0-reply root = separate ops sprint.
