...
- 2026-07-19: **ADR-123 LIVE `670f5793`** — Jiya client-identity split-brain fix: portal (`/portal/content`,`/me`,`_biz_name`) + `customer_delivery_status` ab billing/login id (`d79d690f61b3`) ko marketing id (`jiya-makeover`) pe canonicalize karte hain (billing/invoice RAW untouched). Jiya portal 10%→60% visible. gbp/review generators = remaining.
- 2026-07-18: **ADR-121 LIVE `f8a5f6e9`** — billing containment + voice fallback hardening + CI lock pins; ops plan B done, voids C still USER-pending.
- 2026-07-17: **ADR-119 Hybrid Agentic RAG + OKF** — OKF curated; Qdrant Hybrid keep; prod still e5-small. `SOCIAL_PREFS_HONOR=1` LIVE. (Older → `memory/decisions.md`.)
...
**Next action:** (0) ✅ ADR-123 LIVE `670f5793` — Jiya identity fix deployed+proven (portal 10%→60%, both ids unified). (1) Jiya delivery finish: `gbp_suggestions` + `review_replies` REAL generators (60%→80%+), phir 14 drafts customer approval (stage=approval_pending). (2) **USER: void INV/0002–0013** via `/app/automation` Void (keep INV/0001 Jiya). (3) GTM: Hot Queue → 2nd paying customer. (4) Monitor email auto-reply + Sentry triage. Social/Postiz/YT = self-running.
