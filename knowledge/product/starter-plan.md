---
type: ProductPlan
title: Starter Marketing Plan
description: AI Marketing Automation starter (₹1,999/mo) — curated promise shape; code truth in packages.py.
tags: [pricing, customer-deliverables, starter]
timestamp: 2026-07-17T00:00:00Z
resource: app/marketing/packages.py
---

# Starter Marketing Plan

Monthly price: **₹1,999** (Marketing product — voice is not the USP).

Includes (product promise; delivery honesty per ADR-116):

- Daily social content drafts (approve / 1-click; auto only with owned channels + consent)
- Four branded posters monthly (`type=poster` — not festival padding)
- WhatsApp content pack (1-click human send; bulk auto OFF)
- GBP audit / tips (needs GBP link where required)
- Review reply drafts

Related:

- [Deliverables](deliverables.md)
- [Pricing rules](pricing-rules.md)
- [Customer onboarding](../operations/customer-onboarding.md)

**Code wins:** public packages and feature lists → `app/marketing/packages.py` + `tests/test_billing_truth_2026.py`.
