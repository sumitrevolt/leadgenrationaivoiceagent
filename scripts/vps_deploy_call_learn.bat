...
"%SSH%" -i "%KEY%" -o BatchMode=yes %HOST% "set -o pipefail; cd /opt/leadgen && git fetch --all -q && git reset --hard origin/main -q && docker compose -f docker-compose.vps.yml build app && docker compose -f docker-compose.vps.yml up -d --no-deps worker && systemctl restart leadgen && sleep 22 && curl -sf http://127.0.0.1:8000/health" >> "%LOG%" 2>&1
if errorlevel 1 goto fail

REM NOTE: DND_FAIL_OPEN=1 removed (TC-002) — it turns the TRAI DND gate fail-OPEN; never set in prod.
"%SSH%" -i "%KEY%" -o BatchMode=yes %HOST% "cd /opt/leadgen && python3 scripts/env_set.py VOBIZ_CALL_RECORD=1 AUTO_QUALIFY_CALLS=1 DND_CARRIER_SCRUB=1 && docker compose -f docker-compose.vps.yml up -d --no-deps --force-recreate worker && systemctl restart leadgen && sleep 20" >> "%LOG%" 2>&1
