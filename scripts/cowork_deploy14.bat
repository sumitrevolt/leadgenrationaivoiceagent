@echo off
REM Godmode batch: chatbot + sentiment + hashtags + logo. Log -> cowork_deploy14.log
cd /d C:\Users\Ratanshila\Documents\leadgenrationaivoiceagent
set LOG=cowork_deploy14.log
...
echo === py_compile === > %LOG%
call python -m py_compile app\marketing\chatbot.py app\marketing\sentiment.py app\marketing\hashtags.py app\marketing\ai_image.py app\api\marketing.py >> %LOG% 2>&1
echo EXIT_PYCOMPILE=%errorlevel% >> %LOG%
echo === git add/commit/push === >> %LOG%
call "C:\PROGRA~1\Git\cmd\git.exe" add -A >> %LOG% 2>&1
call "C:\PROGRA~1\Git\cmd\git.exe" commit -m "feat(godmode): AI chatbot (client-KB) + sentiment + hashtags + AI logo (4 marketing endpoints)" >> %LOG% 2>&1
...
echo. >> %LOG%
echo === QA: prod_check (route count ~229) === >> %LOG%
