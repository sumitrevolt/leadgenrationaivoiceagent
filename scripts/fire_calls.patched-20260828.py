...
async def fire_vobiz(
    ...
    for p in prospects:
        ...
        if result.get("placed"):
            ...
            # Deterministic provider rejection (e.g. caller-ID not owned / carrier
            # reject) should never re-cycle the same lead forever. Advance it.
            err_str = str(result.get("error") or "") + " " + str(body)
            if "not owned" in err_str or "USER_BUSY" in err_str:
                mark_called(p["phone"])
            fail += 1
