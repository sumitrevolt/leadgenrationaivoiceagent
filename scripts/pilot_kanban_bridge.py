    ...
    while True:
        ...
        for task in tasks:
            ...
            if 'Failure' in task['title']:
                ...
                conn.execute("INSERT INTO task_events (task_id, kind, payload, created_at) VALUES (?, 'blocked', '{\"reason\": \"Failed as part of demonstration\"}', ?)", (tid, now))
                conn.commit()
            else:
                ...
                if result and 'result' in result:
                    ...
                    for item in content:
                        if item.get('type') == 'text' and 'screenshot saved to' in item.get('text', ''):
                            # extract path
                            t = item['text']
