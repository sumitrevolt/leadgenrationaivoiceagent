...
def is_campaign_eligible(payment_received_at):
    ...
    try:
        ...
        end = datetime.strptime("2026-08-30", "%Y-%m-%d").replace(tzinfo=timezone.utc) + timedelta(
...
def update_verified_cash(amount):
    ...
    logger.info(
        f"VERIFIED CASH: ₹{float(VERIFIED_CASH):,.2f} / ₹{float(TARGET):,.2f} = {progress}%"
    )
    return progress
...
def process_event(event):
    ...
    try:
        ...
        if customer_id != inv_customer_id:
            ...
            conn.commit()
            logger.warning(
                f"Event {event_id}: Customer mismatch {customer_id} vs {inv_customer_id}"
            )
            return False
        ...
        cursor.execute(
            "UPDATE upi_submissions SET status = 'VERIFIED_PAID' WHERE id = %s", (event_id,)
        )

        # Settle invoice
        cursor.execute(
            "UPDATE invoices SET status = 'PAID', paid_at = NOW(), verified_at = NOW(), verified = TRUE WHERE id = %s",
            (invoice_id,),
        )
        ...
        # Record audit trail
        cursor.execute(
            "INSERT INTO payment_audit (invoice_id, transaction_id, amount, verified_at, verified_by) VALUES (%s, %s, %s, NOW(), 'AUTOMATED_REVENUE_LOOP')",
...
def revenue_loop():
    ...
    progress = calculate_progress()
    logger.info(
        f"Revenue loop stopped. Final progress: {progress}% (₹{float(VERIFIED_CASH):,.2f} / ₹{float(TARGET):,.2f})"
    )
