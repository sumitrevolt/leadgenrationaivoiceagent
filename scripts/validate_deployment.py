...
class ValidationReport:
    ...
    def print_report(self):
        ...
        print("=" * 70)
        print(
            f"  Results: {self.passed} passed, {self.warnings} warnings, "
...
def check_tests(report: ValidationReport, strict: bool):
    ...
    code, stdout, stderr = run_command(
        [sys.executable, "-m", "pytest", "tests/test_production_ready.py", "-v", "--tb=short", "-q"]
    )
...
def check_migrations(report: ValidationReport):
    ...
    code, stdout, stderr = run_command([sys.executable, "-m", "alembic", "check"])
...
def check_terraform(report: ValidationReport):
    ...
    code, stdout, stderr = run_command(["git", "ls-files", str(tf_dir / "*.tfstate")])
    if stdout.strip():
        report.add(
            CheckResult(
                "Terraform",
