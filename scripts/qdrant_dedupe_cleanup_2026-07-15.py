...
APPROVED_PAIRS = [
    ("df07a21be4ad", "0c54070c-361a-5be4-ab3c-af2a00ebbb6b", ["e6a6e099-2164-4d48-832b-9190c3e6c4fd"]),
    ("ea3a0cfb889b", "111b3ab5-9a90-518f-a107-b64d112c2358", ["450f4c6f-fb5a-4c9c-9e9e-7805a134758d", "a870bf1f-1685-4205-a6ea-18de6b57adc7"]),
    ("f49bc13997ca", "357291d2-c89a-4c81-ab5e-1dc72e0008e1", ["9ff5d624-9a21-534b-a0e5-25c7a85d2c21"]),
    ("1df386f90e64", "4d012ae5-858b-5ae9-be6b-cfd18cf7e3e1", ["51da46e5-011c-4cda-8154-bc5571b039eb"]),
    ("5b7a96305ca9", "531c04e8-4a79-5a1b-a215-336f63c004b5", ["7ac06641-a726-4ecc-ae83-8e386c728920"]),
    ("759ce6691241", "5bcad415-4b13-56b7-83c7-7ee7bf72c994", ["e54603b0-4df8-4eab-bdae-e21a65425703"]),
    ("4877dc02a5d7", "617fd6f4-2027-5c94-aafc-0e8324ff1b7d", ["d401da52-a431-4067-bd66-03a0e0d2f336"]),
]
...
def main() -> int:
    ...
    print(f"[revalidate] live drop-id set == approved drop-id set: {live_drop_set == approved_drop_set}")

    if live_drop_set != approved_drop_set:
        print("ABORT: live candidate set does not match the approved 8-point scope. "
    ...
    bad_ns = [r.id for r in retrieved if str((r.payload or {}).get("namespace", "")) not in APPROVED_NAMESPACES]
    if bad_ns:
        print(f"ABORT: drop ids outside approved namespaces: {bad_ns}", file=sys.stderr)
        return 3
    if len(retrieved) != 8:
        print(f"ABORT: expected to retrieve exactly 8 drop points, got {len(retrieved)}", file=sys.stderr)
        return 4

    print("[revalidate] PASS -- live candidate set matches the approved 8-point scope exactly. Proceeding.")
    ...
    for ns in ("solar_residential", "insurance", "ai_marketing", "home_loans", "_global"):
        ...
        print(f"[verify] namespace '{ns}' count before={before_n} after={after_n} unchanged={before_n == after_n}")
