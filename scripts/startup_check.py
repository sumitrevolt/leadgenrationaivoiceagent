    ...
    def check_environment(self) -> bool:
        ...
        if not all(os.environ.get(v) for v in telephony_vars):
            self.warnings.append(
                "No telephony provider configured (VOBIZ_AUTH_ID/VOBIZ_AUTH_TOKEN)"
            )
    ...
    async def check_redis(self) -> bool:
        ...
        try:
            ...
            if isinstance(client, InMemoryCache):
                self.warnings.append(
                    "Redis: Using in-memory fallback (not recommended for production)"
                )
                return True
                    ...
                    if os.environ.get("APP_ENV") == "production":
                        try:
                            ...
                            if mode & stat.S_IROTH:
                                self.warnings.append(
                                    f"{env_file} is world-readable (security risk)"
                                )
                        except Exception:
                            pass
