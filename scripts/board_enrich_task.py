...
async def process_prospects():
    ...
    async def enrich_row(row):
        ...
        # Derive Quality
        has_e = "@" in email
        has_p = ("+" in phone) or (phone.strip().replace("-", "").isdigit() and len(phone.replace("-", "").strip()) >= 10)
        
        if has_e and has_p: contact_q = "Email + Phone"
        elif has_e: contact_q = "Email Only"
        elif has_p: contact_q = "Phone Only"
        else: contact_q = "Form/Public Contact Only"
        ...
        offer = "Combo ₹5,999/mo" if score >= 80 or "Agency" in row.get("industry", "") else "Main ₹1,999/mo"
