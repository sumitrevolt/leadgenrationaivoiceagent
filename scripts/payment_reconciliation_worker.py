...
def is_campaign_eligible(payment_received_at, campaign_start='2026-08-23', campaign_end='2026-08-30'):
    ...
    try:
        ...
        end = datetime.strptime(campaign_end, '%Y-%m-%d').replace(tzinfo=timezone.utc) + timedelta(days=1, hours=-1, minutes=-30)  # EOD IST
        return start <= pt <= end
    except Exception:
        return False
