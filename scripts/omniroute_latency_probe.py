...
PROMPT = (
    "Customer: kitna charge karte ho? "
    "Reply in one short Hindi sentence."
)
...
def _key() -> str:
    ...
    if not k:
        sys.stderr.write("ERROR: neither OMNIROUTE_API_KEY nor OMNIROUTE_MANAGEMENT_API_KEY is set\n")
        sys.exit(2)
    return k
    ...
    for model in models:
        ...
        for i in range(args.iters):
            ...
            if "error" in r:
                ...
                print(f"  [{model}] iter {i + 1}: TTFT {r['ttft_ms']} ms | total {r['total_ms']} ms")
        results[model] = runs
        print()
