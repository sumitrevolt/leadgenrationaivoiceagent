...
async def main():
    ...
    r, prov = await free_ai.chat("You are terse.", [{"role": "user", "content": "say OK"}], max_tokens=5)
    print("free_ai.chat ->", repr((r or "")[:30]), "| provider:", prov or "(none)")
    print("cooldown state:", {k: round(v - __import__('time').time(), 1) for k, v in free_ai._LLM_COOLDOWN_UNTIL.items()})

    from app.marketing import content_schedule

    item = content_schedule.schedule("Test Cafe", "restaurant_cafe", date_iso="2020-01-01", offer="20% off")
    ...
    print("list total:", len(lst), "| ready:", len(ready), "| sample_caption:", (ready[0]["content"]["caption"][:50] if ready and ready[0].get("content") else "—"))
