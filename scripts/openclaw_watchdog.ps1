...
if (Test-Health) { Trim-Log; exit 0 }

Write-Log "UNHEALTHY: gateway not responding on port $Port - restarting"
...
if ($ok) {
  Write-Log "RECOVERED: gateway healthy again after restart"
  Notify "OpenClaw recovered" "Gateway was down and has been restarted successfully on $env:COMPUTERNAME."
} else {
  Write-Log "FAILED: gateway still down 60s after restart attempt"
  Notify "OpenClaw DOWN" "Gateway restart FAILED on $env:COMPUTERNAME. Check ~/.openclaw/logs."
}
