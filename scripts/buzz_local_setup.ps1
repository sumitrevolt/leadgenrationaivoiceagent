...
Owner decision 2026-08-10: local-first relay (ws://127.0.0.1:3100) on the dev
machine. Buzz = coordination plane ONLY - never production authority
...
# Pin the published relay port to loopback (upstream compose publishes 0.0.0.0).
$composeFile = Join-Path $ComposeDir "compose.yml"
$cc = Get-Content $composeFile -Raw
$cc = $cc.Replace('"${BUZZ_HTTP_PORT:-3000}:3000"', '"127.0.0.1:${BUZZ_HTTP_PORT:-3000}:3000"')
Set-Content -NoNewline -Encoding utf8 $composeFile $cc
...
# 5. Liveness poll (BUZZ_HTTP_PORT, default 3100)
$port = 3100
try {
