    ...
    while i < len(fm_lines):
        ...
        if ":" not in raw:
            return (
                None,
                body,
                "cannot parse frontmatter line %d: %r (expected 'key: value')" % (i + 2, stripped),
            )
                ...
                if sub and not sub.startswith("#"):
                    if ":" not in sub:
                        return (
                            ...
                            body,
                            (
                                "cannot parse nested entry under %r at frontmatter line %d: %r"
                                % (key, i + 2, sub)
                            ),
                        )
                ...
                for part in inner.split(","):
                    if ":" not in part:
                        return (
                            None,
                            body,
                            "cannot parse inline map for %r: %r" % (key, part.strip()),
                        )
...
def check_description(desc, errors, warnings):
    ...
    if FIRST_PERSON.search(desc_unquoted):
        warnings.append(
            ...
            "when the user …'" % FIRST_PERSON.search(desc_unquoted).group(0)
        )
...
def check_body(body, skill_dir, strict, errors, warnings):
    ...
    for marker in sorted(set(TODO_MARKER.findall(body))):
        warnings.append(
            "unfinished-work marker %r found in SKILL.md body: resolve or remove it" % marker
        )
    ...
    for ph in sorted({m.group(0) for m in TEMPLATE_PLACEHOLDER.finditer(prose_no_code)})[:5]:
        errors.append(
...
def lint(skill_dir, strict=False):
    ...
    if compat is not None:
        ...
        if not str(compat).strip():
            ...
            errors.append(
                "compatibility is %d characters (max %d)" % (len(str(compat)), MAX_COMPAT)
            )
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Validate an agent skill directory (agentskills.io spec)."
    )
    parser.add_argument(
        "skill_dir", help="path to the skill directory (the folder containing SKILL.md)"
    )
    parser.add_argument(
        "--strict",
        action="store_true",
        help="promote spec-limit violations (500+ line body) to errors",
    )
    parser.add_argument(
    ...
    if args.as_json:
        ...
        print(
            "Linting %s (strict=%s)"
            % (os.path.abspath(args.skill_dir), "on" if args.strict else "off")
        )
        for msg in errors:
            print("  ERROR: %s" % msg)
        for msg in warnings:
