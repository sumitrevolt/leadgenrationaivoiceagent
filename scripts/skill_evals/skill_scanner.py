...
PIPE_SHELL_PATTERNS = [
    ...
    (
        re.compile(
            r"(?i)(?:iwr|irm|invoke-webrequest|downloadstring)[^\n]{0,160}\|\s*iex\b"
        ),  # skillscan:allow
        "EXEC01",
        "CRITICAL",
        "PowerShell download piped into Invoke-Expression (iex)",
    ),
    (
        re.compile(
            r"(?i)\biex\b[^\n]{0,160}(?:downloadstring|invoke-webrequest|\biwr\b|\birm\b)"
        ),  # skillscan:allow
        "EXEC01",
        "CRITICAL",
        "Invoke-Expression executing downloaded content",
    ),
    (
        re.compile(
        ...
        "EXEC02",
        "CRITICAL",
        "base64-decoded data piped into a shell",
    ),
    (
        re.compile(
            r"\|\s*base64\s+(?:-d|-D|--decode)\b[^\n]{0,80}\|\s*(?:python3?|node|perl|ruby)\b"
        ),  # skillscan:allow
        "EXEC02",
        "CRITICAL",
        "base64-decoded data piped into an interpreter",
    ),
]
...
OBFUSCATION_EXEC_PATTERNS = [
    (
        re.compile(
            r"(?:\bexec|\beval|\bcompile)\s*\([^\n]{0,160}(?:b64decode|base64|fromhex|codecs\.decode|rot13)"
        ),  # skillscan:allow
        "OBF02",
        "CRITICAL",
    ...
    (
        re.compile(
            r"(?:\beval|new\s+Function)\s*\([^\n]{0,160}(?:atob|Buffer\.from)\s*\("
        ),  # skillscan:allow
        "OBF02",
        "CRITICAL",
    ...
    (
        re.compile(
            r"(?:python3?|node)\s+-[ce]\s+[^\n]{0,60}(?:b64decode|base64|atob)"
        ),  # skillscan:allow
        "OBF02",
        "CRITICAL",
        "Interpreter one-liner decoding embedded data",
    ),
]
...
NET_PATTERNS = [
    (
        re.compile(
            r"^\s*(?:import|from)\s+(?:requests|httpx|aiohttp|urllib3?|websockets?|socket|http\b)"
        ),
        "python network import",
    ),
    (
        re.compile(
            r"\burllib\.request\b|\bhttp\.client\b|\bsocket\.(?:socket|create_connection)\b"
        ),
    ...
    (re.compile(r"\b(?:curl|wget)\s+(?:-[A-Za-z-]+\s+)*[\"']?https?://"), "curl/wget invocation"),
    (re.compile(r"\b(?:nc|ncat|netcat)\s+[\w.-]+\s+\d{2,5}\b"), "raw netcat connection"),
]
...
CRED_PATTERNS = [
    (
        ...
        "CRED01",
        "CRITICAL",
        "Reads SSH key material (SSH dir, id_rsa, id_ed25519)",
    ),  # skillscan:allow
    (
        re.compile(r"\.aws/credentials|\.aws/config"),  # skillscan:allow
        "CRED01",
        "CRITICAL",
        "Reads AWS credential files",
    ),
    (
        re.compile(
            r"(?i)security\s+(?:find-generic-password|find-internet-password|dump-keychain)"
        ),
        "CRED01",
        "CRITICAL",
        "Queries the macOS keychain from a script",
    ),
    (
        re.compile(r"\.netrc\b|\.npmrc\b|\.pypirc\b"),  # skillscan:allow
        "CRED01",
        "WARN",
        "Touches credential-bearing dotfiles (netrc/npmrc/pypirc)",
    ),  # skillscan:allow
    (
        re.compile(r"gcloud/(?:credentials|application_default_credentials)"),
        "CRED01",
        "WARN",
        "Reads gcloud credential files",
    ),
    (
        re.compile(
            r"(?i)(?:Login Data|Cookies)['\"]|browser.{0,20}profile.{0,20}(?:passw|cookie)"
        ),  # skillscan:allow
        "CRED01",
        "WARN",
        "References browser credential/cookie stores",
    ),
    (
        re.compile(
            r"(?i)(?:exodus|electrum|phantom|solana)[^\n]{0,40}(?:wallet|keystore|id\.json)"
        ),  # skillscan:allow
        "CRED01",
        "WARN",
        "References cryptocurrency wallet storage",
    ),
    (
        re.compile(
...
FETCH_CMD_RE = re.compile(
    r"(?i)\b(?:curl|wget|iwr|irm|invoke-webrequest)\b[^\n]*https?://"
)  # skillscan:allow
...
def network_declared(fm):
    blob = (
        " ".join(str(fm.get(k, "")) for k in ("compatibility", "description")).lower() if fm else ""
    )
    ...
    return any(w in compat for w in NETWORK_DECLARE_WORDS), any(
        w in blob for w in NETWORK_DECLARE_WORDS
    )
...
def scan_skill(skill_dir, include_fixtures=False):
    ...
    def add(check, sev, file, line, msg, evidence):
        findings.append(
            Finding(skill_name, check, sev, str(file.relative_to(skill_dir)), line, msg, evidence)
        )
    ...
    if fm is None:
        add(
            ...
            skill_md,
            1,
            "SKILL.md has no YAML frontmatter (--- block). Agents cannot discover this skill safely.",
            text[:80],
        )
    else:
        name = fm.get("name", "")
        ...
        if name and not NAME_RE.match(name):
            add(
                ...
                skill_md,
                1,
                "Frontmatter name '%s' violates spec (lowercase a-z, 0-9, hyphens)." % name,
                name,
            )
        ...
        if not desc:
            ...
            add(
                ...
                skill_md,
                1,
                "Description exceeds 1024 chars (spec limit).",
                desc[:60],
            )
        ...
        if in_fence and FETCH_CMD_RE.search(line):
            if i <= lure_window_until:
                ...
                add(
                    ...
                    skill_md,
                    i,
                    "SKILL.md contains a command that fetches remote content; verify the destination and pin it.",
                    line,
                )
    ...
    for path in iter_scan_files(skill_dir, include_fixtures=include_fixtures):
        ...
        for i, line in enumerate(content.splitlines(), 1):
            ...
            def doc_sev(sev, in_doc_fence=in_doc_fence):
                return "INFO" if (in_doc_fence and sev == "WARN") else sev

            def doc_msg(sev, msg, in_doc_fence=in_doc_fence):
                    ...
                    if not is_skill_md:
                        ...
                        extra = (
                            ...
                            if net_declared
                                ...
                                "inside a documentation code example"
                                if in_doc_fence
                                else "not declared in frontmatter 'compatibility'"
                            )
                        )
                        add(
                            "NET01",
                            ...
                            path,
                            i,
                            "Script makes network calls (%s), %s." % (sev_msg, extra),
                            line,
                        )
...
def main(argv=None):
    ...
    if not root.exists():
        print(
            ...
            "e.g. python3 skill_scanner.py PATH/skills/some-skill" % root,
            file=sys.stderr,
        )  # skillscan:allow
        return 2
    ...
    if not skills:
        print(
            "error: no SKILL.md found under %s\n"
            "Fix: agent skills are directories with a SKILL.md at their root. "
            "If you meant to scan a single file, name it SKILL.md or pass its parent directory."
            % root,
            file=sys.stderr,
        )
        return 2
            ...
            json.dumps(
                {
                    ...
                    "version": VERSION,
                    "root": str(root),
                    "skills_scanned": [str(s) for s in sorted(skills)],
                    "findings": [f.as_dict() for f in all_findings],
                    "summary": counts,
                    "verdict": (
                        "REJECT-PENDING-REVIEW"
                        if counts["CRITICAL"]
                        else ("REVIEW-WARNINGS" if counts["WARN"] else "PASS")
                    ),
                },
                indent=2,
            )
