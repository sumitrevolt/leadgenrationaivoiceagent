@echo off
REM AI reply triage agent + scheduler wiring. Log -> cowork_deploy8.log
cd /d C:\Users\Ratanshila\Documents\leadgenrationaivoiceagent
set LOG=cowork_deploy8.log
...
echo === py_compile === > %LOG%
call python -m py_compile app\platform\reply_agent.py app\platform\team_scheduler.py >> %LOG% 2>&1
echo EXIT_PYCOMPILE=%errorlevel% >> %LOG%
echo === git add/commit/push === >> %LOG%
call "C:\PROGRA~1\Git\cmd\git.exe" add -A >> %LOG% 2>&1
call "C:\PROGRA~1\Git\cmd\git.exe" commit -m "feat(automation): AI reply-triage agent (IMAP->classify->draft->notify) + hourly scheduler, gated REPLY_AGENT" >> %LOG% 2>&1
...
echo. >> %LOG%
echo === QA: prod_check (imports reply_agent + team_scheduler edits) === >> %LOG%
