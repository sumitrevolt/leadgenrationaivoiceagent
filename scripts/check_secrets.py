...
PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    # Bare `sk-` was missing until 2026-09-22. The alternation only knew `gsk`,
    # `sk-proj` and `sk-ant`, so a plain `sk-<64-hex>` key was INVISIBLE — and four
    # `scripts/*.js` files on PUBLIC main carried exactly that shape as a hardcoded
    # fallback (`apiKeyMatch ? … : 'sk-…'`), the same "credential as source default"
    # anti-pattern as the Telethon one. Requires 32+ chars to stay well clear of
    # prose and of the `sk_` (underscore) Pollinations pattern above.
    (
        "Groq/OpenAI-style key",
        re.compile(r"\b(?:gsk|sk-proj|sk-ant)[-_][A-Za-z0-9_\-]{20,}|\bsk-[A-Za-z0-9_\-]{32,}"),
    ),
