    ...
    with (
        ...
        for row in reader:
            ...
            if score > 85 or "Agency" in row.get("industry", ""):
                ...
                row["pain_hypothesis"] = (
                    "Manual SEO audits consume too much team throughput. Needs automated AI delivery."
                )
            else:
                row["recommended_offer"] = "leadgen_combo"
                row["pain_hypothesis"] = (
                    "Struggling to capture inbound intent effectively. Needs immediate AI callback + audit."
                )
