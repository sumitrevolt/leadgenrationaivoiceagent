...
print(f"  GROQ_API_KEY: {'SET (len ' + str(len(g)) + ')' if g else 'MISSING ❌ — STT fail -> agent sun nahi payega'}")
...
try:
    ...
    print(f"  _edge_tts_mp3_b64 -> {('OK ' + str(r) + ' b64-chars' ) if isinstance(r,int) and r>1000 else ('PROBLEM: ' + str(r))}")
except Exception as e:
    print(f"  [ERR] {e}")
...
async def _brain_test():
    try:
        ...
        r = await b.reply([{"role": "assistant", "content": "Namaste"}], "bijli ka bill bahut zyada aata hai")
        return r
    except Exception as e:
        return f"FAIL: {e}"
try:
    ...
    print(f"  -> brain {'OK (reply deta hai)' if r and not str(r).startswith('FAIL') else 'PROBLEM'}")
except Exception as e:
    print(f"  [ERR] {e}")
