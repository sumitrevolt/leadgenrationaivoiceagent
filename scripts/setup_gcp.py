...
def run_command(
    cmd: list, dry_run: bool = False, capture_output: bool = True, check: bool = True
) -> str | None:
...
def enable_apis(project_id: str, dry_run: bool = False):
    ...
    for i, api in enumerate(REQUIRED_APIS, 1):
        try:
            run_command(
                ["gcloud", "services", "enable", api, "--project", project_id], dry_run=dry_run
            )
...
def create_artifact_registry(project_id: str, region: str, dry_run: bool = False):
    ...
    try:
        run_command(
            [
                ...
                project_id,
                "--description",
                "LeadGen AI Voice Agent Docker images",
            ],
            dry_run=dry_run,
        )
...
def create_cloud_sql(project_id: str, region: str, dry_run: bool = False):
...
def create_redis(project_id: str, region: str, dry_run: bool = False):
    ...
    try:
        run_command(
            [
                ...
                sa_name,
                "--display-name",
                "LeadGen Cloud Run Service Account",
                "--project",
                project_id,
            ],
            dry_run=dry_run,
...
def print_summary(project_id: str, region: str):
    ...
    print(
        f"     python scripts/setup_secrets.py --project-id {project_id} --env production --interactive"
    )
    ...
    print("  # View Cloud Run logs")
    print(f"  gcloud run services logs read leadgen-ai-agent --region={region}")
    print("  ")
    print("  # Get Cloud SQL connection name")
    print("  gcloud sql instances describe leadgen-db --format='value(connectionName)'")
    print("  ")
    print("  # Get Redis IP")
    print(
        f"  gcloud redis instances describe leadgen-redis --region={region} --format='value(host)'"
    )


def main():
    parser = argparse.ArgumentParser(description="Set up GCP project for LeadGen AI Voice Agent")
    parser.add_argument("--project-id", required=True, help="GCP project ID")
    parser.add_argument("--region", default="asia-south1", help="GCP region (default: asia-south1)")
    parser.add_argument("--dry-run", action="store_true", help="Show commands without executing")
    parser.add_argument(
        "--skip-database", action="store_true", help="Skip Cloud SQL and Redis creation"
    )
