...
# 1. OmniRoute Gateway (Port 20128) — Docker (ADR-189: WSL removed)
...
Write-Output "=== 12 OmniRoute Combos Master Desktop Sync & Launcher (Docker) ==="
...
# 2. Verify OmniRoute Gateway (Port 20128) — Docker
Write-Output "`n[1/4] Checking OmniRoute Gateway (Port 20128)..."
try {
    $omniResp = Invoke-RestMethod -Uri "http://127.0.0.1:20128/v1/models" -TimeoutSec 5
    $count = ($omniResp.data | Measure-Object).Count
    Write-Output " -> OmniRoute Gateway ONLINE: $count combos loaded."
} catch {
    Write-Output " -> Starting OmniRoute via Docker Compose..."
...
Write-Output "2. Claude Desktop App: MCP servers synced (models stay on Claude sub - NOT gateway-wired)"
...
Write-Output "5. OmniRoute Dashboard: http://127.0.0.1:20128 (Docker, loopback-only)"
Write-Output "======================================================="