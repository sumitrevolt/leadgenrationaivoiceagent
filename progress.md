...
## Loop Run — 2026-09-07 06:00 IST (Council cycle 9 — OPS-017: DND_CARRIER_SCRUB was a global fail-open ARMED BY A DEPLOY SCRIPT)
- **Goal:** Close the last known way a single environment variable can silently open the §5 TRAI gate. Authority: local code + tests only — **no deploy, no SSH, no remote state change**. Nothing in this cycle loosens a gate; it removes a bypass.
- **Inspected:** `app/utils/dnd_checker.py::_check_via_registry` (carrier-scrub branch) · `scripts/vps_deploy_call_learn.py:23` + `scripts/vps_deploy_call_learn.bat:33` · `docs/SESSION_LOG.md:1744` · `docs/SWARA_HANDOFF_SOP.md:118/158/316/422` · all 4 real `DNDChecker` call sites (`app/telephony/compliance.py:276`, `app/telephony/call_manager.py:95` (assigned, never called), `app/tasks/whatsapp_automation.py:342`, `app/automation/orchestrator_pipeline.py:384`).
- **🔴 Problem Found (worst of the session so far):** `DND_CARRIER_SCRUB=1` returns `is_dnd=False, verified=True` for **EVERY number** with no per-number lookup. It was introduced for the **voice** path — and `scripts/vps_deploy_call_learn.py:23` arms it on the VPS (`env_set.py VOBIZ_CALL_RECORD=1 AUTO_QUALIFY_CALLS=1 DND_CARRIER_SCRUB=1 TELEPHONY_PROVIDER=vobiz DLT_APPROVED=1`; `docs/SESSION_LOG.md:1744` records cold-calling running with it). But `DNDChecker` is **shared**: `app/tasks/whatsapp_automation.py::_scrub_dnd()` — the promotional WhatsApp §5 gate — calls the same checker, and that function's own docstring asserted the opposite ("any number not already in the local cache / consent ledger returns UNVERIFIED and is therefore BLOCKED"). **One env var inverted the messaging gate, and the docstring lied about it.**
- **Council decision (D7): scope the allowance to voice, do not delete it.** Voice and messaging are legally different: TCCCPR 2018 permits a **call** to a DND number with documented consent (kept in `consent_ledger`); for **promotional messaging** NCPR scrubbing is mandatory and, per ADR §3.1, *no consent mechanism overrides a DND registration*. So "our carrier scrubs" is defensible for calls and indefensible for messages. Deleting the flag would have stopped live cold-calling — a revenue action that is not mine to take unattended.
- **Changed:** `app/utils/dnd_checker.py` — `CARRIER_SCRUB_CHANNELS={voice}`, `DEFAULT_CHANNEL="messaging"`, `carrier_scrub_armed()`, `carrier_scrub_verifies()`, `carrier_scrub_warning()` (loud, once, names OPS-017); `check_single/check_batch/filter_dnd` take `channel=`; messaging + armed flag falls through to `no_provider / verified=False` (§5 fails CLOSED); **a `vobiz_carrier_scrub` verdict is never cached** — otherwise a voice allowance gets laundered into the next messaging lookup · `app/telephony/compliance.py` → `channel="voice"` (with a `TypeError` fallback so older/double checkers keep working) · `app/tasks/whatsapp_automation.py` → `channel="messaging"` · `app/automation/orchestrator_pipeline.py` — stage 3 feeds **both** stage 5 (WhatsApp warm-up) and stage 6 (voice), so it scrubs on the **strictest channel in use**: messaging whenever WhatsApp is enabled (the default `channels=[whatsapp, voice]`), voice only for a voice-only run · `scripts/vps_deploy_call_learn.py` — loud compliance comment on the arming line · `tests/test_dnd_carrier_scrub_channel_scope.py` (**15 new**) · `tests/test_whatsapp_automation_body.py` — doubles now **assert** they are asked for the `messaging` channel · ADR §5.3 + D7 + §8 OPS-018 · ledger **OPS-017** (DONE) + **OPS-018** (owner, TODO).
- **Tests Run:** 12-suite combo → **72 passed, 1 failed** (the pre-existing `tests/test_compliance.py::test_dnd_fail_open_honoured_outside_production`, identical at HEAD). New suite pins: voice still verifies via carrier scrub · **messaging does NOT** (the regression guard) · omitted/unknown channel ⇒ strict · batch + `filter_dnd` channel-scoped · verdict never cached · opt-out still beats carrier scrub on voice · warning silent off/voice, loud on messaging, never raises.
- **Verification Evidence:** ruff **All checks passed** on all 6 changed files · `check_secrets` **OK (6 files)** · **`prod_check` EXIT=0 — `routes checked (1396 registered)` UNCHANGED, ALL CHECKS PASSED** · ledger **42 tasks, duplicate_ids=none** (`.bak-20260907-060002`).
- **Detect → Diagnose → Recover → Verify (three real ones):**
  1. **I broke 4 tests and caught it.** Adding `channel="voice"` in `compliance.py` made `tests/test_compliance.py` go 1 → 5 failures, because its doubles define `check_single(self, phone)`. Fixed with a `TypeError` fallback (real checker honours the channel; a double that cannot is called the legacy way instead of being misread as a lookup error). Same guard already in `orchestrator_pipeline.py`. Back to 1 pre-existing failure.
  2. **`git stash push` silently stashed my uncommitted cycle-9 edits** (printed "No local changes to save", then `git stash list` showed them as `stash@{0}`). Detected because `grep -c carrier_scrub_verifies app/utils/dnd_checker.py` returned **0** after the so-called HEAD comparison run — which would have made that comparison meaningless. Restored with `git stash pop`. **Lesson: after any git operation, re-verify the working tree actually contains your change before drawing a conclusion from a run.**
  3. **Environmental, not a gate failure:** `prod_check` died at `[1/6]` twice with `SAFE_DELETE_BULK_CONFIRM_REQUIRED` (count 181, threshold 50) — its step-1 hygiene deletes stale `.pyc` files and the sandbox bulk-delete counter was already at 181 for this turn. Fixed by running `python -m compileall -q app tests scripts` and then deleting the 3 remaining stale `.pyc`; after that `prod_check` completed EXIT=0. The earlier EXIT=1 was never a code failure.
- **Risks / honest limits:** local-only and **undeployed** — if `DND_CARRIER_SCRUB=1` is live on the VPS, the messaging gate is open **until OPS-017 ships**. `orchestrator_pipeline` stage 3 becomes strict whenever WhatsApp is enabled, so that harness will now keep zero leads without a real DND provider; that is the compliant answer, and the live cold-call path (`tasks/calling` → `compliance.py`) is unaffected because it declares `channel="voice"`. TCCCPR-vs-NCPR reasoning rests on the same secondary sources as ADR §3 — legal confirmation is the owner's call.
- **Remaining:** **OPS-018 (owner, 10 s, highest urgency): `grep DND_CARRIER_SCRUB /opt/leadgen/.env`** · A1 (`grep WHATSAPP_AI_AUTOREPLY /opt/leadgen/.env`) · OPS-011 · OPS-014 (owner/legal) · OPS-016 · D5 BSP · deploy cycles 2-9 · OPS-012 reboot confirmation · `upi_12_bd74bae8` · Kamal VPS record · Jiya city defect.
- **Next Highest Priority:** **OPS-018 (owner, 10 seconds) — confirms whether the promotional WhatsApp gate has been open in production**; then Jiya manual send (₹19,990); then A1; then OPS-011.

## Loop Run — 2026-09-07 06:25 IST (Council cycle 10 — OPS-019: sweep for other env vars that can open a §5 gate)
- **Goal:** OPS-017 proved one env var could silently open the §5 messaging gate. The obvious, non-negotiable follow-up: **how many other escape hatches can do that, and are they refused in production?** Authority: local code + tests only — no deploy, no SSH, no remote state change.
- **Inspected:** sweep for `FAIL_OPEN|CARRIER_SCRUB|SKIP|BYPASS|ALLOW_ALL|DRY_RUN` across `app/` · `app/telephony/compliance.py:160-192` (`_is_production`, `_dnd_fail_open`) · `app/integrations/whatsapp_selfhost.py:155-167, 295-308` (`_recipient_check_fail_open`) · `app/middleware/__init__.py:672-700` (`REQUEST_GUARD_SKIP`) · `app/config.py:349` · `app/platform/runtime_data.py:82` · `app/integrations/openclaw/policies.py:128`.
- **Problems Found:**
  1. **`WHATSAPP_RECIPIENT_CHECK_FAIL_OPEN=1` is a second global fail-open and NOTHING refused it in production.** Default is `0` (fail-closed) but that flag turns the *only* guard on the WAHA send path back into fail-OPEN — a recipient check that never completed (transport/HTTP error) would again fall through to a real send. `DND_FAIL_OPEN` has always been refused in prod with a one-time CRITICAL log; this one had no such guard. ❌ **FIXED.**
  2. **FOUR independent `is_production` implementations.** `telephony/compliance` (settings-first + env), `platform/runtime_data` (env only, also accepts `prod`), `integrations/openclaw/policies` (set membership over both vars), `config` (a settings property). Four answers to "are we in production?" is exactly how a safety flag ends up honoured in prod — the gate that refuses it and the gate that checks it can disagree. ❌ **FIXED for compliance** via one shared authority.
  3. `REQUEST_GUARD_SKIP` **replaces** the entire default skip list rather than extending it (`os.environ.get("REQUEST_GUARD_SKIP", _default_skip)`), so adding one path silently un-skips `/ws`, `/health`, `/api/voiceai`, `/agents/coordinate` — voice/WS paths would then get the 55 s request timeout and calls would be cut mid-flight. Availability foot-gun, **not** a compliance gate — reported as OPS-020, deliberately not changed (middleware behaviour).
- **Council decisions:** D8 reuse the proven `DND_FAIL_OPEN` pattern (dev/staging legitimate ⇒ keep the flag, refuse it in prod + one-time CRITICAL log) — do NOT delete, and do NOT make it a behaviour change outside production · D9 add `app/utils/env_probe.py` as the single authority rather than a fifth copy · D10 leave `runtime_data` and `openclaw` alone (unifying them is a behaviour change in two unrelated domains) — recorded as OPS-021.
- **Changed:** **new** `app/utils/env_probe.py` — `is_production()` (settings-first, `ENVIRONMENT`/`APP_ENV` fallback, never raises), `fail_open_refused(flag, consequence)`, `refusal_message(flag, consequence)` · `app/telephony/compliance.py` — `_is_production()` now **delegates** to it (behaviour identical), `_dnd_fail_open()` uses `refusal_message` and also refuses the `on` spelling · `app/integrations/whatsapp_selfhost.py` — `_recipient_check_fail_open()` refuses in production with a one-time CRITICAL log; a probe that raises **closes** the gate · `tests/test_fail_open_refused_in_production.py` (**17 new**) · ledger **OPS-019** (DONE) + **OPS-020** + **OPS-021** + **OPS-022**.
- **Tests Run:** 13-suite combo → **89 passed, 1 failed** (only the pre-existing `tests/test_compliance.py::test_dnd_fail_open_honoured_outside_production`, identical at HEAD). New suite pins: settings wins · env fallbacks (`ENVIRONMENT`, `APP_ENV`) · unset ⇒ False · **settings raising falls back to env without crashing** · `fail_open_refused` matrix · `refusal_message` names flag + consequence · **`WHATSAPP_RECIPIENT_CHECK_FAIL_OPEN` refused in production (the regression guard — returned True before this cycle)** · honoured outside production · **a broken probe closes the gate** · `DND_FAIL_OPEN` behaviour unchanged in all three cases · compliance and env_probe never disagree.
- **Verification Evidence:** ruff **All checks passed** (7 changed files, 3 auto-fixed `not in` nits) · `check_secrets` **OK (11 files)** · **`prod_check` EXIT=0 — `routes checked (1396 registered)` UNCHANGED, `2199 source files parsed` (+2 = env_probe + new test), ALL CHECKS PASSED** · ledger **46 tasks, duplicate_ids=none** (`.bak-20260907-062355`).
- **Detect → Diagnose → Recover → Verify:**
  1. **A second worker silently reverted my cycle-9 `TypeError` fallback in `app/telephony/compliance.py::_is_dnd`.** Symptom: `tests/test_compliance.py` went from 1 failure back to 5 (`dnd_lookup_failed` ×4). Diagnosed by reading the current source — the `try/except TypeError` was simply gone. Re-added; back to 1. **This is the dangerous case: a silent revert inside a compliance gate, caught only because the suite was re-run.** Recorded as **OPS-022** (concurrent edits to `app/` by multiple agents; needs a per-file lock or ownership map).
  2. **`prod_check` EXIT=1 twice again** — safe-delete guard (`count 244 > threshold 50`) on its stale-`.pyc` hygiene step. Same fix as cycle 9: `python -m compileall -q app tests scripts`, then delete the remaining stale `.pyc`, then re-run ⇒ EXIT=0. Never a code failure.
- **Risks / honest limits:** local-only and **undeployed**. `env_probe.is_production()` returns **False** when the environment is unset — correct for "is it production", but a caller that needs *safety* must still treat unknown as the safe side itself; the module docstring says so explicitly. OPS-021 leaves two competing predicates in the tree. OPS-022 means any of these files may be edited underneath the next cycle — re-read before trusting.
- **Remaining:** **OPS-018 (owner, 10 s): `grep DND_CARRIER_SCRUB /opt/leadgen/.env`** · A1 (`grep WHATSAPP_AI_AUTOREPLY`) · OPS-011 · OPS-014 (owner/legal) · OPS-016 · OPS-020 · OPS-021 · **OPS-022 (concurrency discipline)** · D5 BSP · deploy cycles 2-10 · OPS-012 reboot confirmation · `upi_12_bd74bae8` · Kamal VPS record · Jiya city defect.
- **Next Highest Priority:** **OPS-022 — stop concurrent agents from silently reverting compliance-gate code** (it just removed a safety fallback mid-cycle); then OPS-018 (owner, 10 s); then Jiya manual send (₹19,990).

## Loop Run — 2026-09-07 06:45 IST (Council cycle 11 — OPS-023: pin the channel-forwarding contract so a silent revert fails loudly)
- **Goal:** OPS-022 proved a concurrent worker can remove compliance-gate code silently. Audit what would NOT be caught, and close it. Authority: local tests only — no production code behaviour change, no deploy, no SSH.
- **Inspected:** which of the OPS-017 call sites are actually pinned by a test · `app/telephony/compliance.py::_is_dnd` · `app/automation/orchestrator_pipeline.py::_stage_dnd_scrub` · `app/tasks/whatsapp_automation.py::_scrub_dnd` · `inspect.signature(DNDChecker.check_single)`.
- **Problems Found:**
  1. **Nothing pinned that the call sites forward `channel=` at all.** Delete `channel="voice"` from `compliance.py` and **every test stays green** — while voice silently loses its carrier-scrub allowance and the live cold-call path quietly goes fail-closed (an outage that looks like "no leads", not like a compliance event).
  2. Same for `orchestrator_pipeline`: delete the strictest-channel logic and stage 5 would WhatsApp-blast leads that were only ever cleared for a call. Silent, and compliance-shaped.
  3. `test_wa_inbound_session` / `test_dnd_carrier_scrub_channel_scope` pin the *predicate*, not the *wiring*. Predicate-correct + wiring-reverted = green suite, broken gate.
- **Council decision:** pin the contract **behaviourally** (record what the checker is asked), not by scraping source text. Source-scraping tests break on every refactor and get deleted; a recording double survives refactors and still fails on a revert. No production code changed this cycle.
- **Changed:** **new** `tests/test_channel_forwarding_contract.py` (**13 tests**): `RecordingChecker` records `(phone, channel)`; asserts compliance forwards `"voice"` · transactional traffic does **not** consult the DND registry at all (documents that DND is a promotional-only obligation) · pipeline stage 3 scrubs `messaging` for `["whatsapp","voice"]`, `["whatsapp"]` and `None`/unknown, and `voice` only for `["voice"]` · WA automation forwards `messaging` · `CARRIER_SCRUB_CHANNELS == {"voice"}` and `DEFAULT_CHANNEL == "messaging"` (and the default is not in the allowlist) · `check_single`'s `channel` default is `"messaging"` via `inspect.signature` · promotional window constants `09:00-21:00` unchanged · ledger **OPS-023** (DONE).
- **Tests Run:** 14-suite combo → **103 passed, 1 failed** (only the pre-existing `tests/test_compliance.py::test_dnd_fail_open_honoured_outside_production`, identical at HEAD).
- **Verification Evidence:** ruff **All checks passed** (9 changed files) · `check_secrets` **OK (13 files)** · **`prod_check` EXIT=0 — `routes checked (1396 registered)` UNCHANGED, `2200 source files parsed`, ALL CHECKS PASSED** · ledger **47 tasks, duplicate_ids=none** (`.bak-20260907-063701`).
- **Detect → Diagnose → Recover → Verify:** one test failed twice on my own assumptions, not on the code — (a) `gate.check(..., TRANSACTIONAL)` with no `now` used wall-clock 00:59 IST and was blocked by the calling window before ever reaching DND, so `checker.calls` was empty; (b) after fixing the time, transactional still never consults DND — which is **correct** (DND is a promotional obligation). Rewrote that test to pin the real contract instead of forcing it to fire.
- **Risks / honest limits:** these tests pin wiring, so a legitimate future change (e.g. adding a `"sms"` channel) will turn them red by design — the docstring says to check *why* before changing an assertion. They do **not** protect against a worker deleting the test file itself; OPS-022 (concurrency discipline) is still open and is the real fix.
- **Remaining:** **OPS-022 (P1 — concurrent edits to `app/`)** · **OPS-018 (owner, 10 s): `grep DND_CARRIER_SCRUB /opt/leadgen/.env`** · A1 (`grep WHATSAPP_AI_AUTOREPLY`) · OPS-011 · OPS-014 (owner/legal) · OPS-016 · OPS-020 · OPS-021 · D5 BSP · deploy cycles 2-11 · OPS-012 reboot confirmation · `upi_12_bd74bae8` · Kamal VPS record · Jiya city defect.
- **Next Highest Priority:** **OPS-022** — until concurrency is disciplined, any of these files can be edited underneath the next cycle; then OPS-018 (owner, 10 s); then Jiya manual send (₹19,990).

## Loop Run — 2026-09-07 07:16 IST (OmniRoute Project-Best upgrade)
- **Goal:** Select and apply the strongest honest combo architecture after comparing legacy `leadgen-project-best` with all 14 canonical combos.
- **Inspected:** live combo DB, 42-slot seed, desktop sync, routing contracts, legacy manifest, provider/model catalog, direct inference behavior, relevant ADRs and incidents.
- **Problems Found:** 42 slots represented only 33 provider ids; only OpenCode/OpenCode-Zen were currently inference-proven; Groq/Cerebras had no active credentials and Gemini key was invalid; combo 12 description still claimed 50 models; desktop sync retained alias-shaped metadata.
- **Changed:** combo 12 is now canonical Project-Best with four explicit-free live heads, two live Big Pickle fallbacks, then catalog tail; description corrected to 42 models; desktop app ids normalized to canonical; unique 14-email and canonical-only contracts added.
- **Tests Run:** `tests/test_omniroute_scripts.py tests/test_omniroute_canonical_combos.py -q` => 18 passed, 1 documented xfail.
- **Verification Evidence:** live seed `SEED_14x42_OK`; DB read-back count=42 and expected heads; combo inference served by `nemotron-3.5-lightning-free` in 2816ms; `prod_check.py` ALL CHECKS PASSED; secrets scan clean; `git diff --check` only existing line-ending warnings.
- **Risks:** remaining 36 tail models are catalog-only until credential plus inference proof; desktop authentication remains separate from anonymous loopback canary; no production deploy performed.
- **Remaining:** provision/rotate valid provider credentials through approved admin flow, then health-probe and promote each provider independently; replace stale narrative-only manifest entries in a separate bounded cleanup.
- **Next Highest Priority:** provider credential onboarding and real per-provider free-tier canaries, starting with Groq, Cerebras, and Gemini.

## Loop Run — 2026-09-07 07:15 IST (Council cycle 12 — OPS-016: make AI scope flag-independent; found + fixed OPS-024)
- **Goal:** execute OPS-013 action **A3** (OPS-016): when `WHATSAPP_AI_AUTOREPLY=1` is armed, still exclude `other` from `_draft_intents`, so the WhatsApp agent can never become a general-purpose chatbot. Authority: local code + tests only. No deploy, no SSH, no remote state change, no compliance gate weakened.
- **Inspected:** `docs/OPS_013_WHATSAPP_AI_SCOPE_2026-09-07.md` §3 row 4 + §5 A3 · `app/platform/reply_agent.py::autoreply_policy_warning` (:1582), `whatsapp_reply` (:1609-1715), `_classify` (:689-734) · `tests/test_wa_conversation.py` (existing auto-send tests use `interested`, not `other` ⇒ safe) · `grep -rn WHATSAPP_AI_AUTOREPLY` (only `.env.example:225` `# WHATSAPP_AI_AUTOREPLY=0` outside the module).
- **Problems Found:**
  1. **OPS-016 (the drift vector itself).** `_draft_intents = ("interested","question","objection") + (("other",) if _auto else ())` — one env var turned a task-scoped sales bot into an open-ended LLM responder. That is the shape Meta's policy bars. ❌ **FIXED.**
  2. **OPS-024 — a P1 defect found by OPS-016's own failing test.** `_classify` resolved the label with `for c in _CATS: if c in lab` — a **substring** test in `_CATS` order. `"not_interested"` **contains** `"interested"`, and `"interested"` is checked first ⇒ **`not_interested` was unreachable**. Proven directly: `not_interested -> interested` (all other `_CATS` members correct). Impact, on BOTH the email and WhatsApp paths (`_classify` is shared): (a) rejections became **hot leads** — sales draft + Hot-Queue + `calling_priority: high`; (b) with auto-reply on, a sales pitch is **auto-sent to someone who just declined** — the fastest route to a block/report, i.e. the exact ban OPS-013 exists to prevent; (c) the auto-send guard `intent not in ("unsubscribe","not_interested","ooo")` was **dead code**, because `intent` was never `not_interested`. The code looked safe and was not. ❌ **FIXED.**
- **Council decisions:** D11 exclude `other` **unconditionally** (not "draft but don't send") — a human-reviewed draft for open-ended inbound would *widen* today's behaviour, and A3 as reviewed is the strictest reading · D12 additionally name `"other"` in the auto-send exclusion tuple as defence-in-depth, so a future re-widening of `_draft_intents` still cannot push open-ended replies · D13 fix `_classify` with **exact match first, word-boundary fallback second**, so `_CATS` *ordering* can never decide a verdict again.
- **Changed:** `app/platform/reply_agent.py` — (1) `_draft_intents` is now the constant `("interested","question","objection")` + OPS-016 comment; (2) auto-send guard → `intent not in ("unsubscribe","not_interested","ooo","other")`; (3) `_classify` exact-then-word-boundary matching (OPS-024); (4) `autoreply_policy_warning` docstring + message updated to describe the **capped** scope instead of a behaviour that no longer exists; (5) `whatsapp_reply` docstring states scope is flag-independent · **new** `tests/test_ops016_autoreply_scope.py` (**14 tests**) · **new** `tests/test_ops024_classify_exact_label.py` (**21 tests**) · **new** `docs/OPS_024_INTENT_CLASSIFY_EXACT_MATCH_2026-09-07.md` · `docs/OPS_013_...md` — row 4 and A3 marked closed, new §7 · ledger **OPS-016** (DONE) + **OPS-024** (DONE).
- **Tests Run:** 17-suite combo → **257 collected, 256 passed, 1 failed** (only the pre-existing `tests/test_compliance.py::test_dnd_fail_open_honoured_outside_production`, identical at HEAD). OPS-016 pins: `other` never drafted with the flag armed · never auto-sent · identical for all four truthy spellings (`1/true/yes/on`) · off-vs-on records are byte-identical · `interested/question/objection` **still** draft and send (narrowing, not amputation) · `unsubscribe/not_interested/ooo` never auto-send · warning text no longer claims `other` is drafted. OPS-024 pins: `not_interested` → `not_interested` · `not interested` / `Not-Interested` / `not_interested.` all resolve · **every `_CATS` member round-trips under both original AND reversed `_CATS` order** · prose labels ("I am interested", "label: question", "unsubscribe please") still resolve · junk still → `other`.
- **Verification Evidence:** ruff **All checks passed** · `check_secrets` **OK (17 files)** · **`prod_check` EXIT=0 — `routes checked (1396 registered)` UNCHANGED, ALL CHECKS PASSED** · ledger **49 tasks, duplicate_ids=none** (`.bak-20260907-071504`).
- **Detect → Diagnose → Recover → Verify:**
  1. **OPS-022 struck twice.** My `_classify` fix was present after the edit and **gone** before the test run (twice); the line-1702 `"other"` guard was also reverted once and re-applied. Caught only because I ran `grep -c "OPS-024"` immediately before pytest. A green suite after an edit is NOT proof the edit survived. **This is now the top operational risk.**
  2. **My own first fix was incomplete.** Normalising spaces to underscores broke the word-boundary `interested` fallback (`"I am interested"` → `"i_am_interested"`, no word boundary) — 4 tests red. Fixed by probing on a separator-collapsed string and matching each category with its underscores turned back into spaces.
  3. **The trailing-period case exposed the real ordering hazard**: `not_interested.` missed the exact match and then matched `interested` first. Fixed by collapsing **all** non-alphanumerics to spaces before probing, so `not_interested.` compacts to `not_interested` and hits the exact path.
- **Risks / honest limits:** both fixes are **local-only and undeployed** — prod `b4a457f2` vs local HEAD `a531833b`, so prod still misclassifies `not_interested` until cycles 2-12 ship. OPS-016 has **zero** effect in the shipped default (flag OFF), because `other` was already excluded there — it only removes a latent one-flag widening. OPS-024 **does** change live classification once deployed: some leads currently marked `interested` will correctly become `not_interested` and leave the Hot-Queue (that is the fix, but expect the hot-lead count to drop). `_classify` still falls back to `"other"` on unparseable output — unchanged fail-safe.
- **Remaining:** **OPS-022 (P1 — concurrent agents silently reverting edits in `app/`)** · **OPS-018 (owner, 10 s): `grep DND_CARRIER_SCRUB /opt/leadgen/.env`** · A1 (`grep WHATSAPP_AI_AUTOREPLY`) · OPS-011 · OPS-014 (owner/legal) · OPS-020 · OPS-021 · D5 BSP · **deploy cycles 2-12** · **re-examine historical Hot-Queue for false `interested` on `not_interested` replies** · OPS-012 reboot confirmation · `upi_12_bd74bae8` · Kamal VPS record · Jiya city defect.
- **Next Highest Priority:** **OPS-022** — two edits vanished this cycle; without a per-file lock or ownership map the next cycle can lose work the same way. Then **deploy cycles 2-12** (OPS-024 is a live-behaviour fix that does nothing until it ships), then OPS-018 (owner, 10 s).

## Loop Run — 2026-09-07 07:26 IST (OmniRoute Project-Best source-truth completion)
- **Goal:** finish the best project routing design across live OmniRoute and all five desktop apps.
- **Inspected:** live `/v1/combos`, direct provider canaries, seed, sync generator, distributor, manifest, active Hermes/OpenClaw/DSH/WorkBuddy/Verdant configs, focused contracts.
- **Problems Found:** stale manifest names; snapshot-first reconciliation; model-path provider inference disagreed with `providerId`; Hermes/OpenClaw preserved legacy routes after DB cleanup.
- **Changed:** canonical manifest with 14 unique emails and all-five-app coverage; Project-Best combo 12 free-first head; live-first reconciliation; providerId-first comparison; targeted active-config alias purge; OpenClaw default to combo 12.
- **Tests Run:** focused OmniRoute suite 20 passed, 1 documented xfail; thousand-engineers preflight pytest PASS.
- **Verification Evidence:** live reconcile 14/14, missing=0, mismatches=0, status=OK; active checked configs legacy refs=0; real combo 12 inference served by `nemotron-3.5-lightning-free`; preflight ruff/secrets PASS; prod_check ALL CHECKS PASSED (1394 routes).
- **Risks:** 31 provider IDs remain catalog-only until account credentials and live inference prove them; 42 model slots are not 42 verified providers.
- **Remaining:** owner-assisted provider account/API-key onboarding, then one-provider-at-a-time health promotion.
- **Next Highest Priority:** Groq, Cerebras and Gemini credential repair followed by bounded free-tier canaries.

## Loop Run — 2026-09-07 08:00 IST (Council cycle 13 — OPS-026: sweep the OPS-024 bug class; found one more, deliberately NOT changed)
- **Goal:** OPS-024 (substring match over an ordered constant) was found by accident in cycle 12. Generalise: sweep `app/` for the same shape and assess every instance, so the next one is found on purpose. Authority: local inspection + local tests. No deploy, no SSH, no remote state change, no compliance gate weakened.
- **Inspected:** regex sweep of `app/` for `for X in <CONST>: if X in <input>: …` (both any-match detectors and first-match-wins classifiers) · `app/api/ratelimit.py::_client_tier` (:101-122) + `_TIER_MULT` (:90-98) · `app/api/ai.py:19` (router-level `tier_rate_limit("ai", 30, 60)`) · real plan vocabulary (`customer_auth._VALID_PLANS`, `customer_onboard._MKT_PLANS`, `billing/usage.PLAN_MINUTES`) · `app/ml/codebase_indexer.py:341-347` · `app/niche_knowledge.py:235-243` · `app/voice_agent/guardrails.py:572-577`.
- **Problems Found:**
  1. **One more instance of the OPS-024 class, in a budget path.** `app/api/ratelimit.py:117` — `for key in _TIER_MULT: if key in t: return key`. `_client_tier` decides the budget for the **entire `/ai` router** (`app/api/ai.py:19`). ⚠️ **LATENT, NOT ACTIVE** — and deliberately left unchanged (see decision D14).
  2. Seven other `if marker in text` sites are **any-match detectors** (banned words, prompt injection, secret fragments, error markers). They return a boolean, so ordering cannot mislabel anything — benign by construction. Cleared.
  3. Two low-stakes first-match-wins lookups left alone and documented: `ml/codebase_indexer.py:344` (file-pattern → agent role, indexing label only) and `niche_knowledge.py:236` (objection → rebuttal; dict order decides which rebuttal wins when keys overlap — copy quality, not money or compliance).
  4. `app/voice_agent/guardrails.py:572-576` already does it **right** (word-boundary `re.search` first, substring only as fallback) — the pattern OPS-024's fix adopted.
- **Council decision D14 — do NOT fix `_client_tier` unattended.** The tempting one-liners are all net-negative: (a) **exact match only** ⇒ a plan like `"voice_4999"` drops 8x → 1x, i.e. throttling paying voice customers **8x harder** — revenue-visible harm; (b) **longest match wins** ⇒ actively *worse*, `"free_admin"` would match `"admin"` (5 chars) over `"free"` (4) and inherit 20x; (c) **lowest multiplier wins** ⇒ behaviour-identical to today for every realistic string, i.e. a no-op refactor. Justification for inaction: the tenant plan is **server-derived** (the spoofable `X-Client-Tier` header was removed 2026-07-01, so no remote exploit path), no real plan string (`free/trial/starter/growth/advanced/voice/admin`) contains another so **substring == exact today**, and the fail-safe direction is already correct (unknown/missing/broken tenant ⇒ `"free"`, the smallest budget). Rate limiting is an availability/cost control, not a compliance gate — this needs the owner's plan-vocabulary decision, not an unattended edit.
- **Changed:** **new** `tests/test_ops026_rate_tier_resolution.py` (**19 tests**) — no production file modified this cycle, on purpose. Pins: every real plan resolves to itself · **no tier key is a substring of another** (the OPS-024 class invariant) · resolution is identical under **reversed** `_TIER_MULT` order · the real plan vocabulary is collision-free · unknown/empty/non-dict/missing tenant state still ⇒ `"free"` · **a tripwire asserting the current over-grant** (`"not_admin" ⇒ "admin"` 20x) so fixing OPS-026 later is a visible event rather than silent drift · **new** `docs/OPS_026_RATE_TIER_SUBSTRING_2026-09-07.md` · ledger **OPS-026** (DONE).
- **Tests Run:** 18-suite combo → **276 collected, 275 passed, 1 failed** (only the pre-existing `tests/test_compliance.py::test_dnd_fail_open_honoured_outside_production`, identical at HEAD). 19 new OPS-026 tests all green.
- **Verification Evidence:** ruff **All checks passed** · `check_secrets` **OK (22 files)** · **`prod_check` EXIT=0 — `routes checked (1396 registered)` UNCHANGED, ALL CHECKS PASSED** · ledger **50 tasks, duplicate_ids=none** (`.bak-20260907-072730`).
- **Detect → Diagnose → Recover → Verify:** no revert incident this cycle (contrast cycle 12, where OPS-022 struck twice) — no file under `app/` was edited, which is itself the mitigation. The interesting failure mode was **self-inflicted and caught by reasoning, not by a test**: my first instinct was "make it exact", which would have silently throttled paying customers. Enumerating the real plan vocabulary before touching the code is what stopped it.
- **Risks / honest limits:** the sweep was **pattern-based**, so it finds the shape, not every semantic instance — a hand-rolled matcher that avoids `for … in CONST` would be missed. "No collision today" is asserted against the plan vocabulary found **by grep**; if the VPS `.env` or DB holds a plan string absent from the code, collision test 4 cannot see it. OPS-026 is therefore reported, not closed — the tripwire only fires for strings someone writes into the codebase.
- **Remaining:** **OPS-022 (P1 — concurrent agents silently reverting edits in `app/`)** · **OPS-018 (owner, 10 s): `grep DND_CARRIER_SCRUB /opt/leadgen/.env`** · A1 (`grep WHATSAPP_AI_AUTOREPLY`) · OPS-011 · OPS-014 (owner/legal) · OPS-020 · OPS-021 · **OPS-026 owner decision (canonical plan → tier map)** · D5 BSP · **deploy cycles 2-12** · re-examine historical Hot-Queue for false `interested` · OPS-012 reboot confirmation · `upi_12_bd74bae8` · Kamal VPS record · Jiya city defect.
- **Next Highest Priority:** **OPS-022** (still the top risk — it cost two edits last cycle) then **deploy cycles 2-12** (OPS-024 is inert until it ships) then OPS-018 (owner, 10 s).

## Revenue War Room — 2026-09-07 08:30 IST (Sprint Day 5 of 8)
- **Deliverable:** `docs/REVENUE_WAR_ROOM_2026-09-07.md`.
- **Prod:** `/health` healthy, `environment: production`, **`bdf82eec`**, uptime 2h43m (restart ≈05:47 IST). Deploy ran 05:39 IST today (owner-gated). `git merge-base --is-ancestor bdf82eec HEAD` → YES; local HEAD `82e3ea17` ahead by 2 **docs-only** commits ⇒ **no code drift**.
- **Revenue truth UNREACHABLE (4th run) — but root cause CHANGED.** The 09-06 diagnosis (JWT-only, no API-key branch) is **stale in prod**: OPS-008 is deployed (`git show bdf82eec:app/api/auth_deps.py | grep -c OPS-008` → 1; `config.py:35 ops_readonly_token`; `ops_mcp_tools.py` lines 41 + 105 use `require_admin_or_ops_readonly`). 401 persists **only** because the env var is unset/unknown — `_ops_readonly_allows()` is fail-closed on empty (`auth_deps.py:149-151`) and local `.env` has **0** `OPS_READONLY_TOKEN` matches. Cannot distinguish "unset on VPS" vs "unknown value" — both give 401; owner-gated to resolve.
- **Collected today: ₹0 confirmed** (NOT "confirmed zero"). Net-new sprint Day 1–4: **₹0 evidence-backed**. Gaps: Floor **₹9,995** (pace ₹2,499/day), Base **₹16,000** (₹4,000/day), Stretch **₹25,000** (₹6,250/day) — **4 days left** (Sep 7–10).
- **Local data is 100% test fixtures** — `data/revenue_attribution.jsonl` (12 rows, all `Example Business`, `amount_inr:0`), `data/deals.jsonl` (2 rows: `Lead`/9876543210, `Example Business`/owner@biz.in), `data/cadence_leads.jsonl` (1 row), `data/wa_conversations.jsonl` (inbound only, all `"mid":""`). No `invoices.jsonl` / `upi_payments.json` locally. **Do not read as pipeline.**
- **BLK-11 status CORRECTION (split verdict):** **manual send PROVEN WORKING** — real WAHA ids `3EB00CFC09FB70376AA279` (09-05 07:01 IST) + `3EB0767664B1732E444721` (08:14 IST) to `197126499872961`. **Automated sweep STILL UNPROVEN** (`esc_0904_1252.jsonl` `"wa_msg_id":0`, `"wa_auto_sent_none":1829`). ⇒ BLK-11 no longer blocks today's manual cash; re-rank below automation backlog, do not delete.
- **Actual #1 blocker today: OPS-011** (read-only ops token not armed) — council ledger `scripts/council_ledger_sync.py:390-405`: status **BLOCKED**, **P1**, deadline **2026-09-07T21:00 IST**. It is why 4 consecutive closes were blind; 4 more blind days if skipped. #2 = **BLK-01 residual `upi_12_bd74bae8`** (open 16 days since 08-22, payment-authorization gate).
- **Verified RESOLVED (stop re-litigating):** existing-customer suppression in hot-queue pack **IS DEPLOYED** — `git merge-base --is-ancestor 8546c170 bdf82eec` → YES (09-04 flagged it local-only; that is now stale).
- **Actions A1–A5:** (A1) send Jiya ₹19,990 annual — draft `data/outreach_drafts/JIYA_SEND_READY_2026-09-07.txt` **exists on disk** (4,564 B) unlike the phantom 09-04 file; (A2) 3rd touch on inbound `197126499872961` + **stop rule** → NOT-INTERESTED if no reply; (A3) arm `OPS_READONLY_TOKEN` on VPS (+mutation must stay 401/403); (A4) Kamal Advanced ₹5,999/mo or lock ₹1,999 renewal — **renewal date unknown, needs A3 first**; (A5) decide `upi_12_bd74bae8` + check 09-07 hot-queue pack has **0** jiya/kamal rows.
- **Guardrails intact:** manual UPI only, `owner_confirmed_upi`, no `PROVIDER_VERIFIED`; cold WA OFF; email cap 25/day; no DND/TRAI/consent/opt-out change; no deploy/SSH/remote state change.
- **Next run checks:** A1 non-null WAHA id · reply from `197126499872961` · **revenue-summary 200?** · `upi_12` decision · Kamal renewal date · hot-queue pack suppression count.

### Revenue War Room 2026-09-07 — addendum 08:42 IST (price re-verification + one correction)
- **Every ₹-figure in A1–A5 re-verified from source this run** (09-06 had inherited them un-rechecked). New §3.1 in `docs/REVENUE_WAR_ROOM_2026-09-07.md`.
- `app/marketing/packages.py` — starter **₹1,999/mo, ₹19,990/yr** (:192-196) · growth ₹2,999/₹29,990 (:206-212) · advanced **₹5,999/mo, ₹59,990/yr** (:239-246). Line 12 = not niche-gated; grep `available_for|allowed_niche|niche_gate|restricted|eligible` → **0 hits**.
- ⚠️ **CORRECTION — ₹2,999 "Growth" is NOT sellable.** `packages.py:207-212` → `public: False`, `marketing_only: True`, tagline "Legacy/internal only — hidden from public pricing". I nearly recommended it as a soft step for Kamal; caught before issuing. **Only sellable step up from Starter is Advanced ₹5,999/mo.** Do not repeat in future runs.
- `app/marketing/voice_packages.py:38-84` — all 5 bands confirmed: S ₹1,999 (100 min) · F ₹0 (10 calls) · A ₹4,999/₹49,990 · B ₹9,999/₹99,990 · C ₹19,999/₹1,99,990. Matches the A2 draft. Harmless copy drift: draft says band-B "CA & Legal", code says "Modular Kitchen … CA" — no price impact; use the code's wording with the lead.
...
- **Date:** 2026-09-12 (13:00–13:20 IST)
- **Goal:** Finish the in-flight Smartflo routing work on `cherry-pick-smartflo-probe` — make outbound AI (stream) calls actually dial on the Smartflo rail, without weakening any compliance gate.
- **Inspected:** worktree diff of `app/api/telephony_vobiz.py` + `app/tasks/calling.py` + `tests/test_vobiz.py`; `app/telephony/tata_smartflo_handler.py::place_call`; `app/telephony/vobiz_handler.py::place_call`; `app/telephony/voice_launch.py::record_provider_result`; `app/tasks/calling.py::_dial_vobiz_campaign`; `app/telephony/telephony_service.py::_detect_provider`; `app/main.py:405-411`; `tests/test_voice_launch_kill_failclosed.py::_harness`; `frontend/admin_dashboard.html:4579`.
- **Problems Found:**
  1. **P0 — Smartflo compliance blocks were reported as provider FAILURES.** The new Smartflo branch tested `result.get("blocked")`, which is Vobiz's contract only (`vobiz_handler.place_call` → `{"status_code": 0, "blocked": True}`); `TataSmartfloClient` returns `{"status_code": 0, "body": {"error": "compliance_blocked: …"}}`. So `_dial_vobiz_campaign` never saw `error == "compliance_blocked"` → the lead counted as `fail`, and `voice_launch.record_provider_result(False, "")` counted a strike → **circuit breaker trip → campaign PAUSED_BY_CIRCUIT_BREAKER** on legally-correct refusals.
  2. **RED suite (5 tests) — `_dial_vobiz_campaign` lost its provider pre-check.** `tests/test_voice_launch_kill_failclosed.py` (4 parametrised cases + the invalid-token case) assert `client_created == 1` / `available == 1` BEFORE the kill gate; the diff had deleted the `VobizClient()` construction, so the fence was never reached. Pre-existing RED on the branch, not caused by this loop.
  3. **Admin manual-call card dead on the Smartflo rail.** `POST /api/telephony/vobiz/stream-call` (the dashboard's manual AI call) hardcoded `VobizClient` → 503 "Vobiz not configured" on a Smartflo-only prod.
  4. **Duplicated provider resolution** (inline in `start_stream_call`) with no single source of truth.
- **Changed:** `app/api/telephony_vobiz.py` — new `resolve_stream_provider()` (single routing authority: explicit `TELEPHONY_PROVIDER` wins, else Smartflo only when creds AND `TATA_SMARTFLO_ENABLED` are armed, else Vobiz) + `stream_provider_ready()` fail-fast; Smartflo branch of `start_stream_call` now surfaces `error == "compliance_blocked"` verbatim and a named error on any other rejection; `/stream-call` is provider-aware (Smartflo path delegates to `start_stream_call`; Vobiz path byte-identical) with `_log_manual_stream_call()` extracted. `app/tasks/calling.py` — provider-aware fail-fast (`stream_provider_ready()`) replacing the Vobiz-only gate, plus the result-contract note. Tests: `tests/test_vobiz.py` (`TestResolveStreamProvider` incl. a regression guard that creds alone never arm Smartflo; Smartflo block / rejection / unconfigured contracts), `tests/test_campaign_launch.py` (end-to-end: a Smartflo block is `skip`, never `fail`), `tests/test_admin_manual_customer_call.py` (Smartflo rail places + 422 on block). **No compliance gate touched; no dial placed; no commit/push/deploy.**
- **Tests Run:** 31 telephony/voice/console suites (513 tests) + the admin-manual-call, 429-contract and Vobiz suites.
- **Verification Evidence:** **513 passed / 0 failed** (exit 0, all 31 suites); the 5 previously-RED kill-fence tests are green **with zero test edits** (the suite already specified the wiring — the pre-check just had to be provider-aware); `import app.main` → `APP_IMPORT_OK`; `ruff check` on all 5 changed files → clean; `scripts/check_secrets.py` → `[OK] no secrets detected` (18 files).
- **Risks:** (a) Smartflo `custom_identifier` → the stream's `customParameters` is **provider-documented, not live-proven** — `opening_line`/`template_id`/`voice_role` are Vobiz-pending-store only, so a Smartflo auto-callback greets with the niche script, not the custom opening line; watch the first live call's `start schema` log. (b) `stream_provider_ready()` is the seam the kill-fence harness monkeypatches — it must keep importing clients from the DEFINING modules at call time. (c) `SMARTFLO_WEBHOOK_SECRET` unset → webhook gate fail-OPEN (pre-existing, unchanged).
- **Remaining / Owner gates:** run `scripts/deploy_vps.sh` (owner ask) so prod gets `resolve_stream_provider`; verify on the first live call that the stream `start` frame carries `customParameters` (`custom_identifier` echo) — that is what decides whether niche/client attribution survives on the Smartflo rail; the Smartflo console DID → VOICE Bot destination is still the inbound blocker.
- **Next Highest Priority:** deploy this routing fix (owner ask) and reconcile the FIRST live call's logs (`start schema`, `NO provider call id on the start frame`, `call_meter` dedupe) against the double-billing note in the `leadgen-smartflo-prod-ops` skill.

---

## Loop Run

- **Date:** 2026-09-12 (13:25–13:45 IST) — DEPLOY ATTEMPT: BLOCKED, plus a P0 discovered on prod
- **Goal:** Deploy the Smartflo stream-call routing fix to prod (owner ask) and reconcile the first live call against the double-billing note.
- **Inspected:** `scripts/deploy_vps.sh`, `scripts/_deploy_candidate.sh`, `scripts/_deploy_gate_container.sh` (read in full); live prod `/health` + `/opt/leadgen` checkout + container env (read-only SSH, names/booleans only); `git diff --name-only 33189b70..20c9beb4`; prod `data/cdr/smartflo_cdr.jsonl` + `data/call_transcripts/` + `leadgen_app`/`leadgen_worker` logs.
- **Problems Found:**
  1. 🔴 **P0 — LIVE compliance regression on prod: the TRAI DND fail-open guard has been REMOVED, and the running container carries that build.** `/app/app/telephony/compliance.py` md5 `3ef8680a…` (running container) == the VPS's dirty checkout file `3ef8680a…` ≠ git HEAD `34df4eec…`. `_dnd_fail_open()` now reads "Works in all environments including production" with the `_is_production()` refusal deleted, and the container has **`DND_FAIL_OPEN=1` + `APP_ENV=production`** ⇒ a failed/unverified DND lookup is treated as "not on DND", so promotional dials can reach DND-registered numbers. Byte-identical to `tmp/fix_dnd.py` (a gate-weakening script refused in earlier sessions). Contradicts `AGENTS.md` §5; the code's own comment says there is NO legitimate prod use.
  2. **Deploy is mechanically blocked (fail-closed, good).** The live checkout is dirty in exactly the files the incoming commits touch: `M app/telephony/compliance.py` + `M scripts/prod_check.py`; `db5b1ceb`/`68abd40a`/`61feb9cd` all modify `scripts/prod_check.py` ⇒ `git pull --ff-only` will refuse and the canonical script exits 2 before any container is replaced.
  3. **Blast radius is not "a small fix".** `33189b70..20c9beb4` = **594 files**, incl. the `68abd40a` Creative Engine OS + Video Renderer + Admin Command Center merge and the commit that removes the deploy kill-fence.
  4. **Deploying the fix flips the LIVE calling rail.** Prod already runs `TELEPHONY_PROVIDER=tata_smartflo` + `TATA_SMARTFLO_ENABLED=1` with `VOBIZ_AUTH_*` also set, but prod code (`33189b70`) has no provider routing ⇒ today's AI stream calls dial on **Vobiz**. This fix makes them dial **Smartflo** — whose outbound C2C binding and console Voice-Bot destination are still unverified.
  5. **The requested reconciliation is impossible right now — there is no first live call.** CDR = 10 rows, ALL of them probes/smoke tests (`status=probe`/`unknown`, `call_id=unknown`, `ref_id=probe`, two rows literally named `test-smoke-$(date +%s)` — unexpanded shell text). 0 `smartflo_*` transcripts. Every `/api/telephony/smartflo/stream` WS connection in the log is from the VPS's own IP (`72.61.245.204:0`), the latest at 2026-09-12T07:00:39Z logging `WS open niche=general` → `WS error: (1005, None)` → `NO provider call id on the start frame` ⇒ the double-billing risk is demonstrated only on self-tests; no Tata-routed call has ever reached us.
- **Changed:** nothing on prod, nothing committed/pushed. Four read-only recon probes added under `tmp/` (`prod_recon.sh`, `prod_dirty_probe.sh`, `prod_dnd_verify.sh`, `prod_cdr_recon.sh`); the DND weakening was NOT repaired and the dirty prod files were NOT touched or reverted (they are evidence).
- **Tests Run:** none this loop (no code change); local gates from the previous loop still green.
- **Verification Evidence:** prod `/health` = `33189b70`, checkout HEAD = `33189b70`, `leadgen_app` image `:33189b70` (built 2026-09-11 10:19Z); prod env from inside the container: `TELEPHONY_PROVIDER=tata_smartflo`, `TATA_SMARTFLO_ENABLED=1`, `SMARTFLO_VOICE_STREAM_ENABLED=1`, `VOICE_LAUNCH_KILL=0`, `VOICE_LAUNCH_CAMPAIGN=1`, `PLATFORM_DIAL_DAILY=1`, `PLATFORM_DIAL_LIMIT=100`, `DIAL_TEST_MODE=0`, `DLT_APPROVED=1`; `DND_FAIL_OPEN=1`, `APP_ENV=production`; md5 comparison as in (1); disk 55% / 87G free.
...
## Loop Run — 2026-09-16 (independent verification + owner-action closes)
...
- **Problems found:**
  1. `TELEGRAM_WEBHOOK_SECRET` NOT_SET → inbound webhook fail-open (P1)
  2. `.env APP_VERSION=c959709a` but checkout HEAD=`ab9b414e` → /health version drift (ADR-097 detector lie)
...
## Loop Run — 2026-09-16T08:39 IST (re-verification of the 4-item prod-execution report + nuance corrections)
...
- **Nuance #1 (MATERIAL CORRECTION to prior report):** prior line "celery workers stay `c959709a`" was IMPRECISE — no container is tagged `c959709a`. `c959709a` was only the stale `.env APP_VERSION` *string*. Actual prod image tags: `leadgen_worker`/`worker_heavy`/`worker_video`/`scheduler` = `1ef154c7ba9b`; 6x `worker_cli_*` = `d08f07c5`; `app_vobiz`+`mcp` = `8b7fd7c3`; new `app_staging` = `28ba5d4e`. **Conclusion unchanged**: none at `ab9b414e`, worker-roll still BLOCKED.
- **Nuance #2 (precision):** "`STAFF_JOBS_VALID` imported nowhere" — true for APP RUNTIME, NOT literally: `docs/openclaw/scripts/tmp_metrics.py:15` still imports it (would raise `ImportError` if run). It is a non-shipped docs/dev script, not the prod runtime path, so the "prod clean" intent holds.
- **Nuance #3 (now CLOSED):** `protect-main` ruleset re-verified this loop via authenticated GitHub API — `rulesets` = 1 entry, id=`23507307`, name=`protect-main`, target=`branch`, enforcement=`active` (created 2026-09-16T04:47 IST). **LIVE CONFIRMED**; the "not protected / rulesets -> []" 2026-09-14 note is definitively STALE.
...
- **Parallel-agent dirty files in working tree (NOT touched, NOT committed):** `app/telephony/telephony_readiness.py` (M — `WHATSAPP_AUTO_SEND`->`SALES_AUTOPILOT_WHATSAPP_ENABLED` flag rename) + `app/utils/telegram_egress.py` (untracked, new 13.5K egress helper). These belong to a concurrent agent stream; per R7 no `git add -A`.
