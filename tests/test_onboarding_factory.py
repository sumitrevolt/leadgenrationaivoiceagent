...
class TestPipelineState:
    ...
    def test_all_stages_completed(self):
        from app.marketing.onboarding_factory import PipelineState, STAGE_ORDER
        r, store = _mock_redis()
...
class TestBackpressure:
    ...
    def test_backpressure_triggers_on_active_count(self):
        from app.marketing.onboarding_factory import (
            check_backpressure, increment_active, _MAX_CONCURRENT,
        )
        r, store = _mock_redis()
    ...
    def test_increment_decrement_active(self):
        from app.marketing.onboarding_factory import increment_active, decrement_active, REDIS_BACKPRESSURE_KEY
        r, store = _mock_redis()
        increment_active(r)
        increment_active(r)
    ...
    def test_decrement_floor_at_zero(self):
        from app.marketing.onboarding_factory import decrement_active, REDIS_BACKPRESSURE_KEY
        r, store = _mock_redis()
...
class TestCapacityMetrics:
    def test_record_and_retrieve(self):
        from app.marketing.onboarding_factory import record_stage_metrics, get_capacity_metrics
        r, store = _mock_redis()
...
class TestStageExecutors:
    ...
    async def test_validate_force_skip(self):
        from app.marketing.onboarding_factory import stage_validate
        with patch("app.marketing.clients_store") as mock_cs:
            mock_cs.get_client.return_value = {"id": "c1", "setup_done": True, "business_name": "Test"}
    ...
    async def test_validate_eligible_client(self):
        from app.marketing.onboarding_factory import stage_validate
        with patch("app.marketing.clients_store") as mock_cs:
            mock_cs.get_client.return_value = {"id": "c1", "setup_done": False, "business_name": "Test"}
            result = await stage_validate("c1")
            assert result["ok"] is True
...
class TestPipelineOrchestrator:
    ...
    async def test_full_pipeline_mock(self):
        ...
        with patch.object(mod, "_redis", return_value=r), \
            ...
            result = await run_pipeline(
                "c_full", force=True, send_welcome=True
            )
    ...
    async def test_pipeline_stops_on_stage_failure(self):
        ...
        assert "content_pack" not in result["stages"] or result["stages"].get("content_pack", {}).get("status") != "done"
...
class TestPipelineIntegration:
    ...
    async def test_pipeline_state_with_metrics(self):
        ...
        from app.marketing.onboarding_factory import (
            PipelineState, record_stage_metrics, get_capacity_metrics, STAGE_ORDER
        )
        r, store = _mock_redis()
