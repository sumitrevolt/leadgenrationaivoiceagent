...
visible delivery history: the per-tenant content queue, the content-approvals
gate, and the delivery ledger.

Two properties the repo-wide debt ratchet cannot give:

  * the A4 writer modules carry ZERO uncontrolled in-checkout runtime paths
...
from tests.runtime_data_waves import A4_STORE_IDS
...
A4_RESOLVERS = {
    "app/marketing/auto_content.py": ("_QUEUE_DIR",),
    "app/marketing/content_approval.py": ("_FILE",),
    "app/marketing/delivery_ledger.py": ("_LEDGER_DIR", "_CONTENT_QUEUE_DIR"),
    "app/agents/staff.py": ("_JSONL_ROTATE_DIR",),
}
...
#: Paths that live in an A4 module but belong to a store A4 did NOT migrate.
...
# ------------------------------------------------------------------- code
@pytest.mark.parametrize("module_path", A4_MODULES)
def test_a4_writer_modules_have_zero_uncontrolled_runtime_paths(module_path):
...
@pytest.mark.parametrize("module_path", A4_MODULES)
def test_a4_modules_resolve_at_call_time_not_import_time(module_path):
    ...
    for resolver in A4_RESOLVERS[module_path]:
...
def test_the_a4_rows_are_still_dual_read():
    ...
    assert set(A4_STORE_IDS) <= moved, set(A4_STORE_IDS) - moved
...
def test_migrating_the_code_does_not_reduce_the_blocker_count():
    ...
    assert A4_STORE_IDS <= {s["store_id"] for s in blocking}
