...
class TestRecordStuckEventType:
    ...
    def _capture_event(self, reason, tmp_path):
        ...
        from app.marketing.customer_delivery import _record_stuck, _GATE_REASONS
...
def test_health_no_red_with_zero_recent_failures():
    ...
    client = {"id": "c1", "business_name": "Test", "plan": "starter",
              "socials": {"instagram": "ig"}, "whatsapp_phone": "999"}
    ...
    h = _customer_health(
        client=client, stage="content_ready", risk_flag="ok",
        setup_pct=100, setup=setup, failed_count=0,
        led_events=[{"event": "post_published", "at": datetime.now(timezone.utc).isoformat(), "customer_visible": True}],
        approvals_escalated=[], actions=[],
    )
...
def test_health_red_with_recent_failures():
    ...
    h = _customer_health(
        client=client, stage="content_ready", risk_flag="automation_failed",
        setup_pct=100, setup=setup, failed_count=3,
        led_events=[{"event": "post_published", "at": datetime.now(timezone.utc).isoformat(), "customer_visible": True}],
        approvals_escalated=[], actions=[],
    )
...
def test_ledger_recent_failures_reads_24h(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(delivery_ledger, "_LEDGER_DIR", ledger_dir)
    ...
    for i in range(2):
        ev = {"event": "automation_failed", "at": (now - timedelta(minutes=30 + i)).isoformat(),
              "detail": "send_failed", "actor": "system"}
        with open(path, "a") as f:
            f.write(json.dumps(ev) + "\n")

    # Write 5 old automation_failed events (outside 24h window — should NOT count)
    for i in range(5):
        ev = {"event": "automation_failed", "at": (now - timedelta(hours=48 + i)).isoformat(),
              "detail": "send_failed", "actor": "system"}
        with open(path, "a") as f:
            f.write(json.dumps(ev) + "\n")
...
def test_ledger_recent_failures_ignores_gated(tmp_path, monkeypatch):
    ...
    for i in range(10):
        ev = {"event": "delivery_gated", "at": (now - timedelta(minutes=10 + i)).isoformat(),
              "detail": "auto_delivery_off", "actor": "system"}
        with open(path, "a") as f:
            f.write(json.dumps(ev) + "\n")
