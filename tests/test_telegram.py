...
@pytest.fixture
def bot(tmp_path):
    config = tmp_path / "telegram_tenants.json"
    return MultiTenantTelegramBot(config_path=str(config))
...
class TestMultiTenantTelegramBot:
    ...
    def test_register_tenant(self, bot):
    ...
    def test_unregister_tenant(self, bot):
    ...
    def test_list_tenants(self, bot):
        """Test listing tenants."""
        tenants = bot.list_tenants()
        assert len(tenants) >= 1

    def test_activate_deactivate_tenant(self, bot):
    ...
    def test_webhook_processing(self, bot):
    ...
    def test_audit_logging(self, bot):
        """Test audit logging."""
        bot.register_tenant(
        ...
        assert len(logs) > 0
        assert any(log["action"] == "send_message" for log in logs)
