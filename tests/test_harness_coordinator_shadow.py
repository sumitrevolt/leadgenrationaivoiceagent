...
def _obs(**kw):
    base = dict(coordinator_run_id="coord_abc", orchestration_path="coordinate",
               action_index=0, agent_id="kavya", tenant_id="",
               normalized_action={"tool": "kavya", "task": "health"},
               actual_executor="run_ops", actual_result={"ok": True}, latency_ms=7)
    base.update(kw)
...
def test_raw_response_hash_bounded(monkeypatch):
    _env(monkeypatch)
    rec = _obs(raw_response_hash="abc123deadbeef")
    assert rec["raw_response_hash"] == "abc123deadbeef"     # hash only, no prose
...
def test_real_coordinate_shadow(monkeypatch, tmp_path):
    ...
    coord_rows = [r for r in rows if r.get("kind") == "shadow"
