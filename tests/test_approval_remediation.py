...
_STATES = {
    "a1": {"client_id": "jiya-makeover", "status": "approved", "created_at": _iso(72), "content": {"title": "Jiya ad"}},
    "a2": {"client_id": "105a5a749a81", "status": "pending", "created_at": _iso(72), "content": {"title": "Naya video ad"}},
    "a3": {"client_id": "e02c6e4f1f07", "status": "pending", "created_at": _iso(72), "content": {"title": "Naya video ad"}},
    "a4": {"client_id": "105a5a749a81", "status": "published", "created_at": _iso(72)},  # terminal -> ignored
    "a5": {"client_id": "dfbcbc8e5a08", "status": "pending", "created_at": _iso(2)},     # too young -> ignored
}
...
def _wire(monkeypatch):
    ...
    monkeypatch.setattr(
        clients_store, "list_clients",
        lambda status=None, product=None: [{"id": "jiya-makeover", "billing_client_ids": ["d79d690f61b3"]}],
    )
    monkeypatch.setattr(clients_store, "canonical_client_id", lambda cid: str(cid or "").strip())
    calls = []
    monkeypatch.setattr(content_approval, "cancel", lambda aid, actor="customer", note="": calls.append(aid) or {"ok": True})
...
def test_never_raises_on_store_error(monkeypatch):
    monkeypatch.setattr(content_approval, "_latest_states", lambda: (_ for _ in ()).throw(RuntimeError("store down")))
