...
def _obs(**kw):
    base = dict(dag_run_id="dr1", node_id="calc", attempt=0, agent_id="nikhil",
                tenant_id="", tool_name="internal_calculation", tool_version="v1",
                arguments={"n": 5}, actual_result={"ok": True, "count": 1},
                latency_ms=4, dag_node_status="completed")
    base.update(kw)
...
def test_tool_definition_strict(monkeypatch):
    with pytest.raises(Exception):
        ToolDefinition(name=CANON, version="1.0.0", description="x", risk_class=RiskLane.GREEN,
                       side_effect_class=SideEffectClass.NONE,
                       authority=AuthorityClass.INTERNAL_AUTONOMOUS, bogus_field=1)
...
def test_registry_rejects_unexpected_input_when_strict(monkeypatch):
    ...
    r = CanonicalToolRegistry()
    r.register(_def(name="workflow.dag.strict", input_schema={
        "type": "object", "properties": {"n": {"type": "integer"}},
        "required": ["n"], "additionalProperties": False}))
    e = r.evaluate_action(tool_name="workflow.dag.strict", tool_version="1.0.0",
                          arguments={"n": 1, "extra": "x"}, agent_id="nikhil",
...
def test_version_mismatch_visible(monkeypatch):
    ...
    e = r.evaluate_action(tool_name=CANON, tool_version="2.0.0", arguments={"n": 1},
...
def test_risk_downgrade_detected(monkeypatch):
    r = CanonicalToolRegistry()
    r.register(_def(name="workflow.dag.red", risk_class=RiskLane.RED,
    ...
    e = r.evaluate_action(tool_name="workflow.dag.red", tool_version="1.0.0",
                          arguments={"n": 1}, agent_id="nikhil", tenant_id="__system__",
                          idempotency_key=None, claimed_risk=claimed_lane(RiskClass.READ))
    assert e["risk_class_mismatch"] is True and e["would_deny"] is True


def test_amber_requirement_visible(monkeypatch):
    r = CanonicalToolRegistry()
    r.register(_def(name="workflow.dag.amber", risk_class=RiskLane.AMBER,
                    authority=AuthorityClass.APPROVAL_REQUIRED, requires_approval=True))
    e = r.evaluate_action(tool_name="workflow.dag.amber", tool_version="1.0.0",
                          arguments={"n": 1}, agent_id="nikhil", tenant_id="__system__",
                          idempotency_key=None, claimed_risk=None)
...
def test_owner_os_authority_preserved(monkeypatch):
    ...
    e = r.evaluate_action(tool_name="workflow.dag.owneros", tool_version="1.0.0",
                          arguments={"n": 1}, agent_id="nikhil", tenant_id="__system__",
                          idempotency_key=None, claimed_risk=None)
...
def test_red_denied_in_evaluation(monkeypatch):
    r = CanonicalToolRegistry()
    r.register(_def(name="workflow.dag.red2", risk_class=RiskLane.RED,
                    authority=AuthorityClass.ALWAYS_REFUSED))
    e = r.evaluate_action(tool_name="workflow.dag.red2", tool_version="1.0.0",
                          arguments={"n": 1}, agent_id="nikhil", tenant_id="__system__",
                          idempotency_key=None, claimed_risk=None)
...
def _seed_real_dag(monkeypatch, tmp_path, action="internal_calculation", inputs=None):
    ...
    _env(monkeypatch)
    runs_dir = tmp_path / "process_runs"; runs_dir.mkdir()
...
def _dag_shadow_rows(tmp_path):
    ...
    return [r for r in rows if r.get("kind") == "shadow"
            and (r.get("extra") or {}).get("source_loop") == "dag_engine"]
...
def test_repeated_observe_no_execution(monkeypatch):
    _env(monkeypatch)
    a = _obs(); b = _obs()          # record-only; neither executes the tool
