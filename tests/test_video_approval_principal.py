...
class _User:
    ...
    def __init__(self, uid="u-123", admin=True):
        self.id = uid
...
def test_principal_is_immutable():
    ...
    p = P.from_customer_session("acme")
...
def test_admin_without_platform_authority_has_no_capability():
    """Passing require_admin through an RBAC module grant is not approval
    authority over a tenant's content."""
    p = P.from_admin_user(_User(admin=False), tenant_id="acme")
    assert p.approval_capability is P.ApprovalCapability.NONE
    assert not p.can_approve
...
def test_saga_refuses_principal_without_capability(preview_client):
    p = P.from_admin_user(_User(admin=False), tenant_id="fixture-tenant-p")
    out = SAGA.approve(
        record_id="vid-preview-1",
        expected_revision=0,
        expected_sha256="a" * 64,
...
def test_wrong_tenant_fails_before_any_snapshot(preview_client, tmp_path):
    ...
    out = SAGA.approve(
        ...
        principal=P.from_customer_session("someone-else"),
    )
...
def test_identical_principal_replay_is_idempotent(preview_client):
    ...
    again = SAGA.approve(
        ...
        expected_revision=0,
        expected_sha256=digest,
        principal=P.from_customer_session("fixture-tenant-p"),
    )
