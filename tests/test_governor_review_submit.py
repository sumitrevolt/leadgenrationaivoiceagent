...
def test_submitter_refuses_non_loopback_url(monkeypatch):
    ...
    with pytest.raises(ValueError, match="loopback_url_required"):
        build_request(
            base_url="https://example.com/api", task_id="t1", governor="claude",
            decision="approve", artifact_hash="a" * 64, summary="safe",
        )
...
def test_submitter_sends_scoped_proof_without_printing_secret(monkeypatch):
    ...
    def fake_urlopen(request, timeout):
        captured.update(url=request.full_url, headers=dict(request.headers), body=request.data, timeout=timeout)
        return Response()

    monkeypatch.setattr("scripts.governor_review_submit.urlopen", fake_urlopen)
    result = submit_review(
        base_url="http://127.0.0.1:8000/api", task_id="t1", governor="claude",
        decision="approve", artifact_hash="a" * 64, summary="safe",
    )
