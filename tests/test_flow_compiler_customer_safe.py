...
def test_safe_action_compiles_under_customer_safe():
    proc, errs, kind = compile_flow(_flow([{"id": "a", "action": "seo_blog_draft"}], []), customer_safe=True)
    assert errs == [] and proc is not None


def test_http_request_rejected_under_customer_safe():
    proc, errs, kind = compile_flow(_flow([{"id": "a", "action": "http_request"}], []), customer_safe=True)
...
def test_breakpoint_and_merge_allowed_under_customer_safe():
    proc, errs, kind = compile_flow(_flow(
        ...
        [{"f": "a", "t": "g"}, {"f": "a", "t": "b"}, {"f": "g", "t": "m"}, {"f": "b", "t": "m"}]),
        customer_safe=True)
