...
def _obs(agent="dev", route="data_agent", impl="supervisor", **kw):
    base = dict(supervisor_run_id="s1", graph_run_id="g1", graph_step=1, tool_call_id=None,
                supervisor_implementation=impl, actor_id="manager", delegated_agent_id=agent,
                tenant_id="", tool_name=route, tool_arguments={"task": "t"},
                actual_executor=f"{route}_node", actual_result={"route": route},
                graph_metadata={"selection_source": "GRAPH_ROUTE", "route_label": route,
...
def _dec(**kw):
    base = dict(decision_id="d0", supervisor_implementation="supervisor", actor_id="manager",
                target_agent="dev", task="t", route_label="data_agent", graph_run_id="g1",
                graph_step=0, selection_source=SS.GRAPH_ROUTE)
    base.update(kw)
...
def test_extra_fields_fail():
    with pytest.raises(ValidationError):
        cc.SupervisorDecisionV1(decision_id="d", supervisor_implementation="supervisor",
                                actor_id="m", target_agent="dev", graph_run_id="g",
                                graph_step=0, bogus=1)
...
def test_staff_sup_structured_agent_maps(monkeypatch):
    _env(monkeypatch)
    rec = _obs(agent="dev", impl="staff_supervisor", route=None,
               graph_metadata={"selection_source": "MESSAGE_NAME", "actual_node": "staff_supervisor.graph"})
...
def test_staff_sup_message_name_tagged(monkeypatch):
    _env(monkeypatch)
    rec = _obs(agent="dev", impl="staff_supervisor",
...
def test_heuristic_source_blocks_registry(monkeypatch):
    _env(monkeypatch, agents="kavya")
    rec = _obs(agent="kavya", impl="staff_supervisor", route=None, tool_name="",
...
def _ev(tool=DEV, agent="dev", risk=RiskClass.READ, **kw):
    base = dict(tool_name=tool, tool_version="1.0.0", arguments={}, agent_id=agent,
                tenant_id="__system__", idempotency_key="k", claimed_risk=claimed_lane(risk))
    base.update(kw)
...
def test_conflicting_dev_rejected():
    ...
    with pytest.raises(RegistryConflict):
        r.register(ToolDefinition(name=DEV, version="1.0.0", description="DIFFERENT",
                                  risk_class=RiskLane.RED, side_effect_class=SideEffectClass.NONE,
...
def test_amber_approval_visible():
    assert _ev(tool=ROHAN, agent="rohan", risk=RiskClass.EXTERNAL_SEND)["would_require_approval"] is True


def test_owner_os_preserved():
    from app.agents.harness.registry import ToolDefinition
    r = CanonicalToolRegistry()
    r.register(ToolDefinition(name="agent.delegate.oos2", version="1.0.0", description="x",
               risk_class=RiskLane.GREEN, side_effect_class=SideEffectClass.NONE,
               authority=AuthorityClass.OWNER_OS_REQUIRED, allowed_agents=frozenset({"dev"})))
    e = r.evaluate_action(tool_name="agent.delegate.oos2", tool_version="1.0.0", arguments={},
...
def test_red_refusal_visible():
    from app.agents.harness.registry import ToolDefinition
    r = CanonicalToolRegistry()
    r.register(ToolDefinition(name="agent.delegate.red2", version="1.0.0", description="x",
               risk_class=RiskLane.RED, side_effect_class=SideEffectClass.NONE,
               authority=AuthorityClass.ALWAYS_REFUSED, allowed_agents=frozenset({"dev"})))
    e = r.evaluate_action(tool_name="agent.delegate.red2", tool_version="1.0.0", arguments={},
                          agent_id="dev", tenant_id="__system__", idempotency_key="k", claimed_risk=None)
    assert e["would_deny"] is True


def test_unregistered_route_honest():
    e = REGISTRY.evaluate_action(tool_name="supervisor.staff_supervisor", tool_version="v1",
...
def _rows(tmp_path, agent=None):
    ...
    r = [x["extra"] for x in rows if x.get("kind") == "shadow"
         and x["extra"].get("source_loop") == "supervisor"]
...
def test_supervisor_read_commands():
    from app.integrations.openclaw.harness_commands import (_sup_contract, _sup_samples, _sup_readiness)
...
def test_all_enforcement_off():
    ...
    assert st["AGENT_HARNESS_ENFORCE"] is False and st["bound_executors"] == [
        "batch.internal.safe_calculation@1.0.0"]   # no supervisor executor binding
