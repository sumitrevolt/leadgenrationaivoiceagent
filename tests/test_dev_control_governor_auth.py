...
def test_missing_or_short_secret_fails_closed(monkeypatch):
    ...
    missing = verify_governor_attestation(
        task_id=TASK_ID, governor="claude", decision="approve",
        artifact_hash=ARTIFACT_HASH, summary="safe", issued_at=str(NOW),
        nonce=NONCE, signature=_signature(), now=NOW,
    )
...
def test_governor_secrets_are_not_interchangeable(monkeypatch):
    ...
    result = verify_governor_attestation(
        task_id=TASK_ID, governor="chatgpt", decision="approve",
        artifact_hash=ARTIFACT_HASH, summary="safe", issued_at=str(NOW),
        nonce=NONCE, signature=_signature(secret=CLAUDE_SECRET, governor="chatgpt"), now=NOW,
    )
...
def test_stale_future_and_malformed_attestations_fail(monkeypatch):
    ...
    common = dict(
        task_id=TASK_ID, governor="claude", decision="approve",
        artifact_hash=ARTIFACT_HASH, summary="safe", nonce=NONCE,
        signature=_signature(), now=NOW,
    )
    assert verify_governor_attestation(issued_at=str(NOW - 301), **common)["reason"] == "timestamp_outside_window"
    assert verify_governor_attestation(issued_at=str(NOW + 31), **common)["reason"] == "timestamp_outside_window"
    assert verify_governor_attestation(issued_at="not-a-time", **common)["reason"] == "attestation_malformed"
    assert verify_governor_attestation(issued_at=str(NOW), **{**common, "nonce": "short"})["reason"] == "attestation_malformed"
...
def test_signed_body_tampering_is_rejected(monkeypatch):
    ...
    result = verify_governor_attestation(
        task_id=TASK_ID, governor="claude", decision="approve",
        artifact_hash=ARTIFACT_HASH, summary="changed after signing",
        issued_at=str(NOW), nonce=NONCE, signature=_signature(summary="safe"), now=NOW,
    )
    assert result["ok"] is False and result["reason"] == "signature_invalid"
...
def test_configured_header_builder_never_returns_secret(monkeypatch):
    ...
    headers = build_configured_governor_headers(
        task_id=TASK_ID, governor="claude", decision="approve",
        artifact_hash=ARTIFACT_HASH, summary="safe", issued_at=NOW,
        nonce=NONCE,
    )
    assert set(headers) == {
        "X-Governor-Timestamp", "X-Governor-Nonce", "X-Governor-Signature",
    }
    assert CLAUDE_SECRET not in repr(headers)
    assert verify_governor_attestation(
        task_id=TASK_ID, governor="claude", decision="approve",
        artifact_hash=ARTIFACT_HASH, summary="safe",
        issued_at=headers["X-Governor-Timestamp"],
        nonce=headers["X-Governor-Nonce"],
        signature=headers["X-Governor-Signature"], now=NOW,
    )["ok"] is True
