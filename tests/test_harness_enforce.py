...
from app.agents.harness.enforce import (DenialReason, EnforcementGate,
                                        ExecutorBindingRegistry, HarnessMode,
                                        enforce_batch_item, resolve_mode, _reset_guard)
from app.agents.harness.registry import (AuthorityClass, CanonicalToolRegistry,
                                         RiskLane, SideEffectClass, ToolDefinition)
...
def _def(name, **kw):
    base = dict(name=name, version="1.0.0", description="t",
...
def _ctx(agent="nikhil", tenant="__system__"):
    return RunContext(run_id="t", task_id="t", tenant_id=tenant, agent=agent,
                      source_loop="batch_harness")


def _req(name=SAFE, ver="1.0.0", args=None, idem="enforce:t:i:0"):
    return ToolCall(name=name, tool_version=ver, args=(args or {"id": "x"}),
                    risk_class=RiskClass.READ, idempotency_key=idem)


@pytest.fixture(autouse=True)
def _clean_env(monkeypatch):
    for k in ("AGENT_HARNESS", "AGENT_HARNESS_SHADOW", "AGENT_HARNESS_ENFORCE",
              "AGENT_HARNESS_ENFORCE_AGENTS", "AGENT_HARNESS_ENFORCE_LOOPS",
              "AGENT_HARNESS_ENFORCE_TOOLS", "AGENT_HARNESS_CANARY_AGENTS",
...
def test_mode_missing_tool_allowlist(monkeypatch):
    ...
    assert resolve_mode(agent_id="nikhil", source_loop="batch_harness",
                        tool_token=SAFE_TOKEN)[0] is HarnessMode.OFF
...
def test_decision_claimed_green_cannot_downgrade_red(monkeypatch):
    ...
    g = _gate_with(_def("x.y.z", risk_class=RiskLane.RED,
...
def test_decision_amber_pending_approval(monkeypatch):
    ...
    g = _gate_with(_def("c.d.write", risk_class=RiskLane.AMBER,
...
def test_decision_red_always_refused(monkeypatch):
    ...
    g = _gate_with(_def("shell.exec.run", risk_class=RiskLane.RED,
...
def test_decision_missing_executor_binding_denied(monkeypatch):
    _enforce_env(monkeypatch, tools="demo.calc.run@1.0.0")
    reg = CanonicalToolRegistry(); reg.register(_def("demo.calc.run"))
...
def test_exec_bound_executor_called_once(monkeypatch):
    _enforce_env(monkeypatch, tools="demo.calc.run@1.0.0")
    c = _Counter(); g = _gate_with(_def("demo.calc.run"), c)
...
def test_denied_action_executes_zero(monkeypatch):
    ...
    c = _Counter()
    g = _gate_with(_def("x.y.z", risk_class=RiskLane.RED,
                        authority=AuthorityClass.ALWAYS_REFUSED), c)
...
def test_batch_off_preserves_legacy(batch_env, monkeypatch):
    ...
    r = _run(run_batch(fn, [{"id": "a"}, {"id": "b"}], ckpt_id="off1", agent_id="nikhil",
                       tool_name=SAFE, tool_version="1.0.0"))
...
def test_batch_shadow_preserves_legacy_zero_harness_exec(batch_env, monkeypatch):
    ...
    async def fn(item):
        calls["n"] += 1
        return {"ok": True, "id": item["id"]}
    r = _run(run_batch(fn, [{"id": "a"}], ckpt_id="sh1", agent_id="nikhil",
                       tool_name=SAFE, tool_version="1.0.0"))
...
def test_batch_enforce_uses_registry_executor_only(batch_env, monkeypatch):
    _enforce_env(monkeypatch)
    r = _run(run_batch(_tripwire_fn, [{"id": "a"}, {"id": "b"}, {"id": "c"}],
                       concurrency=2, ckpt_id="enf1", agent_id="nikhil",
                       tool_name=SAFE, tool_version="1.0.0"))
...
def test_batch_enforce_registered_three_items(batch_env, monkeypatch):
    _enforce_env(monkeypatch)
    r = _run(run_batch(_tripwire_fn, [{"id": str(i)} for i in range(3)],
                       concurrency=3, ckpt_id="enf2", agent_id="nikhil",
                       tool_name=SAFE, tool_version="1.0.0"))
...
def test_batch_enforce_unregistered_executes_zero(batch_env, monkeypatch):
    ...
    r = _run(run_batch(_tripwire_fn, [{"id": "a"}, {"id": "b"}], concurrency=2,
                       ckpt_id="enf3", agent_id="nikhil"))
...
def test_batch_enforce_failure_aggregation(batch_env, monkeypatch):
    ...
    r = _run(run_batch(_tripwire_fn, [{"id": "a"}, {"id": "b"}], concurrency=2,
                       ckpt_id="enf4", agent_id="nikhil",
                       tool_name="demo.calc.run", tool_version="1.0.0", _enforce_gate=g))
    assert r["failed"] == 2 and r["done"] == 0 and r["ok"] is False


def test_batch_enforce_checkpoint_writes(batch_env, monkeypatch):
    _enforce_env(monkeypatch)
    _run(run_batch(_tripwire_fn, [{"id": str(i)} for i in range(4)], concurrency=2,
                   ckpt_id="enf5", agent_id="nikhil", tool_name=SAFE, tool_version="1.0.0"))
...
def test_batch_enforce_concurrency_honoured(batch_env, monkeypatch):
    ...
    r = _run(run_batch(_tripwire_fn, [{"id": str(i)} for i in range(6)], concurrency=2,
                       ckpt_id="enf6", agent_id="nikhil",
                       tool_name="demo.slow.run", tool_version="1.0.0", _enforce_gate=g))
...
def test_batch_enforce_kill_prevents_starts(batch_env, monkeypatch):
    ...
    r = _run(run_batch(_tripwire_fn, [{"id": str(i)} for i in range(4)], concurrency=2,
                       ckpt_id="enf7", agent_id="nikhil",
                       tool_name="demo.calc.run", tool_version="1.0.0", _enforce_gate=g))
...
def test_audit_allowed_decision_visible(batch_env, monkeypatch):
    _enforce_env(monkeypatch)
    _run(run_batch(_tripwire_fn, [{"id": "a"}], ckpt_id="au1", agent_id="nikhil",
                   tool_name=SAFE, tool_version="1.0.0"))
...
def test_audit_execution_result_visible(batch_env, monkeypatch):
    _enforce_env(monkeypatch)
    _run(run_batch(_tripwire_fn, [{"id": "a"}], ckpt_id="au3", agent_id="nikhil",
                   tool_name=SAFE, tool_version="1.0.0"))
...
def test_audit_duplicate_suppression_visible(batch_env, monkeypatch):
    ...
    ctx = RunContext(run_id="au4", task_id="au4", tenant_id="__system__",
                     agent="nikhil", source_loop="batch_harness")
    kw = dict(ctx=ctx, batch_run_id="au4", item_id="a", item_index=0, attempt=0,
              tool_name=SAFE, tool_version="1.0.0", item={"id": "a"}, gate=g)
...
def test_audit_shadow_and_enforce_distinguishable(batch_env, monkeypatch):
    _enforce_env(monkeypatch)
    _run(run_batch(_tripwire_fn, [{"id": "a"}], ckpt_id="au5", agent_id="nikhil",
                   tool_name=SAFE, tool_version="1.0.0"))
    rows = _audit.replay("au5")
    assert all((e.get("extra") or {}).get("mode") == "enforce"
               for e in rows if e.get("kind") == "enforce")
    assert all((e.get("extra") or {}).get("layer") == "enforcement"
               for e in rows if e.get("kind") == "enforce")
...
def test_audit_no_secrets_in_payload(batch_env, monkeypatch):
    ...
    _enforce_env(monkeypatch)
    _run(run_batch(_tripwire_fn, [{"id": "a"}], ckpt_id="au6", agent_id="nikhil",
                   tool_name=SAFE, tool_version="1.0.0"))
