"""
Comprehensive tests for Agnes adapter covering mission lifecycle, lease enforcement,
stale-result rejection, and independent review.
"""
import os
import sys
import pytest
...
# Use isolated temp dir for tests to avoid polluting production
TEST_DATA_DIR = tempfile.mkdtemp(prefix=f"agnes_test_{uuid.uuid4().hex[:8]}_")

# Monkey-patch the data directory for this test session
import app.platform.automation_orchestrator as orchestrator_mod
orchestrator_mod.DATA_DIR = TEST_DATA_DIR
...
from app.dev_control.external_agents.store import save as store_save, find_by_idempotency_key
from app.dev_control.external_agents.cas import get_backend
