...
def _kb() -> KnowledgeBase:
    return KnowledgeBase(prefer_chroma=False)  # force deterministic keyword backend (no qdrant/chroma)


def test_reseed_replaces_stale_same_source():
    kb = _kb()
    ns = "client:test"
    kb.add_documents(["hamari pricing 1999 rupaye per month"], source="website:x.com", namespace=ns, replace_source=True)
    kb.add_documents(["hamari pricing 2999 rupaye per month"], source="website:x.com", namespace=ns, replace_source=True)
...
def test_reseed_preserves_other_sources_in_namespace():
    kb = _kb()
    ns = "client:test2"
    kb.add_documents(["owner ka naam Meera hai"], source="kb_interview", namespace=ns)  # manual KB, no replace
    kb.add_documents(["site pricing 1999"], source="website:y.com", namespace=ns, replace_source=True)
    kb.add_documents(["site pricing 2999"], source="website:y.com", namespace=ns, replace_source=True)
...
def test_replace_source_noop_without_source():
    kb = _kb()
    ns = "client:test3"
    kb.add_documents(["doc one alpha"], namespace=ns, replace_source=True)  # no source -> cannot scope -> no delete
...
def test_default_add_still_appends():
    kb = _kb()
    ns = "client:test4"
    kb.add_documents(["fact one gamma"], source="website:z.com", namespace=ns)  # replace_source defaults False
