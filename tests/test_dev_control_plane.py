...
def test_happy_path_transitions_are_legal():
    ...
    for target in (
        TaskState.APPROVED, TaskState.QUEUED, TaskState.CLAIMED, TaskState.RUNNING,
        TaskState.REVIEW_REQUIRED, TaskState.TESTS_RUNNING, TaskState.STAGING_READY,
        TaskState.STAGING_DEPLOYED, TaskState.PRODUCTION_APPROVAL_REQUIRED,
        TaskState.PRODUCTION_DEPLOYED, TaskState.DELIVERY_VERIFICATION, TaskState.COMPLETED,
    ):
...
def test_admit_cost_denies_over_task_budget():
    out = admit_cost(
        provider="claude", estimated_input_tokens=1_000_000, estimated_output_tokens=100_000,
        task_budget_usd=Decimal("0.01"), daily_remaining_usd=Decimal("100"),
    )
...
def test_admit_cost_unknown_provider_denied():
    out = admit_cost(
        provider="nope", estimated_input_tokens=10, estimated_output_tokens=10,
        task_budget_usd=Decimal("1"), daily_remaining_usd=Decimal("1"),
    )
...
def test_gateway_success_with_fake_provider_records_cost():
    ...
    result = asyncio.run(
        invoke(
            task_id="t-1", task_type="code", sensitivity="normal", complexity="medium",
            system="s", messages=[{"role": "user", "content": "fix"}], max_tokens=200,
            task_budget_usd=Decimal("1"), daily_remaining_usd=Decimal("5"),
...
def test_gateway_falls_through_on_provider_error():
    ...
    result = asyncio.run(
        invoke(
            task_id="t-2", task_type="code", sensitivity="normal", complexity="medium",
            system="s", messages=[{"role": "user", "content": "fix"}], max_tokens=100,
            task_budget_usd=Decimal("1"), daily_remaining_usd=Decimal("5"),
...
def test_usage_rows_capture_attempts_and_success():
    rows = build_usage_rows(
        "t-3",
        {
            "ok": True, "provider": "local", "model": "local-coding",
...
def test_usage_rows_capture_budget_denial():
    rows = build_usage_rows(
        "t-4",
        {"ok": False, "reason": "task_budget_exceeded", "selected_provider": "claude",
         "attempted": [], "usage": {"estimated_cost_usd": "9.99"}},
    )
...
def test_health_snapshot_merges_catalog_and_breaker():
    def fake_breaker(alias):
        return {"state": "cooling" if alias == "deepseek" else "closed", "cooldown_remaining_s": 30, "trip_streak": 2}
    ...
    for row in rows:
        for field in ("provider_name", "model_name", "task_capabilities", "privacy_class",
                      "cost_class", "enabled", "circuit_breaker_state", "last_health_check"):
            assert field in row
