...
@pytest.mark.skip(reason="Vobiz removed 2026-09-15 — settings.vobiz_auth_id no longer exists; endpoint is dead code")
...
class TestResolveStreamProvider:
    ...
    def test_explicit_vobiz_now_resolves_to_smartflo(self, monkeypatch):



        """Vobiz REMOVED 2026-09-15 — explicit TELEPHONY_PROVIDER=vobiz now



        resolves to the SmartFlo rail (the sole stream provider)."""
    ...
    def test_unknown_explicit_provider_resolves_to_smartflo(self, monkeypatch):
    ...
    def test_resolve_is_now_constant_smartflo(self, monkeypatch):



        """resolve_stream_provider() is a constant after Vobiz removal — it



        ignores creds and the enable flag entirely."""
...
@pytest.mark.skip(reason="Vobiz removed 2026-09-15 — VobizClient.get_balance depends on removed settings")
...
@pytest.mark.skip(reason="Vobiz status webhook removed 2026-09-15 — VobizClient is dead code")
