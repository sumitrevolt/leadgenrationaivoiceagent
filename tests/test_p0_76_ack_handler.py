...
ACK_TEXT = "P0-76 ACK"
TEST_OWNER_CHAT_ID = 123456789


def _owner_update(
    update_id: int,
    text: str,
    chat_id: int = TEST_OWNER_CHAT_ID,
    user_id: int = 1,
...
def p076_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TELEGRAM_OWNER_CHAT_IDS", str(TEST_OWNER_CHAT_ID))
...
def test_command_linked_fingerprint_distinct(p076_env):
    ...
    a = _compute_p0_76_execution(
        task_id="task_79406871",
        update_id=11,
        fencing_token="fixture-a",  # nosecret: deterministic test-fixture fencing token (not a real secret)
    ...
    cfp = _compute_p0_76_execution(
        task_id="task_79406871",
        update_id=11,
        fencing_token="fixture-b",  # fresh claim token  # nosecret: deterministic test-fixture fencing token (not a real secret)
        owner_bot=P0_76_EXPECTED_OWNER_BOT,
    )
...
def test_owner_review_approval_releases_to_ready(tmp_path, monkeypatch):
    ...
    seed = TaskRecord(
        ...
        idempotency_key=P0_76_IDEMPOTENCY_KEY,
        input_payload={"admin_task_ref": 76},
        fencing_token="fixture-stale",  # nosecret: deterministic test-fixture fencing token (not a real secret)
