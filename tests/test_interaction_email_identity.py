...
def test_interested_replies_are_counted_separately():
    ...
    p = bf.plan(
        [_rec("i1", "a@b.com", outcome="interested"),
         _rec("i2", "c@d.com", outcome="sent")],
...
def test_backfill_does_not_touch_lead_status():
    ...
    assert "lead_status_history" not in low.split("SCOPE")[-1].split('"""')[0].lower() \
...
def test_record_resolves_identity_by_email():
    ...
    src = (bf.ROOT / "app" / "platform" / "interaction_log.py").read_text(
        encoding="utf-8")
