...
def _wire(monkeypatch, members=None, overdue=None, killed=None, *, armed=True):
    ...
    if armed:
        # Arm every gate the CANONICAL registry actually declares, derived from
        # the contracts rather than hand-listed here. `_wire` used to assume the
        # agents under test were "core (ungated)", which stopped being true when
        # `neha` gained `primary_flag="LEAD_SCORING"` — the tests then asserted
        # `enabled is True` while `resolve_agent_health` correctly reported
        # `disabled`, i.e. the PRODUCT was right and the fixture was stale.
        # Deriving the flags keeps the two in lock-step: a new primary_flag on a
        # contract is honoured automatically. Tests that specifically exercise the
        # gated-off path (test_disabled_when_flag_off) delenv the flag after
        # calling `_wire`, so their assertion still wins.
        from app.platform.agent_registry import all_contracts

        for contract in all_contracts().values():
            flag = getattr(contract, "primary_flag", "") or ""
            if flag:
...
def test_healthy_when_recent_activity(monkeypatch):
    ...
    h = asx.agent_health("neha")  # gated by LEAD_SCORING (armed by _wire)
...
def test_idle_when_event_driven_no_work(monkeypatch):
    ...
    h = asx.agent_health("ira")  # event-driven (useful_work_gap None), gated by JOURNEY_ENGINE
    assert h["enabled"] is True
