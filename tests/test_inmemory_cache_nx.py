...
@pytest.mark.asyncio
async def test_session_idem_claim_does_not_fail_closed_on_inmemory(monkeypatch):
    ...
    # monkeypatch the module-level _redis and current_session_id safely
    monkeypatch.setattr(vl, "_redis", fake_redis)

    async def fake_sid():
        return "sess-1"

    monkeypatch.setattr(vl, "current_session_id", fake_sid)
