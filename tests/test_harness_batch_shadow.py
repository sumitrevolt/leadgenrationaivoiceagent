...
from app.agents.harness.adapters import observe_batch_item, shadow_loop_eligible
from app.agents.harness.adapters import batch_shadow
...
def _obs(**kw):
    base = dict(batch_run_id="b1", batch_name="proof", item_id="item_0", item_index=0, attempt=0,
               agent_id="nikhil", tenant_id="", operation_name="calc", actual_executor="calc",
               actual_result={"ok": True}, latency_ms=4, checkpoint_state="completed", resumed=False)
    base.update(kw)
...
def test_shadow_off(monkeypatch):
    _env(monkeypatch, shadowf="0"); assert _obs() is None


def test_empty_agent(monkeypatch):
    _env(monkeypatch, agents=""); assert _obs() is None


def test_empty_loop(monkeypatch):
    _env(monkeypatch, loops=""); assert _obs() is None
...
def test_peer_ineligible(monkeypatch):
    _env(monkeypatch); assert _obs(agent_id="manager") is None


def test_wrong_loop(monkeypatch):
    _env(monkeypatch, loops="supervisor"); assert _obs() is None


def test_enforce_on(monkeypatch):
    _env(monkeypatch, enforce="1"); assert _obs() is None
...
def test_tenant_and_bounds(monkeypatch):
    ...
    blob = json.dumps(rec)
    assert "sk_live_Q" not in blob and "REDACTED" in blob and len(rec["legacy_result_summary"]) <= 620
...
def test_real_run_batch(monkeypatch, tmp_path):
    ...
    out = asyncio.run(bh.run_batch(safe_calc, items, concurrency=3, ckpt_id="t1", agent_id="nikhil"))
    assert out["done"] == 4 and out["failed"] == 1
    assert all(v == 1 for v in calls.values())               # each item executed exactly once

    rows = [json.loads(x) for x in open(tmp_path / "r.jsonl", encoding="utf-8")]
    actions = [r for r in rows if r.get("kind") == "shadow" and r["extra"].get("source_loop") == "batch_harness"]
    ...
    before = len(actions)
    calls.clear()
    out2 = asyncio.run(bh.run_batch(safe_calc, items, concurrency=3, ckpt_id="t1", agent_id="nikhil"))
    assert out2["skipped"] == 5 and calls == {}              # legacy re-ran nothing
    rows2 = [json.loads(x) for x in open(tmp_path / "r.jsonl", encoding="utf-8")]
    actions2 = [r for r in rows2 if r.get("kind") == "shadow" and r["extra"].get("source_loop") == "batch_harness"]
