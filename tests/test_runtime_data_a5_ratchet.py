...
Two properties the repo-wide debt ratchet cannot give:
  ...
  * the A5 writer modules carry ZERO uncontrolled in-checkout runtime paths
...
from tests.runtime_data_waves import A5_STORE_IDS
...
A5_MODULES = (
    ...
    "app/marketing/clients_store.py",
)

#: Resolver entry points each module must expose as functions.
A5_RESOLVERS = {
    "app/billing/gst_invoice.py": ("_STORE", "_lock_path"),
    "app/platform/upi_payments.py": ("_STORE",),
    "app/platform/upi_config.py": ("_STORE",),
    "app/marketing/clients_store.py": ("_CLIENTS_FILE",),
}
...
#: Paths that live in an A5 module but belong to a store A5 did NOT migrate.
...
# ------------------------------------------------------------------- code
@pytest.mark.parametrize("module_path", A5_MODULES)
def test_a5_writer_modules_have_zero_uncontrolled_runtime_paths(module_path):
...
@pytest.mark.parametrize("module_path", A5_MODULES)
def test_a5_modules_resolve_at_call_time_not_import_time(module_path):
    ...
    for resolver in A5_RESOLVERS[module_path]:
