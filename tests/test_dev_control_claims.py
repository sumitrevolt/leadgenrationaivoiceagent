...
def _task(state: str = TaskState.QUEUED.value, priority: int = 50, created_at: datetime | None = None, **kw) -> DevTask:
    now = datetime.utcnow()
    return DevTask(
        id=str(uuid.uuid4()), idempotency_key=f"idem-{uuid.uuid4()}",
        parent_objective="test objective", priority=priority, state=state,
        retry_count=0, created_at=created_at or now, updated_at=now, **kw,
    )
...
def test_heartbeat_refuses_non_owner():
    async def go():
        ...
        async with make() as db:
            ...
            assert await atomic_heartbeat(db, task.id, "worker-2") is False, "lease steal must be refused"
...
def test_reconcile_requeues_then_fails_expired_leases():
    async def go():
        ...
        async with make() as db:
            ...
            task = _task(state=TaskState.CLAIMED.value, lease_owner="dead-worker", lease_until=expired)
            task.retry_count = 0
            db.add(task)
            await db.commit()
