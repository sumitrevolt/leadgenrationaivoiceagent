...
container WA=0 vs disk .env=1 INERT (owner-approve restart needed)
/health 308 (auth-gate OK).
...
bots = load(bots_path)
bots["Pilot"]["status"] = (f"{ts_short} IST REV-COMMAND: loop DEAD 48h+ (batch 211, proc 0, cron 0)
SIP 4 vars len=0
"
                           f"api.vobiz.com 000 @8s DAY5
                           VOBIZ_CALLER_ID REVOKED len13
                           WA-sent 0
                           leads/ ABSENT
                           "
                           f"/health 308 auth-gate OK
                           WAHA with-key 200 WORKING
                           hot-queue 43/43. FLEET 0 ACK - REINFORCE {ts_short} sent. Gates 09:00-12:00 IST.")
...
bots["platform"]["status"] = ("PLT-004 P0: egress 000 @8.0s DAY5
SIP 4 vars len=0
VOBIZ_CALLER_ID REVOKED len13. Root-cause + Jio SIP template 09:00 IST. ACK missing.")
bots["operations"]["status"] = "OPS-006 P0: loop DEAD 48h+ re-confirm (proc 0, cron 0, mtime Aug31 08:39Z). Restart sign = env swap
...
bots["hunter"]["status"] = "HNT-004 P1: leads/ ABSENT re-confirm
hot-queue Sep2 ABSENT
50-lead DND CSV 09:30 IST. ACK missing."
bots["guardian"]["status"] = "GRD-003 P1: 6 verdicts 11:00 IST (revenue-truth + loop-dead + WAHA + auto_sent + leads + DID). ACK missing."
bots["success"]["status"] = "SUC-002 P0: Jiya only payer 1999 churn-risk
SMTP SENT proof 12:00 IST. ACK missing."
bots["board"]["status"] = "BRD-002 P2: VPS mirror present (00:08Z)
PILOT fresh push abhi. Page verify + cadence 12:00. ACK missing."
save(bots_path, bots)
...
pinned["vps_status"] = ("HEALTHY (containers up, /health 308 auth-gated 0.0007s)
calling loop DEAD 48h+ (mtime Aug31 08:39:55Z batch 211
proc 0
cron 0)
"
...
0 dialer connects (loop dead 48h+)
0 WA sends
Jiya P0"
