...
Fresh live evidence this run (SSH 02:16:10Z = 07:46 IST Sep5):
  ...
  - hot-queue 09-04 CSV+MD present (Sep4 03:30)
  09-05 due ~09:00 IST not-yet (expected).
  ...
  - wa_inbound: only status broadcasts (02:01-02:04Z)
  NO reply from 197126499872961 (SAL-006).
  - VERIFIED rev Rs1,999 (Jiya INV/0001 SOLE)
  GAP Rs4,98,001.
...
pin["action"] = ("07:46 REVENUE COMMAND broadcast
...
with open(os.path.join(BASE, "bots.json"), encoding="utf-8") as f:
    bots = json.load(f)
if "Pilot" in bots:
    bots["Pilot"]["status"] = ("07:46 IST Sep5 (LIVE 02:16Z): /health 200 prod 719dbbd6
    SAL-006 proposal+followup SENT reply PENDING
    "
