...
Live re-verify 08:01 IST THIS run (independent): state FROZEN day5+ vs 08:00 esc.
  ...
  - VERIFIED rev Rs1,999 (Jiya sole INV/2026-27/0001)
  GAP Rs4,98,001.
  - Fleet 0-ACK ~5d
...
with open(os.path.join(BASE, "pinned.json"), encoding="utf-8") as f:
    pin = json.load(f)
pin["last_updated"] = TS
pin["vps_status"] = "VPS UP leadgen_app healthy uptime12h
WA flip=1 PAR auto_sent WITH msg-id=0 (ENG-004 sendText never fired)
SIP EMPTY DID not landed dialer DEAD day5
leads 0
...
hot-queue 09-04 ABSENT
dialer 0 connects
genuine WA warm inbound
Jiya P0"
pin["action"] = "08:02 DECISIVE GHANTI ENG-004 (16:00 UPI-close gate). Stand gates: SUC-004 08:00 | SAL-005/OPS-007 08:30 | PLT-005/HNT-005 09:00 | GRD-004 09:30. Fleet 0-ACK ~5d
08:00 OWNER-ESC filed."
