...
Live re-verify 07:36 IST this run (not stale): state UNCHANGED vs 07:21 dispatch.
  ...
  - hot-queue last 09-03 03:30
  09-04 ABSENT 2nd day (date-lock).
  - VERIFIED rev Rs1,999 (Jiya sole INV/0001)
  GAP Rs4,98,001.
  - Fleet 0-ACK ~5d
  07:21 GHANTIs + 07:35 REV-COMMAND already in channel.
...
EV = "PILOT 07:36 IST Sep4 (LIVE 07:36, state frozen): leadgen_app Up12h healthy
WA flip=1 par msg-id=0/auto_sent=0 (ENG-004 sendText NEVER shipped)
SIP 5 vars EMPTY DID NOT landed (CLI 911171366938 REVOKED, dialer DEAD day5 mtime Aug31 batch211)
leads/=0 ammo
hot-queue 09-04 ABSENT 2nd day
rev Rs1,999 Jiya sole
...
with open(os.path.join(BASE, "pinned.json"), encoding="utf-8") as f:
    pin = json.load(f)
pin["last_updated"] = TS
pin["vps_status"] = "VPS UP leadgen_app Up12h healthy
WA flip=1 PAR auto_sent WITH msg-id=0 of 2298 (ENG-004 NOT shipped)
SIP 5 vars EMPTY DID not landed (CLI REVOKED) dialer DEAD day5
leads 0
hot-queue 09-04 ABSENT 2nd day
rev Rs1,999 (Jiya sole) GAP Rs4,98,001. Fleet 0-ACK ~5d."
