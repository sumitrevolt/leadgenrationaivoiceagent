...
class GoogleSheetsIntegration:
    ...
    async def create_spreadsheet(
        self, title: str, share_with: list[str] | None = None
    ) -> str | None:
        """
        ...
        try:
            spreadsheet = await asyncio.to_thread(self.client.create, title)
    ...
    async def add_leads_sheet(self, spreadsheet_id: str | None = None) -> bool:
        """
        ...
        try:
            spreadsheet = await asyncio.to_thread(self.client.open_by_key, spreadsheet_id)
            ...
            # Add headers
            await asyncio.to_thread(sheet.update, "A1", [self.LEAD_HEADERS])
    ...
    async def append_lead(
        self, lead_data: dict[str, Any], spreadsheet_id: str | None = None
    ) -> bool:
        """
    ...
    async def append_leads_batch(
        self, leads: list[dict[str, Any]], spreadsheet_id: str | None = None
    ) -> int:
        """
    ...
    async def update_lead_status(
        ...
        try:
            ...
            # Find the lead by ID
            cell = await asyncio.to_thread(sheet.find, lead_id)
    ...
    async def add_call_log_sheet(self, spreadsheet_id: str | None = None) -> bool:
        """Add Call Logs sheet"""
        spreadsheet_id = spreadsheet_id or self.default_spreadsheet
        ...
        try:
            ...
            await asyncio.to_thread(sheet.update, "A1", [self.CALL_LOG_HEADERS])
    ...
    async def log_call(self, call_data: dict[str, Any], spreadsheet_id: str | None = None) -> bool:
    ...
    async def get_leads_for_calling(
        self, limit: int = 100, spreadsheet_id: str | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            pending_leads = [
                record for record in records if record.get("Call Status", "").lower() == "pending"
            ]
