...
class ClientJourneyManager:
    ...
    async def record_call_outcome(self, lead_id: str, outcome: str, details: dict = None):
        """
...
Let's generate some leads! 🚀"""
        ...
        await whatsapp_handler.send_message(to=tracker.contact_phone, message=welcome_message)
    ...
    async def _schedule_nurturing(self, tracker: ClientJourneyTracker):
        ...
        await self._schedule_message(
            tracker, delay_days=3, stage=JourneyStage.TRIAL_DAY_3, message_type="trial_day_3"
        )

        # Day 6: Trial ending warning
        await self._schedule_message(
            tracker, delay_days=6, stage=JourneyStage.TRIAL_ENDING, message_type="trial_ending"
        )

        # Day 8: Trial ended - conversion push
        await self._schedule_message(
            tracker, delay_days=8, stage=JourneyStage.TRIAL_ENDED, message_type="trial_ended"
        )

    async def _schedule_message(
        self, tracker: ClientJourneyTracker, delay_days: int, stage: JourneyStage, message_type: str
    ):
    ...
    async def _schedule_callback(self, tracker: ClientJourneyTracker, callback_time: str | None):
        """Schedule a callback as requested by the lead"""

        if callback_time:
            logger.info(f"📞 Callback scheduled for {tracker.company_name} at {callback_time}")
        else:
    ...
    async def send_nurturing_message(self, lead_id: str, message_type: str):
        ...
        message = script["message"].format(
            ...
            or settings.support_whatsapp_number
            or "Contact Support",
        )

        # Send via WhatsApp
        await whatsapp_handler.send_message(to=tracker.contact_phone, message=message)
        tracker.total_whatsapp_messages += 1
...
Reply UPGRADE to continue, or call us to discuss."""
        ...
        await whatsapp_handler.send_message(to=tracker.contact_phone, message=conversion_message)

        logger.info(f"💰 Conversion attempt made for {tracker.company_name}")

        return True

    async def handle_conversion_response(self, lead_id: str, response: str, plan: str = None):
...
We're excited to continue generating leads for you!"""
            ...
            await whatsapp_handler.send_message(to=tracker.contact_phone, message=payment_message)
    ...
    async def _handle_objection(self, tracker: ClientJourneyTracker, objection: str):
        ...
        await whatsapp_handler.send_message(to=tracker.contact_phone, message=response)
