        ...
        for niche in target_niches:
            try:
                leads = await self.scraper.scrape_leads(
                    niche=niche, cities=target_cities, max_leads=5  # Small batch for safety
                )
                ...
                for lead in leads:
                    ...
                    # QUEUE THE CALL!
                    await self.call_manager.queue_call(
                        CallRequest(
                            lead_id=lead.id,
    ...
    async def _run_tenant_monitor(self):
        ...
        while self.is_running:
            try:
                ...
                self.stats.active_campaigns = len([t for t in tenants if t.is_running])
