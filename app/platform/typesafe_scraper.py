...
class ScraperDecision:
    ...
    latency_ms: float
    timestamp: str
    source_type: str  # "typesafe" or "heuristic"
...
class TypeSafeScraper:
    ...
    def evaluate_source_quality(self, source_config: dict[str, Any]) -> ScraperDecision:
        ...
        try:
            ...
            response = client.evaluate(model="jev-latest", state=state, questions=questions)
    ...
    def select_scraping_strategy(
        self, niche: str, target_count: int, budget: float
    ) -> ScraperDecision:
        """
        ...
        try:
            ...
            if not client.enabled:
                return self._heuristic_strategy(
                    niche, target_count, budget, decision_id, start_time
                )
            ...
            decision = ScraperDecision(
                ...
                result={
                    "primary_source": answers.get("primary_source", {}).get(
                        "choice", "google_maps"
                    ),
                    "secondary_source": answers.get("secondary_source", {}).get(
                        "choice", "indiamart"
                    ),
                    "scraping_frequency": answers.get("scraping_frequency", {}).get(
                        "choice", "daily"
                    ),
    ...
    def decide_deduplication(self, lead1: dict, lead2: dict) -> ScraperDecision:
        ...
        start_time = time.time()
        decision_id = (
            f"dedupe_{lead1.get('id', 'unknown')}_{lead2.get('id', 'unknown')}_{int(time.time())}"
        )
    ...
    def _heuristic_quality(
        self, source_config: dict, decision_id: str, start_time: float
    ) -> ScraperDecision:
    ...
    def _heuristic_strategy(
        self, niche: str, target_count: int, budget: float, decision_id: str, start_time: float
    ) -> ScraperDecision:
        ...
        frequency = (
            "real_time" if target_count > 500 else "daily" if target_count > 100 else "weekly"
        )
    ...
    def _heuristic_validate(
        self, lead_data: dict, decision_id: str, start_time: float
    ) -> ScraperDecision:
        ...
        if has_phone and has_email:
            ...
            quality, can_contact, needs = (
                "good",
                "yes_phone" if has_phone else "yes_email",
                "phone_only" if not has_phone else "email_only",
            )
        else:
        ...
        return ScraperDecision(
            ...
            source=lead_data.get("source", "unknown"),
            decision_type="validate",
            result={"data_quality": quality, "can_contact": can_contact, "needs_enrichment": needs},
            confidence=0.5,
            latency_ms=(time.time() - start_time) * 1000,
            timestamp=datetime.now(timezone.utc).isoformat(),
            source_type="heuristic",
    ...
    def _heuristic_dedup(
        self, lead1: dict, lead2: dict, decision_id: str, start_time: float
    ) -> ScraperDecision:
        ...
        return ScraperDecision(
            ...
            source="deduplication",
            decision_type="dedupe",
            result={"is_duplicate": is_dup, "confidence": confidence, "merge_action": action},
            confidence=0.5,
            latency_ms=(time.time() - start_time) * 1000,
            timestamp=datetime.now(timezone.utc).isoformat(),
            source_type="heuristic",
...
# Singleton instance
_scraper: TypeSafeScraper | None = None
