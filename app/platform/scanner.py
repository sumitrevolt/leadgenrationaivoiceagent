...
import re, json, hashlib, pathlib, datetime, logging
from typing import List, Dict, Tuple
from .assessment_models import Feature, FeatureCategory, FeatureStatus, Evidence
...
def _extract_api_endpoints(html: str) -> List[Dict]:
    ...
    for ep in endpoints:
        ctx_match = re.search(
            rf"""['"`]{re.escape(ep['path'])}['"`][^{{]*method\s*:\s*['"`](\w+)['"`]""",
            html, re.I
        )
        if ctx_match:
...
def scan_dashboard(filepath: pathlib.Path, dashboard_name: str) -> Dict:
    ...
    try:
        ...
        return {"dashboard_name": dashboard_name, "error": "file_not_found", "features": [], "api_endpoints_discovered": []}
    except PermissionError:
        log.error(f"Permission denied: {filepath}")
        return {"dashboard_name": dashboard_name, "error": "permission_denied", "features": [], "api_endpoints_discovered": []}
    ...
    raw_features.append({
        ...
        "status": FeatureStatus.COMPLETE.value if chart_count > 0 else FeatureStatus.BROKEN.value,
    ...
    raw_features.append({
        ...
        "status": FeatureStatus.COMPLETE.value if table_count > 0 else FeatureStatus.BROKEN.value,
    ...
    raw_features.append({
        ...
        "status": FeatureStatus.COMPLETE.value if form_count > 0 else FeatureStatus.BROKEN.value,
    ...
    raw_features.append({
        ...
        "description": "WebSocket/SSE/polling for live data" if has_realtime else "No real-time detected",
        "ui_elements": [],
        "api_endpoints": [],
        "evidence": _extract_evidence(REALTIME_PATTERNS[0], html).__dict__ if has_realtime else Evidence().__dict__,
    })
    ...
    raw_features.append({
        ...
        "description": "Spinner/skeleton/loading indicators" if has_loading else "No loading states detected",
    ...
    raw_features.append({
        ...
        "description": "ARIA labels and roles present" if has_aria else "No ARIA labels — WCAG violation",
        "ui_elements": [],
        "api_endpoints": [],
    ...
    # 16. Focus Indicators
    has_focus = _match([r':focus\b'], html) and not re.search(r'outline\s*:\s*0|outline\s*:\s*none', html)
    raw_features.append({
        "feature_name": "Keyboard Focus Indicators",
        "category": FeatureCategory.ACTION.value,
        "status": FeatureStatus.COMPLETE.value if has_focus else (FeatureStatus.PARTIAL.value if _match([r':focus\b'], html) else FeatureStatus.BROKEN.value),
        "description": ":focus styles present" if _match([r':focus\b'], html) else "No :focus styling — WCAG violation",
        "ui_elements": [],
        "api_endpoints": [],
