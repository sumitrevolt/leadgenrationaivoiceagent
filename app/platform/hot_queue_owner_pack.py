...
def _typesafe_score_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("[hq_pack_ts] TypeSafe scoring failed (fail-open, original sort): %s", exc)
        return rows
    ...
    try:
        with open(md_path, "w", encoding="utf-8") as f:
            ...
            for i, x in enumerate(rows[:15], 1):
                ...
                phone = wa.split("wa.me/")[1].split("?")[0] if "wa.me/" in wa else "?"
                ts_badge = (
                    f" 🎯 `ts={x.get('ts_score', '?')}`" if x.get("ts_score") is not None else ""
                )
                f.write(f"### {i}. {x.get('business_name', '?')}{ts_badge}\n")
                f.write(f"- **Phone:** `{phone}`\n")
                f.write(
                    f"- **Intent:** {x.get('intent', '?')} | **Niche:** {x.get('niche', '?')}\n"
                )
                f.write(f"- **City:** {x.get('city', '?')}\n")
                f.write(f"- **WA link:** <{wa}>\n")
                f.write(f"- **Draft preview:** {(x.get('draft') or '')[:140]}\n\n")
            f.write(
...
async def _push_ntfy(rows: list, today: str) -> str:
    ...
    try:
        ...
        for x in top3:
            ...
            lines.append(
                f"- {str(x.get('business_name', '?'))[:30]} ({x.get('city', '?')}) -> wa.me/{ph}"
            )
        lines += [
            "",
