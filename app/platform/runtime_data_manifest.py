...
STORES: list[dict[str, Any]] = [
    ...
    # 2026-09-26 task 117: current-main runtime-data debt classifications.
    _e(
        store_id="communications.telegram_poll_lease",
        display_name="Telegram polling-owner lease",
        ...
        business_category="communications",
        durability_class="rebuildable",
        concurrency_model="single-writer TTL lease; atomic tmp + os.replace",
        tenant_scope="owner/admin polling coordination (not customer data)",
        target_runtime_subpath="communications/telegram_poll_lease.json",
        migration_tier=TIER_3,
        migration_state=REBUILDABLE_CACHE,
        deployment_blocker=False,
        evidence=(
            "The lease prevents concurrent Telegram getUpdates pollers. It is "
            "regenerated by the next polling cycle if lost; the .tmp file is only "
            "an atomic staging companion."
        ),
    ),
    _e(
        store_id="platform.typesafe_keys",
        display_name="TypeSafe optional admin runtime credential file",
        legacy_paths=["data/typesafe_keys.json"],
        reader_modules=["app/platform/typesafe_integration.py"],
        production_activity="PRODUCTION_ACTIVE",
        current_authority="FILE",
        business_category="governance",
        durability_class="secret-config",
        concurrency_model="read-only in application runtime",
        tenant_scope="owner/admin credentials (not customer data)",
        migration_tier=TIER_NONE,
        migration_state=FALLBACK_ONLY,
        deployment_blocker=False,
        evidence=(
            "typesafe_integration.py reads this optional file only after environment "
            "key sources. The file can contain raw API keys, is never written by the "
            "application integration, and must never be copied, committed, logged, or "
            "included in runtime-data cutover."
        ),
    ),
    _e(
        store_id="automation.log_fallback",
        display_name="Automation event log JSONL fallback",
        legacy_paths=["data/automation_logs.jsonl"],
        writer_modules=["app/platform/automation_log_service.py"],
        reader_modules=["app/platform/automation_log_service.py"],
        database_equivalent="automation_logs",
        production_activity="PRODUCTION_ACTIVE",
        current_authority="DATABASE",
        business_category="automation",
        durability_class="operational-telemetry",
        ...
        deployment_blocker=False,
        evidence=(
            "AutomationLog database rows are the primary path. The JSONL file is "
            "used only when DB writes or reads fail; losing the fallback loses "
            "telemetry history, not business authority."
        ),
    ),
]
