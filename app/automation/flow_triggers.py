...
def _cron_due(trigger: dict, now: datetime) -> str | None:
    ...
    try:
        ...
        if (minute_ok and _match(h_f, now.hour, 0, 23) and _match(dom_f, now.day, 1, 31)
                and _match(mon_f, now.month, 1, 12) and _match(dow_f, cron_dow, 0, 6)):
    ...
    try:
        ...
        for flow in flow_store.list_flows_full():
            ...
            if fired >= _MAX_STARTS_PER_TICK:
                break
            inputs = {**payload, "_trigger": "event", "_event": event_type,
                      "_flow_origin": fid, "_client_id": str(client_id or "")}
            if _fire(fid, inputs):
