...
class CampaignManager:
            ...
            if campaign.sync_to_sheets:
                ...
                if not campaign.spreadsheet_id:
                    ...
                    campaign.spreadsheet_id = await self.sheets.create_spreadsheet(
                        title=f"{campaign.name} - Leads", share_with=campaign.notify_email
                    )
    ...
    async def _queue_campaign_calls(self, campaign: Campaign, leads: list[UnifiedLead]):
        ...
        for i, lead in enumerate(leads):
            ...
            scheduled_time = self._calculate_call_time(campaign, i, campaign.daily_call_limit)
    ...
    def _calculate_call_time(
        self, campaign: Campaign, call_index: int, daily_limit: int
    ) -> datetime:
        ...
        call_time = campaign.start_date + timedelta(
            days=day_offset, hours=start_hour + hour_offset, minutes=minute_offset
        )
    ...
    async def _monitor_campaign(self, campaign: Campaign):
        ...
        while campaign.status == CampaignStatus.RUNNING:
            try:
                # Update stats
                self.call_manager.get_stats()

                # Get completed calls for this campaign
                completed = [
                    c
                    for c in self.call_manager.completed_calls
                ...
                campaign.appointments_booked = sum(
                    1 for c in completed if c.outcome == "appointment"
                )
    ...
    async def _handle_hot_lead(self, campaign: Campaign, call: CallResult):
        ...
        if campaign.sync_to_hubspot:
            try:
                ...
                if contact_id:
                    await self.hubspot.log_call_activity(
                        contact_id,
                        {
                            "phone": call.phone_number,
    ...
    async def _send_campaign_summary(self, campaign: Campaign):
        ...
        summary = {
            ...
            "connection_rate": (
                campaign.leads_called / campaign.leads_scraped if campaign.leads_scraped > 0 else 0
            ),
            "qualification_rate": (
                campaign.leads_qualified / campaign.leads_called if campaign.leads_called > 0 else 0
            ),
        }
