...
class CallScheduler:
    ...
    def is_working_time(self, dt: datetime | None = None) -> bool:
    ...
    def get_next_working_time(self, from_time: datetime | None = None) -> datetime:
    ...
    def get_optimal_call_times(
        self, niche: str, count: int, start_from: datetime | None = None
    ) -> list[datetime]:
        """
        ...
        optimal_config = self.OPTIMAL_CALL_TIMES.get(
            niche,
            {
                "best_hours": [10, 11, 14, 15, 16],
                "best_days": [DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY],
    ...
    def calculate_call_distribution(
        self, total_calls: int, days: int, niche: str
    ) -> dict[str, list[int]]:
        """
    ...
    async def schedule_task(
        self, task_id: str, scheduled_time: datetime, callback: Callable, *args, **kwargs
    ):
...
class AutoFollowUp:
    ...
    async def schedule_followup(
        ...
        self.pending_followups[lead_id] = {
            ...
            "message": template["message"].format(client_name=client_name, service=service),
        }
