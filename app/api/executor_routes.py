...
def _require_tool_identity(
    ...
    """FastAPI dependency: validate HMAC + return authenticated tool identity.

    Reads the raw body once, then verifies the signature matches the body's SHA.
    Replays / unknown tools / missing secrets are rejected with HTTP errors.
    """
    _check_hub_enabled()
    _check_known_tool(x_tool_id)
    secret = _check_secret_configured(x_tool_id)
    ...
    ident = ToolIdentity(
        ...
        body_sha256=x_body_sha256,
        event_type=x_event_type,
    )
    _check_window(ident)
    _replay_protection(ident.nonce)

    # The HMAC signature is over the X-Body-Sha256 declared by the client;
    # the server does NOT re-hash the request body. The client is trusted to
    # declare the correct sha256 of the exact bytes it sent. This avoids
    # needing to consume the async stream twice (TestClient + real curl both
    # deliver the body once and FastAPI caches it).
    _verify_hmac(ident, secret, b"")
...
async def next_task(
    ...
    candidate_roles = [tool_id]
    extras = os.getenv(f"EXECUTOR_{tool_id.upper()}_ALLOWED_AGENTS", "")
    if extras:
        candidate_roles.extend(r.strip() for r in extras.split(",") if r.strip())
    ...
    for role in candidate_roles:
        ...
        try:
            from app.platform.automation_orchestrator import TaskStatus
...
async def heartbeat(
    ...
    if task.assigned_agent != tool_id:
        if tool_id not in os.getenv(f"EXECUTOR_{tool_id.upper()}_ALLOWED_AGENTS", "").split(","):
...
async def claim(
    ...
    lm = getattr(orch, "lease_manager", None)
    lease_acquired = False
    if lm is not None:
        try:
            lease_acquired = lm.acquire(task_id, task.fencing_token or "")
        except Exception:
            lease_acquired = False
