    ...
    try:
        with get_db_session() as db:
            ...
            unsynced_leads = (
                ...
                .filter(
                    Lead.status.in_(
                        [LeadStatus.QUALIFIED, LeadStatus.APPOINTMENT, LeadStatus.INTERESTED]
                    ),
...
def sync_to_hubspot(lead_data: dict):
    ...
    try:
        ...
        try:
            contact_id = loop.run_until_complete(hubspot.create_contact(lead_data))
        finally:
            loop.close()

        return {"status": "completed", "hubspot_id": contact_id}
...
def sync_to_sheets(lead_data: dict, spreadsheet_id: str):
    ...
    try:
        ...
        try:
            loop.run_until_complete(sheets.append_lead(lead_data, spreadsheet_id))
        finally:
            loop.close()
...
def batch_sync_leads(leads: list, target: str, config: dict):
    ...
    return {"status": "completed", "queued": success_count, "failed": failed_count}
                ...
                try:
                    ...
                    contacts = loop.run_until_complete(
                        hubspot.get_contacts(
                            limit=config.get("limit", 100), after=config.get("after")
                        )
                    )
        ...
        with get_db_session() as db:
            ...
            for call in recent_calls:
                ...
                outcome_data = {
                    ...
                    "appointment_date": (
                        call.appointment_date.isoformat() if call.appointment_date else None
                    ),
...
def update_hubspot_contact(phone: str, data: dict):
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(hubspot.update_contact_by_phone(phone, data))
        finally:
            loop.close()
