...
_TRANSITIONS: dict[TaskState, set[TaskState]] = {
    ...
    TaskState.RUNNING: {TaskState.REVIEW_REQUIRED, TaskState.TESTS_RUNNING, TaskState.BLOCKED, TaskState.FAILED},
    TaskState.BLOCKED: {TaskState.QUEUED, TaskState.CANCELLED, TaskState.FAILED},
    TaskState.REVIEW_REQUIRED: {TaskState.CHANGES_REQUESTED, TaskState.TESTS_RUNNING, TaskState.FAILED},
    ...
    TaskState.STAGING_READY: {TaskState.STAGING_DEPLOYED, TaskState.PRODUCTION_APPROVAL_REQUIRED, TaskState.FAILED},
...
def admit_cost(*, provider: str, estimated_input_tokens: int, estimated_output_tokens: int, task_budget_usd: Decimal, daily_remaining_usd: Decimal) -> dict[str, Any]:
    ...
    cost = (Decimal(estimated_input_tokens) * Decimal(str(meta["cost_input_usd_per_million"])) + Decimal(estimated_output_tokens) * Decimal(str(meta["cost_output_usd_per_million"]))) / Decimal(1_000_000)
