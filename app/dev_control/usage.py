...
def build_usage_rows(task_id: str, result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for att in result.get("attempted", []) or []:
        ...
        rows.append({
            "attempt_no": n, "provider": str(att.get("provider") or "")[:60], "model": None,
            "outcome": outcome, "input_tokens": None, "output_tokens": None,
            "cost_usd": None, "estimated": True, "detail": detail,
        })
    ...
    if result.get("ok"):
        ...
        rows.append({
            "attempt_no": n, "provider": str(result.get("provider") or "")[:60],
            "model": result.get("model"), "outcome": "success",
            "input_tokens": int(usage.get("prompt_tokens") or 0),
            "output_tokens": int(usage.get("completion_tokens") or 0),
            "cost_usd": Decimal(str(usage.get("actual_cost_usd") or "0")),
            "estimated": bool(usage.get("estimated", False)), "detail": None,
        })
    elif result.get("reason") in _BUDGET_REASONS:
        n += 1
        usage = result.get("usage") or {}
        rows.append({
            "attempt_no": n, "provider": str(result.get("selected_provider") or "")[:60],
            "model": None, "outcome": "budget_denied",
            "input_tokens": None, "output_tokens": None,
            "cost_usd": Decimal(str(usage.get("estimated_cost_usd") or "0")),
            "estimated": True, "detail": str(result.get("reason"))[:500],
        })
    return rows


async def persist_usage(db, task_id: str, rows: list[dict[str, Any]], scope: str | None = None) -> dict[str, Any]:
    ...
    for r in rows:
        db.add(DevTaskUsage(
            id=str(uuid.uuid4()), task_id=task_id, scope=(scope or None),
            created_at=now, attempt_no=r["attempt_no"], provider=r["provider"],
            model=r.get("model"), outcome=r["outcome"],
            input_tokens=r.get("input_tokens"), output_tokens=r.get("output_tokens"),
            cost_usd=r.get("cost_usd"), estimated=bool(r.get("estimated", True)),
            detail=r.get("detail"),
        ))
    ...
    if task is not None:
        ...
        task.updated_at = now
    await db.commit()
    return {"rows_written": len(rows), "success_cost_usd": str(total_cost), "input_tokens": in_tok, "output_tokens": out_tok}


async def record_gateway_result(db, task_id: str, result: dict[str, Any], scope: str | None = None) -> dict[str, Any]:
