...
from app.llm.vertex_client import VertexAIClient, get_vertex_client
...
{{
  ...
  "sentiment": "positive, neutral, negative"
    ...
    async def start_conversation(
        ...
        # Add to history
        context.history.append({"role": "assistant", "content": opening})
...
Generate ONLY the spoken text, no instructions or formatting."""
    ...
    async def process_response(
        ...
        if not context:
            ...
            return (
                "I'm sorry, there seems to be a connection issue. Could you please repeat that?",
        ...
        if intent in [
            ConversationIntent.DND,
            ConversationIntent.WRONG_NUMBER,
            ConversationIntent.END_CALL,
        ]:
        ...
        logger.debug(
            f"Call {call_id}: Intent={intent.value}, Temp={context.lead_temperature.value}"
        )
    ...
    async def _generate_response(self, user_text: str, context: ConversationContext) -> str:
...
Maya:"""
        ...
        response, _ = await self.client.generate(
            prompt, system_instruction=system_prompt, temperature=0.8, max_tokens=150
        )
    ...
    async def end_conversation(self, call_id: str, outcome: str | None = None) -> dict[str, Any]:
        """
        ...
        summary = {
            ...
            "follow_up_required": context.lead_temperature
            in [LeadTemperature.WARM, LeadTemperature.HOT],
        ...
        logger.info(
            f"Ended call {call_id}: outcome={summary['call_outcome']}, temp={summary['lead_temperature']}"
        )
        return summary
...
# Singleton instance
_brain_instance: VoiceAgentBrain | None = None
...
async def start_call(
    call_id: str, lead_name: str, lead_phone: str, **kwargs
) -> tuple[str, ConversationContext]:
    """Start a new call"""
