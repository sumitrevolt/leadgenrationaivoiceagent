...
class ExotelHandler:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
                    url, data=data, headers={"Authorization": f"Basic {self.auth}"}, timeout=30.0
                )
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(
                    url, headers={"Authorization": f"Basic {self.auth}"}, timeout=15.0
                )
                response.raise_for_status()
    ...
    async def send_sms(self, to_number: str, message: str, sender_id: str | None = None) -> bool:
        """
        Send SMS via Exotel
        ...
        data = {"From": sender_id or self.caller_id, "To": to_number, "Body": message}

        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
                    url, data=data, headers={"Authorization": f"Basic {self.auth}"}, timeout=15.0
                )
                response.raise_for_status()
...
class ExotelCallFlow:
    ...
    def generate_greeting_flow(
        self, greeting_text: str, gather_url: str, language: str = "hi-IN"
    ) -> str:
        """
...
<Response>
    ...
    def generate_play_and_gather(self, audio_url: str, gather_url: str) -> str:
