...
class VoiceAgentBrain:
    ...
    async def generate_greeting(
        ...
        similar = await self._get_similar_patterns(
            query="successful greeting opening", industry=state.industry, intent="greeting"
        )
    ...
    async def process_customer_speech(
        ...
        similar = await self._get_similar_patterns(
            query=customer_text, industry=state.industry, intent=intent.value
        )
...
Generate ONLY the agent's response (no quotes, no labels):"""
    ...
    async def _get_similar_patterns(
        ...
        try:
            results = await self.vector_store.search(
                query=query,
                limit=limit,
                filter_metadata={"industry": industry, "intent": intent, "outcome": "success"},
            )
            return results
        except Exception as e:
            ...
            for i, turn in enumerate(state.history):
                if turn["role"] == "agent":
                    await self.vector_store.add_conversation(
                        ...
                        intent=(
                            state.intents_detected[i // 2].value
                            if i // 2 < len(state.intents_detected)
                            else "unknown"
                        ),
                    )
    ...
    def _build_greeting_prompt(
        ...
        similar_greetings = (
            "\n".join([f"- {s.get('response', '')}" for s in similar[:3]]) if similar else ""
        )
...
Generate ONLY the greeting (no quotes, no labels):"""
    ...
    def _calculate_temperature(self, state: ConversationState) -> LeadTemperature:
        ...
        positive = sum(
            1
            for i in state.intents_detected
            if i in [CallIntent.INTERESTED, CallIntent.APPOINTMENT, CallIntent.QUESTION]
        )
        negative = sum(
            1
            for i in state.intents_detected
            if i in [CallIntent.NOT_INTERESTED, CallIntent.DND, CallIntent.BUSY]
        )

        if (
            CallIntent.DND in state.intents_detected
            or CallIntent.WRONG_NUMBER in state.intents_detected
        ):
    ...
    async def _handle_appointment(
        ...
        return ResponseGeneration(
            ...
            suggested_next_action=(
                "confirm_appointment" if state.appointment_confirmed else "collect_time"
            ),
            emotion="excited",
        )
    ...
    async def train_on_successful_calls(self, min_calls: int = 10) -> dict[str, Any]:
        ...
        try:
            ...
            if self.vector_store:
                ...
                logger.info(
                    f"✅ Voice Agent Brain training complete: {stats['patterns_extracted']} patterns extracted"
                )
