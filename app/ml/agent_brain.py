...
class AgentBrain:
    ...
    async def get_project_context(
        self, query: str, agent_role: AgentRole, limit: int = 5
    ) -> list[dict]:
        """
        ...
        try:
            ...
            results = await self.vector_store.search(
                query=enhanced_query, limit=limit, filter_metadata={"domain": agent_role.value}
            )
    ...
    async def generate_suggestion(
        self, context: AgentContext, use_vertex: bool = True
    ) -> AgentSuggestion:
        """
        ...
        if use_vertex:
            try:
                ...
                response, _ = await client.generate(
                    prompt=full_prompt, max_tokens=1000, temperature=0.7
                )

                # Parse the response into a suggestion
                suggestion = self._parse_llm_response(
                    response=response, agent_role=context.agent_role, context=context
                )
...
"""
    ...
    def _parse_llm_response(
        self, response: str, agent_role: AgentRole, context: AgentContext
    ) -> AgentSuggestion:
    ...
    def _generate_template_suggestion(self, context: AgentContext, config: dict) -> AgentSuggestion:
    ...
    def record_suggestion_outcome(self, suggestion_id: str, accepted: bool):
        """
    ...
    def get_agent_report(self) -> dict:
        ...
        return {
            ...
            "overall_acceptance_rate": (
                sum(1 for s in self.suggestion_history if s.accepted) / len(self.suggestion_history)
                if self.suggestion_history
                else 0
            ),
            "agent_metrics": {role.value: metrics for role, metrics in self.agent_metrics.items()},
            "top_agents": [
