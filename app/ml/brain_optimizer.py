...
class BrainOptimizer:
    ...
    async def get_optimized_system_prompt(
        self, agent_type: str, industry: str, language: str = "hinglish", base_prompt: str = None
    ) -> str:
        """
        ...
        candidates.sort(key=lambda p: p.success_rate * min(1.0, p.times_used / 20), reverse=True)

        best = candidates[0]

        # Inject industry-specific context
        enhanced_prompt = self._enhance_prompt_for_industry(best.content, industry, language)
...
Be efficient but friendly.""",
    ...
    def _enhance_prompt_for_industry(self, prompt: str, industry: str, language: str) -> str:
...
- Decision factors: Courses offered, student demographics""",
    ...
    async def get_rag_context(
        self, context: ConversationContext, current_query: str, top_k: int = 3
    ) -> list[dict]:
        """
    ...
    async def get_optimized_response(
        ...
        metadata = {"source": "base", "variant": None, "rag_used": False}
        ...
        ab_response, variant = self._get_ab_variant(context.call_id, response_type)
        ...
        if base_response:
            rag_context = await self.get_rag_context(
                context=context, current_query=context.current_intent, top_k=2
            )
    ...
    def _get_ab_variant(self, call_id: str, response_type: str) -> tuple[str | None, str | None]:
        ...
        variants = [
            p for p in self.prompts.get(response_type, []) if p.variant_id and not p.is_control
        ]
    ...
    def _enhance_with_rag(self, base_response: str, rag_context: list[dict]) -> str:
    ...
    async def get_objection_response(
        self, objection_type: str, context: ConversationContext, base_response: str = None
    ) -> str:
        """
        ...
        best = self.feedback_loop.get_best_objection_response(
            objection_type=objection_type, fallback=base_response
        )

        if best:
            # Personalize for industry
            personalized = self._personalize_response(
                best, context.lead_industry, context.lead_company
            )
            return personalized

        return base_response or "Main samajh sakta hoon. Kya aur koi concern hai?"

    def _personalize_response(self, response: str, industry: str, company: str) -> str:
        ...
        if company:
            personalized = personalized.replace("aapki company", f"{company}")
    ...
    async def record_response_outcome(
        ...
        await self.feedback_loop.record_outcome(
            ...
            language=context.language,
            responses_used=[
                {"type": response_type, "content": response_used, "intent": context.current_intent}
            ],
            outcome=outcome,
        )
    ...
    def add_prompt_variant(
        self, prompt_type: str, content: str, industry: str = "general", is_control: bool = False
    ):
                    ...
                    "variants": [
                        {
                            ...
                            "lift": (
                                ...
                                / control[0].success_rate
                                * 100
                                if control[0].success_rate > 0
                                else 0
                            ),
                        }
                        for v in variants
    ...
    def get_optimization_stats(self) -> dict:
        ...
        return {
            ...
            "prompt_types": {ptype: len(prompts) for ptype, prompts in self.prompts.items()},
            "ab_tests_active": len(
                [
                    p
