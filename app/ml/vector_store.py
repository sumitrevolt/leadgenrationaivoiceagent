...
class VectorStore:
    ...
    def __init__(
        self, persist_directory: str = "data/vectorstore", collection_name: str = "conversations"
    ):
    ...
    async def add_batch(self, conversations: list[dict]):
        """
    ...
    async def search_similar(
        ...
        outcome_filter: str | None = None,  # "successful", "failed", or specific outcome
        ...
        if outcome_filter:
            if outcome_filter == "successful":
                ...
                where_filter["outcome"] = {"$in": ["not_interested", "dnd", "wrong_number"]}
            else:
    ...
    async def find_best_response(
        self, user_message: str, industry: str, top_k: int = 3
    ) -> str | None:
        """
        ...
        results = await self.search_similar(
            query=user_message, industry=industry, outcome_filter="successful", top_k=top_k
        )
    ...
    async def find_objection_responses(
        self, objection: str, industry: str = None, top_k: int = 5
    ) -> list[dict]:
        """
        ...
        results = await self.search_similar(
            query=objection, industry=industry, outcome_filter="successful", top_k=top_k
        )
