...
class ConversationRecord:
    ...
    human_rating: int | None = None  # 1-5 if reviewed
...
class ConversationDataPipeline:
    ...
    def _extract_features(self, record: ConversationRecord) -> ConversationFeatures:
        ...
        if user_contents:
            features.avg_user_response_length = sum(len(c) for c in user_contents) / len(
                user_contents
            )
        if agent_contents:
            features.avg_agent_response_length = sum(len(c) for c in agent_contents) / len(
                agent_contents
            )
