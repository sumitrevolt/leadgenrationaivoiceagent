...
class VertexContinuousTrainer:
    ...
    def __init__(self, config: VertexTrainingConfig | None = None):
    ...
    async def _check_and_train(self, brain_type: str):
        ...
        if should_train:
            logger.info(
                f"🎯 Training triggered for {brain_type}: {reason} (priority: {priority.name})"
            )
    ...
    async def _should_train(self, brain_type: str) -> tuple[bool, TrainingPriority, str]:
        ...
        recent = [
            b
            for b in behaviors
            if b.get("timestamp", "") > (datetime.now() - timedelta(hours=1)).isoformat()
        ]
        if len(recent) >= 10:
        ...
        if len(with_feedback) >= 20:
            rejection_rate = len([b for b in with_feedback if not b.get("user_accepted")]) / len(
                with_feedback
            )
            if rejection_rate > self.config.rejection_rate_threshold:
                return (
                    ...
                    f"Rejection rate {rejection_rate:.1%} > threshold",
                )
    ...
    async def train_brain_with_vertex(
        ...
        async with self._training_lock:
            ...
            logger.info(
                f"🧠 Starting Vertex AI training for {brain_type} (priority: {priority.name})"
            )
            ...
            try:
                ...
                metrics.improvement_percent = (
                    (metrics.after_accuracy - metrics.before_accuracy)
                    / max(metrics.before_accuracy, 0.01)
                    * 100
                )
...
{{
    ...
    "user_preferences": [
        ...
        try:
            ...
            return json.loads(response).get("success_patterns", []) + json.loads(response).get(
                "failure_patterns", []
            )
        except Exception as e:
...
["skill1", "skill2", "skill3"]"""
    ...
    async def _apply_improvements(
        ...
        logger.info(
            f"📝 Applying {len(improvements.get('improvements', []))} improvements to {brain_type}"
        )
    ...
    def _calculate_revenue_impact(self, improvements: dict) -> float:
        ...
        high_impact = len(
            [i for i in improvements.get("improvements", []) if i.get("revenue_impact") == "high"]
        )
    ...
    async def get_training_status(self) -> dict[str, Any]:
        ...
        for brain_type in ["sub_agent", "voice_agent", "production"]:
            ...
            recent_sessions = [
                m for m in self.training_history if m.brain_type == brain_type and m.completed_at
            ][-5:]
...
# Singleton instance
_vertex_trainer: VertexContinuousTrainer | None = None
