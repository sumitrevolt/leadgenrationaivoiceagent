...
class AutoTrainer:
    ...
    async def run_nightly_training(self):
        ...
        try:
            ...
            return {"status": "success", "duration_seconds": training_duration, "results": results}
    ...
    async def train_intent_classifier(self) -> dict:
        ...
        try:
            # Get training data
            records = self.data_pipeline.get_training_data(min_turns=3, limit=5000)
            ...
            metrics = await self._train_sklearn_classifier(X_texts, y_intents, "intent_classifier")
    ...
    async def _train_sklearn_classifier(
        self, texts: list[str], labels: list[str], model_name: str
    ) -> TrainingMetrics:
    ...
    async def train_lead_scorer(self) -> dict:
        ...
        try:
            ...
            # Train model
            metrics = await self._train_sklearn_regressor(X_features, y_scores, "lead_scorer")

            return {"status": "success", "samples": len(X_features), "accuracy": metrics.accuracy}
    ...
    async def _train_sklearn_regressor(
        self, features: list[list[float]], labels: list[float], model_name: str
    ) -> TrainingMetrics:
    ...
    async def optimize_prompts(self) -> dict:
        ...
        try:
            ...
            best_patterns = []
            for _pattern_type, patterns in self.feedback_loop.patterns.items():
                top_patterns = sorted(
    ...
    async def _generate_optimized_prompts(self, best_patterns: list) -> dict[str, str]:
    ...
    async def generate_ab_variants(self) -> dict:
        ...
        try:
            ...
            return {"status": "success", "variants_generated": len(variants)}

        except Exception as e:
            logger.error(f"A/B variant generation failed: {e}")
            return {"status": "failed", "error": str(e)}

    def load_model(self, model_type: ModelType) -> Any | None:
    ...
    def get_training_stats(self) -> dict:
        ...
        return {
            ...
            "jobs_completed": len(
                [j for j in self.training_history if j.status == TrainingStatus.COMPLETED]
            ),
            "jobs_failed": len(
                [j for j in self.training_history if j.status == TrainingStatus.FAILED]
            ),
            "last_training": (
                self.training_history[-1].completed_at.isoformat()
