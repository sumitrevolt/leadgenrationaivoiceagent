...
class MLTrainingScheduler:
    ...
    def __init__(self, config: TrainingScheduleConfig | None = None, tenant_id: str = "default"):
    ...
    async def start(self) -> None:
        ...
        self.scheduler.add_job(
            ...
            CronTrigger(
                hour=self.config.nightly_training_hour, minute=self.config.nightly_training_minute
            ),
            id="nightly_training",
        ...
        logger.info(
            f"   📅 Nightly training: {self.config.nightly_training_hour}:{self.config.nightly_training_minute:02d}"
        )
        if self.config.weekly_training_enabled:
            logger.info(
                f"   📅 Weekly training: {self.config.weekly_training_day} at {self.config.weekly_training_hour}:00"
            )
    ...
    async def _run_nightly_training(self) -> TrainingReport:
        ...
        report = TrainingReport(
            run_id=f"nightly_{datetime.now().strftime('%Y%m%d_%H%M%S')}", started_at=datetime.now()
        )
        ...
        try:
            ...
            if data_stats["total_conversations"] < self.config.min_conversations_for_training:
                ...
                logger.warning("⚠️ Skipping training: insufficient data")
                return report
    ...
    async def _run_weekly_training(self) -> TrainingReport:
        ...
        report = TrainingReport(
            run_id=f"weekly_{datetime.now().strftime('%Y%m%d_%H%M%S')}", started_at=datetime.now()
        )
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"🏭 Brain #3 (Production) check completed: {readiness.get('overall_score', 0):.0f}% ready"
                )
            ...
            logger.info("✅ Weekly training completed - All 3 Brains processed")
    ...
    async def _get_training_data_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"total_conversations": 0, "positive_outcomes": 0, "new_since_last_training": 0}
    ...
    async def _cleanup_vector_store(self) -> None:
        ...
        try:
            ...
            removed = await self.vector_store.cleanup(
                max_age_days=cutoff_days, min_success_score=min_score
            )
    ...
    async def get_training_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "next_nightly": (
                str(self.scheduler.get_job("nightly_training").next_run_time)
                if self.is_running
                else None
            ),
            "next_weekly": (
                str(self.scheduler.get_job("weekly_training").next_run_time)
                if self.is_running and self.config.weekly_training_enabled
                else None
            ),
            "recent_runs": [
                {
    ...
    async def get_training_metrics(self) -> dict[str, Any]:
        ...
        avg_improvements = {
            model: sum(vals) / len(vals) for model, vals in all_improvements.items()
        }
...
_scheduler_instance: MLTrainingScheduler | None = None
