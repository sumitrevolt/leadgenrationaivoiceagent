...
class PipelineState:
    ...
    def mark_stage(self, stage: str, status: str, result: Any = None, error: str | None = None) -> None:
        state = self.load()
        ...
        all_done = all(
            state["stages"].get(s, {}).get("status") == "done"
            for s in STAGE_ORDER
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        done = sum(1 for s in STAGE_ORDER if state.get("stages", {}).get(s, {}).get("status") == "done")
...
def get_capacity_metrics(hours: int = 24, r=None) -> dict[str, Any]:
    ...
    try:
        ...
        for stage in STAGE_ORDER:
            stage_data: dict[str, Any] = {"ok": 0, "fail": 0, "ok_ms_sum": 0, "fail_ms_sum": 0, "samples": []}
            ...
            stage_data["failure_rate"] = round(stage_data["fail"] / total * 100, 1) if total > 0 else 0
...
async def stage_complete(cid: str, **kw) -> dict[str, Any]:
    ...
    try:
        clients_store.update_client(
            cid, setup_done=True, setup_at=_dt.now(_tz.utc).isoformat()
        )
    except Exception:
        pass
...
async def run_pipeline(
    ...
    *,
    ...
    for i in range(start_idx, len(STAGE_ORDER)):
        ...
        t0 = time.monotonic()
        try:
            stage_result = await executor(cid, force=force, send_welcome=send_welcome, kb_result=kb_result)
            ...
            if stage_result.get("ok") is False and stage_result.get("error"):
                # Stage failed
                state.mark_stage(stage_name, "failed", result=stage_result, error=stage_result["error"])
                result["stages"][stage_name] = {"status": "failed", "error": stage_result["error"], "duration_s": round(elapsed, 2)}
            ...
            result["stages"][stage_name] = {"status": "failed", "error": str(exc)[:200], "duration_s": round(elapsed, 2)}
            record_stage_metrics(stage_name, elapsed, False, r)
            result["overall_ok"] = False
            result["failed_at"] = stage_name
