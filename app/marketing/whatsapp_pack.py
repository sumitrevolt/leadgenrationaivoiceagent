...
def _template_pack(business_name: str, niche: str, occasion: str, offer: str) -> Dict[str, List[str]]:
...
async def broadcast_pack(
    ...
    def _pick(marker: str, pool: List[str], idx: int) -> str:
        ...
        return (val[:_MAX_BROADCAST_CHARS] if marker.startswith("BROADCAST") else val) \
            or pool[idx]

    broadcast = [_pick("BROADCAST1", fallback["broadcast"], 0),
                 _pick("BROADCAST2", fallback["broadcast"], 1)]
    status_lines = [_pick("STATUS1", fallback["status_lines"], 0),
                    _pick("STATUS2", fallback["status_lines"], 1),
                    _pick("STATUS3", fallback["status_lines"], 2)]
    reply_templates = [_pick("REPLY1", fallback["reply_templates"], 0),
