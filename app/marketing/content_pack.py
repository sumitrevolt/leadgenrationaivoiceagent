...
def _build_html(business_name: str, niche: str, month: str,
                calendar: List[Dict[str, str]], posts: List[Dict[str, Any]],
                poster_svgs: List[str], gbp: Dict[str, Any],
                wa: Dict[str, Any], fests: List[Dict[str, str]]) -> str:
    ...
    # --- Posters (inline SVG) ---
    poster_html = "".join(
        f"<div class='posterbox'>{svg}</div>" for svg in poster_svgs if svg
    )
    poster_html = (f"<div class='posters'>{poster_html}</div>"
                   "<p class='idea'>Tip: poster ko screenshot/PNG karke status + GBP par lagayein.</p>"
                   ) if poster_html else "<p>Posters nahi bane — Poster tab se banayein.</p>"

    # --- GBP description + services ---
    desc = str(gbp.get("description") or "").strip()
    gbp_html = (f"<p><b>Description (Edit profile → From the business me paste karein):</b></p>"
                f"<pre>{_e(desc)}</pre>") if desc else "<p>GBP description nahi bana.</p>"
    ...
    if status_lines:
        wa_html += ("<p><b>Status lines:</b></p><ul>"
                    + "".join(f"<li>{_e(s)}</li>" for s in status_lines) + "</ul>")
    ...
    fest_html = f"<table><tr><th>Date</th><th>Festival</th><th>Marketing angle</th></tr>{fest_rows}</table>"
...
async def build_client_pack(
    ...
    try:
        ...
        (calendar, post_plain, post_offer, post_fest,
         gbp, wa, poster_fest, poster_offer) = await asyncio.gather(
            ...
            _safe(_poster("festival-glow", business_name=business_name,
                          tagline=tagline, offer=offer, phone=phone_eff,
                          festival=fest_name, brand_primary=brand_primary,
                          brand_accent=brand_accent), {}),
            _safe(_poster("offer-burst", business_name=business_name,
                          tagline=tagline, offer=offer, phone=phone_eff,
        ...
        posts = [p for p in (post_plain, post_offer, post_fest)
                 if isinstance(p, dict) and str(p.get("caption") or "").strip()]
        poster_svgs = [str((p or {}).get("svg") or "").strip()
                       for p in (poster_fest, poster_offer)]
        poster_svgs = [s for s in poster_svgs if s.startswith("<svg")]

        html = _build_html(business_name, niche, month, calendar, posts,
                           poster_svgs, gbp, wa, fests)
        counts = {
            "posts": len(posts),
            "posters": len(poster_svgs),
            "calendar_days": len(calendar),
            "gbp_services": len(gbp.get("services") or []),
            "whatsapp_msgs": (len(wa.get("broadcast") or [])
                              + len(wa.get("status_lines") or [])
                              + len(wa.get("reply_templates") or [])),
            "festivals": len(fests),
        }
        return {"html": html, "counts": counts,
                "business_name": business_name, "niche": niche, "month": month}
        ...
        return {"html": html,
                "counts": {"posts": 0, "posters": 0, "calendar_days": 0,
                           "gbp_services": 0, "whatsapp_msgs": 0, "festivals": 0},
                "business_name": business_name, "niche": niche, "month": month,
                "error": str(e)}
