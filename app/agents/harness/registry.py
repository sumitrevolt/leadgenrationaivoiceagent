...
class ToolDefinition(BaseModel):
    ...
    def _name_ok(cls, v: str) -> str:
        if not _NAME_RE.match(v):
            raise ValueError(f"invalid canonical tool name: {v!r} (want <domain>.<capability>.<action>)")
        return v
    ...
    def public_view(self) -> dict:
        ...
        return {
            ...
            "risk_class": self.risk_class.value, "side_effect_class": self.side_effect_class.value,
            "authority": self.authority.value, "allowed_agents": sorted(self.allowed_agents),
            "allowed_tenant_scopes": sorted(self.allowed_tenant_scopes),
            "requires_approval": self.requires_approval,
            "requires_idempotency": self.requires_idempotency,
            "timeout_seconds": self.timeout_seconds, "sandbox_required": self.sandbox_required,
            "network_policy": self.network_policy, "enabled": self.enabled_by_default,
...
def _minimal_schema_check(schema: dict, args: dict) -> tuple[bool, str]:
    ...
    _pytypes = {"string": str, "integer": int, "number": (int, float),
...
class CanonicalToolRegistry:
    ...
    def list_tools(self) -> list[dict]:
        return [d.public_view() for d in sorted(self._defs.values(), key=lambda d: (d.name, d.version))]
    ...
    def evaluate_action(self, *, tool_name: str, tool_version: Optional[str],
                        arguments: dict, agent_id: str, tenant_id: str,
                        idempotency_key: Optional[str], claimed_risk: Optional[RiskLane]) -> dict:
        names = {n for (n, _) in self._defs}
        out: dict[str, Any] = {
            "registry_comparison": None, "resolved_tool_name": None, "resolved_tool_version": None,
            "schema_validation": None, "agent_permission": None, "tenant_permission": None,
            "registry_risk_class": None, "claimed_risk_class": (claimed_risk.value if claimed_risk else None),
            "risk_class_mismatch": None, "authority": None, "approval_requirement": None,
            "idempotency_requirement": None, "timeout_policy": None, "sandbox_requirement": None,
            "would_allow": None, "would_require_approval": None, "would_deny": None,
            "would_deny_reason": None,
        }
        if tool_name not in names:
            out.update(registry_comparison=RegistryStatus.UNREGISTERED_TOOL.value,
                       would_allow=False, would_deny=True, would_require_approval=False,
                       would_deny_reason="tool not in canonical registry (fail-closed)")
            return out
        defn = self.resolve(tool_name, tool_version)
        if defn is None:
            out.update(registry_comparison=RegistryStatus.VERSION_MISMATCH.value,
                       resolved_tool_name=tool_name, would_allow=False, would_deny=True,
                       would_deny_reason=f"version {tool_version} not registered")
            return out
        out.update(resolved_tool_name=defn.name, resolved_tool_version=defn.version,
                   registry_risk_class=defn.risk_class.value, authority=defn.authority.value,
                   timeout_policy=defn.timeout_seconds, sandbox_requirement=defn.sandbox_required)
        if not defn.enabled_by_default:
            out.update(registry_comparison=RegistryStatus.DISABLED.value, would_allow=False,
                       would_deny=True, would_deny_reason="tool disabled")
            return out
        ok_schema, serr = _minimal_schema_check(defn.input_schema, arguments)
        out["schema_validation"] = ok_schema
        if not ok_schema:
            out.update(registry_comparison=RegistryStatus.SCHEMA_MISMATCH.value, would_allow=False,
                       would_deny=True, would_deny_reason=f"schema: {serr}")
            return out
        ...
        if not agent_ok:
            out.update(registry_comparison=RegistryStatus.AGENT_NOT_ALLOWED.value, would_allow=False,
                       would_deny=True, would_deny_reason=f"agent {agent_id} not allowed")
            return out
        if not tenant_ok:
            out.update(registry_comparison=RegistryStatus.TENANT_NOT_ALLOWED.value, would_allow=False,
                       would_deny=True, would_deny_reason="tenant scope not allowed")
            return out
        out["idempotency_requirement"] = defn.requires_idempotency
        if defn.requires_idempotency and not idempotency_key:
            out.update(registry_comparison=RegistryStatus.IDEMPOTENCY_REQUIRED.value, would_allow=False,
                       would_deny=True, would_deny_reason="mutation requires idempotency key")
            return out
        ...
        amber = (defn.risk_class is RiskLane.AMBER
                 or defn.authority in (AuthorityClass.APPROVAL_REQUIRED, AuthorityClass.OWNER_OS_REQUIRED)
                 or defn.requires_approval)
        out["approval_requirement"] = bool(amber and not red)
        if red:
            out.update(registry_comparison=RegistryStatus.REGISTRY_MATCH.value, would_allow=False,
                       would_deny=True, would_require_approval=False,
                       would_deny_reason="authority ALWAYS_REFUSED / RED lane")
        elif amber:
            out.update(registry_comparison=(RegistryStatus.RISK_CLASS_MISMATCH.value if mism
                                            else RegistryStatus.REGISTRY_MATCH.value),
                       would_allow=False, would_require_approval=True, would_deny=False,
                       would_deny_reason=None)
        else:
            out.update(registry_comparison=(RegistryStatus.RISK_CLASS_MISMATCH.value if mism
                                            else RegistryStatus.REGISTRY_MATCH.value),
                       would_allow=True, would_require_approval=False, would_deny=False)
        return out
...
def _register_builtins() -> None:
    ...
    try:
        REGISTRY.register(ToolDefinition(
            ...
            input_schema={"type": "object", "properties": {"id": {"type": "string", "maxLength": 200}},
    ...
    try:
        ...
        REGISTRY.register(ToolDefinition(
            ...
            version="1.0.0",
            description=("Nikhil Revenue-Ops composite: revenue_digest + client_health + "
    ...
    try:
        ...
        REGISTRY.register(ToolDefinition(
            ...
            version="1.0.0",
            description=("Coordinator delegation to Dev (research). Downstream executor "
    ...
    try:
        ...
        REGISTRY.register(ToolDefinition(
            ...
            version="1.0.0",
            description=("Supervisor/coordinator delegation to Rohan (leads/outreach). Rohan's "
