    ...
    def calculate_percentages(self):
        ...
        for i, stage in enumerate(self.stages):
            ...
            if i > 0 and self.stages[i - 1].count > 0:
                stage.conversion_from_previous = (stage.count / self.stages[i - 1].count) * 100
...
class AnalyticsDashboard:
    ...
    def record_call(self, call_data: dict[str, Any]):
        ...
        self.call_logs.append({**call_data, "recorded_at": datetime.now()})

    def record_lead(self, lead_data: dict[str, Any]):
        """Record lead qualification result"""
        self.lead_records.append({**lead_data, "recorded_at": datetime.now()})

    def record_objection(self, objection_data: dict[str, Any]):
        """Record objection handling"""
        self.objections.append({**objection_data, "recorded_at": datetime.now()})
    ...
    def calculate_roi(
        ...
        # Calculate ROI
        if roi.total_cost > 0:
            roi.roi_percentage = float(((roi.total_value - roi.total_cost) / roi.total_cost) * 100)
    ...
    def get_conversion_funnel(
        ...
        call_metrics = self.get_call_metrics(
            tenant_id=tenant_id, campaign_id=campaign_id, time_range=time_range
        )
    ...
    def get_lead_quality_funnel(
        self, tenant_id: str | None = None, time_range: TimeRange = TimeRange.LAST_30_DAYS
    ) -> ConversionFunnel:
        ...
        for obj_type, instances in objection_groups.items():
            ...
            analysis.append(
                ObjectionAnalysis(
                    ...
                    common_responses=[o.get("ai_response", "")[:100] for o in instances[:3]],
                )
            )
    ...
    def get_dashboard_summary(
        ...
        self.get_call_metrics(tenant_id=tenant_id, time_range=TimeRange.LAST_30_DAYS)
        ...
        return {
            ...
            "top_industries": [
                {"name": p.industry, "appointment_rate": f"{p.appointment_rate * 100:.1f}%"}
    ...
    def _filter_by_time(
        ...
        return [r for r in records if start <= r.get("recorded_at", datetime.min) <= end]
    ...
    def _get_date_range(
        ...
        ranges = {
            ...
            TimeRange.THIS_MONTH: (now.replace(day=1, hour=0, minute=0, second=0), now),
