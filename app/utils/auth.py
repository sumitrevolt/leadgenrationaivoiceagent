...
def create_access_token(
    user_id: str, email: str, role: str, expires_delta: timedelta | None = None
) -> str:
    """
...
def refresh_access_token(refresh_token: str, email: str, role: str) -> str | None:
    """
...
def require_role(*allowed_roles: str):
    ...
    async def role_checker(payload: dict = Depends(get_current_user_payload)) -> dict:
        ...
        if user_role not in allowed_roles:
            raise HTTPException(
                status_code=403, detail=f"Role '{user_role}' not authorized for this action"
            )
        return payload
...
def hash_password(password: str, salt: str | None = None) -> tuple[str, str]:
    ...
    try:
        ...
        logger.warning(
            "passlib not installed, using SHA-256 fallback (not recommended for production)"
        )
        if salt is None:
...
class RateLimiter:
    ...
    def is_allowed(self, identifier: str) -> bool:
        ...
        if identifier in self.requests:
            self.requests[identifier] = [
                (ts, count) for ts, count in self.requests[identifier] if ts > window_start
            ]
    ...
    def get_remaining(self, identifier: str) -> int:
        ...
        request_count = sum(count for ts, count in self.requests[identifier] if ts > window_start)
