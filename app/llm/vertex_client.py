...
class RateLimitBucket:
    ...
    _token_usage: dict[str, int] = field(
        default_factory=lambda: {"minute": 0, "last_reset": time.time()}
    )
        ...
        for attempt in range(retry_count):
            try:
                ...
                if self._client_type == "vertex":
                    response = await self._generate_vertex(
                        prompt, system_instruction, temperature, max_tokens
                    )
                else:
                    response = await self._generate_gemini_api(
                        prompt, system_instruction, temperature, max_tokens
                    )
                ...
                if "500" in error_str or "503" in error_str or "unavailable" in error_str:
                    ...
                    logger.warning(
                        f"Transient error, retrying in {wait_time}s (attempt {attempt + 1}/{retry_count}): {e}"
                    )
                    await asyncio.sleep(wait_time)
                    continue
...
def get_vertex_client(
    model_name: str = "gemini-1.5-flash", force_new: bool = False
) -> VertexAIClient:
    """
...
async def generate_quick(
    prompt: str, system_instruction: str | None = None, model: str = "gemini-1.5-flash"
) -> str:
