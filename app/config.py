...
class Settings(BaseSettings):
    ...
    # Vobiz — INERT legacy fields (2026-09-15). Provider removed from the routing
    # path (default_telephony above + CallManager._build_handler refuse legacy
    # strings), but a dozen diagnostic/read sites still touch these fields
    # (app/api/health.py provider-report, app/api/telephony_vobiz.py status
    # endpoint, app/telephony/vobiz_handler.py, compliance caller-id fallback).
    # Removing them raised AttributeError at import/request time on those read-only
    # surfaces. Keep as inert defaults: empty => every reader reports "not
    # configured", exactly as a removed provider should. Safe to delete only when
    # VobizClient + its endpoints are fully removed (separate task).
    vobiz_auth_id: str = ""
