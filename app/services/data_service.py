...
from app.models.data_credits import (
    DataCredits, CreditTransaction, APIUsageLog, APIKey,
    CreditTransactionType, APIUsageType, CREDIT_COSTS
)
...
class DataService:
    ...
    async def search_companies(
        ...
        if verified_only:
            filters.append(Lead.verified == True)
        if has_email:
    ...
    async def enrich_company(
        ...
        result = await self.db.execute(
            select(Lead).where(Lead.id == company_id)
        )
    ...
    async def export_companies(
        ...
        if not credits_account or not credits_account.has_credits(credits_needed):
            return [], f"Insufficient credits. Need {credits_needed}, have {credits_account.balance if credits_account else 0}"
        
        # Get companies
        result = await self.db.execute(
            select(Lead).where(Lead.id.in_(company_ids))
        )
        leads = result.scalars().all()
    ...
    async def _generate_market_report(
        ...
        query = select(
            ...
            func.count(Lead.id).filter(Lead.verified == True).label("verified_count"),
        ...
        ).where(Lead.niche == niche).group_by(Lead.city).order_by(func.count(Lead.id).desc()).limit(10)
        ...
        score_query = select(
            ...
            func.count(Lead.id).filter(and_(Lead.lead_score >= 40, Lead.lead_score < 70)).label("medium"),
        ...
        report = {
            ...
            "summary": {
                ...
                "verified_percentage": round((stats.verified_count / stats.total_companies * 100) if stats.total_companies else 0, 1),
                "with_email_percentage": round((stats.with_email / stats.total_companies * 100) if stats.total_companies else 0, 1),
                "with_phone_percentage": round((stats.with_phone / stats.total_companies * 100) if stats.total_companies else 0, 1),
            },
        ...
        if report_type in ["detailed", "custom"]:
            ...
            top_companies_query = select(Lead).where(
                and_(*filters)
            ).order_by(Lead.lead_score.desc()).limit(20)
    ...
    async def revoke_api_key(self, client_id: str, key_id: str) -> bool:
        ...
        result = await self.db.execute(
            select(APIKey).where(
                and_(APIKey.id == key_id, APIKey.client_id == client_id)
            )
        )
        api_key = result.scalar_one_or_none()
    ...
    async def _get_credits_account(self, client_id: str) -> Optional[DataCredits]:
    ...
    ) -> Tuple[bool, Optional[DataCredits]]:
    ...
    async def _log_credit_transaction(
        ...
        description: str = "",
        reference_id: Optional[str] = None,
        usage_type: Optional[APIUsageType] = None,
    ) -> None:
    ...
    async def get_usage_stats(
        ...
        usage_by_type = {
            row.usage_type.value: {"count": row.count, "credits": row.credits}
            for row in result
        }
        ...
        daily_usage = [
            {"date": str(row.date), "credits": row.credits}
