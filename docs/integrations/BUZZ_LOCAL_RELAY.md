...
> Owner decision 2026-08-10: relay **local-first** (`ws://127.0.0.1:3000`), phir VPS.
...
| Service | Role |
|---------|------|
| `relay` | Buzz relay (`ws://127.0.0.1:3000`, `/_liveness`) |
...
3. `.env` ensure: copy from example + replace chain hamesha apply (sirf placeholder
   ...
   `RELAY_URL=ws://127.0.0.1:3000` — same posture as `just dev`)
4. `docker compose up -d` (image pull + start)
5. `/_liveness` poll (≤120s) → "Buzz relay LIVE at ws://127.0.0.1:3000"
...
1. **Desktop:** Buzz Desktop → settings → relay `ws://127.0.0.1:3000` (identity wahi
...
2. **Tabhi `BUZZ_RELAY` flip karo** jab local workspace ready ho (pehle nahi —
   ...
   ```powershell
   setx BUZZ_RELAY ws://127.0.0.1:3000
   ```
...
- **CHANNEL_IDS vs BUZZ_RELAY mismatch:** local IDs + hosted default relay = dead
  posts. After liveness OK, `setx BUZZ_RELAY ws://127.0.0.1:3000` BEFORE tooling
