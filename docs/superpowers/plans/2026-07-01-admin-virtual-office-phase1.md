...
**Architecture:** One new self-contained frontend page (`frontend/office_map.html`, same pattern as `team_dashboard.html`/`control_center.html` — inline `<style>`+`<script>`, no build step) using Phaser.js (CDN, MIT license) purely for canvas layout/interactivity — rooms and avatars are drawn with Phaser `Graphics`/`Text` primitives, **no external tile/sprite image assets in Phase 1** (that's an explicit later-phase polish item per the design spec, not a Phase-1 blocker). All data comes from two already-existing, unmodified endpoints: `GET /api/platform/team` (roster + live state) and `GET /api/platform/team/events?member=&limit=` (per-agent event log). Zero backend changes.

**Tech Stack:** Phaser 3.80 (CDN `https://cdn.jsdelivr.net/npm/phaser@3.80.1/dist/phaser.min.js` — same CDN-script pattern as Chart.js already used in `frontend/admin_dashboard.html:7`), vanilla JS (no framework, matches every other admin page in this repo), existing `/design-system/styles.css`.
