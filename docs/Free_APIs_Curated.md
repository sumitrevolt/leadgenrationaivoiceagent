...
| API | Use | Status |
|---|---|---|
| **Nager.Date** (`date.nager.at`, NO key) | Official India public holidays → festival/content calendar enrich | LIVE in `app/marketing/festivals.py` (`fetch_public_holidays`, `upcoming_enriched`). Gated `FESTIVALS_LIVE_HOLIDAYS=1` (default OFF = static list). |
