...
- **`app/`-scoped only** (631 files). `frontend/`, `unity/`, `docker-compose*.yml`, root scripts = NOT in graph → use grep/Read.
...
- **Node-ID scheme is pre-#1504** (graph note) — same-name files (`__init__.py`, `base.py`) can collide. For path-qualified IDs run `graphify extract --force app` once (bigger rebuild); optional, low priority.
