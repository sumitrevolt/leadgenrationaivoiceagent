...
- **PR #162** — A2 compliance runtime-data authority wave. Blocking-count arithmetic unchanged at 21 by design. Read its head, state and checks from GitHub; the SHAs named in the A2 note below are a **historical ledger of what was reviewed and when**, not a statement about where the branch is now. It has already moved more than once.
...
- CI exit 139 is a real SIGSEGV during cyclic garbage collection. It struck three times on the A2 branch at three different points in the suite, and the second attempt named that wave's own AST-scanning test — which is why the scan was made ~330× cheaper. **That was hardening, not the fix**, and the crash recurred afterwards with no A2 code on the stack.
  **The cause is UNKNOWN.** `tests/conftest.py:190-198` describes a *previously observed* exit-139 with a specific mechanism — an orphaned aiosqlite `_connection_worker_thread` traversed by cyclic GC — and reports its NullPool mitigation as effective ("zero with NullPool" in its own repro). An earlier version of this note said that mitigation "evidently does not fully close it"; that **contradicted the cited source** and was retracted. The honest position: the same symptom has reappeared, the documented mechanism was reported fixed, and nobody has shown the two are the same bug. Do not attribute a GC-phase SIGSEGV to whatever function happens to be on the stack, and do not attribute it to a mitigated leak either.
  Open question, in order: is it branch-correlated or global? The docs-only branch passed `prod_check + pytest` on two heads while the A2 branch failed three times — suggestive, not conclusive at that sample size. Get the base rate first (`gh run list --workflow CI` over ~100 runs, grep each failure log for `exit code 139`) before spending another cycle on either branch.
