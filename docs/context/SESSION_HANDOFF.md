...
- CI exit 139 is a real SIGSEGV during cyclic garbage collection. It struck three times on this branch at three different points in the suite, and the second attempt named the wave's own AST-scanning test — which is why that scan was made ~330× cheaper. **That was hardening, not the fix.** `tests/conftest.py:190-198` already documents this failure: an orphaned aiosqlite `_connection_worker_thread` traversed by cyclic GC, with a NullPool mitigation that evidently does not fully close it. Do not attribute a GC-phase SIGSEGV to whatever function happens to be on the stack. Open question for whoever picks this up: is it branch-correlated or global? Sample `feat/ext-ctxtruth-0c09ee` (main-equivalent code) repeatedly before blaming A2.
