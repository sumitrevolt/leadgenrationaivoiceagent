...
2026-08-15 ~14:10Z — prod `/health` = `07870e89` (DIRECT_HOST_VERIFIED, public dual probe 14:09:45Z / 14:09:48Z uptime 0h2m23s→0h2m27s, then 14:10:17Z / 14:10:20Z 0h2m55s→0h2m58s). `environment:production`. Smoke `/` `/pricing` `/start` `/health/ready` = 200. `origin/main` = `94ab3167` (docs-only #372 on top of `07870e89`) — **runtime already live**; docs SHA not required on VPS. GitHub heads = `main` only. Open PRs = 0. This sandbox still has no VPS SSH; Actions Build/Deploy remain skipped (`DEPLOY_ENABLED` not true) — live SHA moved by a host that *does* have SSH (not this agent). Named blocker still owner-side (`blocker_count=1`, `payments_ready=true`, `ready_for_first_paid_customer=false`). `HQ_AUTO_CHASE` CODE-PRESENT, keep INERT. 5/5 image pin = UNVERIFIED here (no SSH).
