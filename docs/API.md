...
## Endpoint Index — auto-generated from OpenAPI (1295 operations)
...
- `GET   ` `/api/upi/pending` — Admin: pending + approved-but-unbound UPI queue
- `POST  ` `/api/upi/pending/{pid}/approve` — Admin: approve a UPI submission (optional JSON `client_id` bind-on-approve)
- `POST  ` `/api/upi/pending/{pid}/reject` — Admin: reject a UPI submission
- `POST  ` `/api/upi/pending/{pid}/bind` — Admin: bind `client_id` on guest UPI; activates if already approved (#304)
