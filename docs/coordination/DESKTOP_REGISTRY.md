...
3. **buzzlock TOOL tokens must match `scripts/buzzlock.py` `TOOLS`.** `FREEBUFF` / `OPENCODE` / `CURSOR` are enrolled today. `ANDROID` is documented-but-pending: `--tool ANDROID` will be rejected until Owner OS adds it to `TOOLS` (a separate change — not in this registry's scope). Buzz has no lock role; it is the bus owner.
