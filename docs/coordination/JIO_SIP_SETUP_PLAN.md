...
>
> ⚠️ **COMPLIANCE REVISION (2026-08-30, deep-research confirmed):** Mobile SIP's CLI is an **ordinary mobile (non-140) DID** → under TRAI TCCCPR (140-series mandatory for promotional outbound, even with consent) it **CANNOT carry cold promotional calls** — that is ``platform_dial``/fire_calls promo territory. Reposition Jio: **transactional / service / reactivation / inbound + web-demo lanes ONLY**. Cold promo stays on Vobiz (or a future 140-provisioned CLI). Code enforced: `app/telephony/trunks.py` lane-guards `jio_mobile` to `lanes={"transactional"}` — `pick_trunk(lead)` fail-closes to the promotional lane on unknown leads, so a promo call never routes over Jio. This reverses the "round-robin 50/50" in activation step 10 (see step 10 note).
...
10. Mirror Vobiz in `app/telephony/call_manager.py` — both trunks live, jio_mobile primary **ONLY for transactional/service/reactivation + inbound-consistent lanes** (2026-08-30 revision: NOT for cold promo — jio CLI is non-140; promo stays vobiz)
