...
# Prepared 2026-08-10 by Cursor during PR #302 remediation.
...
- gdb (Linux CI-equivalent container, prior forensics on PR #302): faulting
  instruction loads `pimpl` then dereferences NULL (`rdi == 0`).
- Trigger surface observed under pytest 9 `gc_collect_harder` / gen-2 GC during
  large pytest-asyncio 1.4.0 suites.

## Not proven
- Exact project test/fixture lifecycle that leaves the invalid greenlet.
