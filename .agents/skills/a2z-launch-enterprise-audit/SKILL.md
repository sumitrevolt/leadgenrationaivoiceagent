...
- `.Codex/skills/context-first/SKILL.md` (MANDATORY pre-flight) + `.Codex/skills/leadgen-composer/SKILL.md`
- `.Codex/skills/production-ready/SKILL.md` (launch gate) + `.Codex/skills/enterprise-readiness-audit/SKILL.md` (12-domain /120)
- `AGENTS.md` §5 invariants + §7 landmines + `## Current State` (auto-loaded) + `docs/LOOP_ENGINEER.md` (9-field format, 8 hats)
- `memory/INDEX.md` → sirf task-relevant: `decisions.md` (recent ADRs), `integrations.md`, `incidents.md`
- Graphify protocol: `AGENTS.md §9.5` + `docs/GRAPHIFY.md`
- Domain dispatch skills (load on demand per domain, see §7): `leadgen-*` audit pack + enterprise pack (list in `.Codex/skills/SKILLS_PARITY.md`).
...
Known landmines to actively check (§7 AGENTS.md): stale-`.pyc` 404 on new page-route · function-level import hides startup failure (grep ALL callers of retired helpers) · `app:8080` in-network vs `8000` host trap · `${APP_VERSION:-latest}` provenance · `USE_SILERO_VAD=0` · Sentry `search_issues` for real bugs invisible in log-grep.
