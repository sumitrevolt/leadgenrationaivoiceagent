...
- **This page IS the observability surface** — control center hi `automation_health` (dead-man overdue), agent events, flags pills, run histories ko operator ke saamne laata. Naya backend automation merge = yahaan tab/badge bhi add karo, warna observability gap (AGENTS.md: API-only = adhoora).
