...
- **Down / negative** (user_correction, verify fail dohraye) → recurring anti-pattern ko ek `guard.py`/`skill_reminder.py` guardrail ya AGENTS.md gotcha-line me convert karo.
Ye Codex (dev-time agent) ke self-improvement ka closed loop hai — naya dashboard NAHI, existing machinery hi.
