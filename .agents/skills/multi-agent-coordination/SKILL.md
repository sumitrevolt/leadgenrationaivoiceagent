...
| **Codex session decision protocol** | Skill **`llm-council-decision`** — recruit experts → opinions → peer review → Chairman verdict (subagents ya LIVE API) | — |
...
- Skills: agents runtime pe `skill_pack.find/snippet_for` se .Codex/skills + `data/skills_extra` padhte hain (SKILL_PACK=1) — naya skill image-rebuild ke baad container me dikhta hai (Dockerfile COPY).
...
- AGENTS.md rule: heavy sub-agents kam — orchestration VPS engines me hai, Codex-session me sirf jab disjoint-files batch ho (`parallel-batch-build`).
