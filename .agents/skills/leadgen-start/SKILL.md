...
Iska maqsad: naya chat minimum token me oriented ho jaye. `AGENTS.md` (lean working memory) pehle se context me hai — use re-read mat karo. Deep history `docs/SESSION_LOG.md` me (auto-load NAHI hota).
...
5. **Windows = source of truth** (sandbox mount file-edits ke baad STALE). Verify Windows pe (Read/Edit/`.venv` python). AGENTS.md/SESSION_LOG sandbox-bash se KABHI mat chhuo.
...
5. Done → `docs/SESSION_LOG.md` milestone (Edit only), AGENTS.md 1-line if needed.
