...
- **Image** (`Dockerfile.lock`): code (`app/`+`frontend/`+`.Codex/skills/`) BAKED → code change = `docker compose build app` + `up -d --no-deps app` recreate (data-only `./data`/`./logs` bind-mount change ko NAHI).
