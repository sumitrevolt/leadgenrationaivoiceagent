...
- User se SIRF tab poochho jab decision genuinely uska hai: pricing/plan change · live deploy auth · destructive op · ambiguous product-intent jo git-history/AGENTS.md se resolve na ho. Ek focused sawaal, concrete options ke saath.
...
### A9. Context hygiene
- `memory/INDEX.md` pehle, phir SIRF task-relevant memory files. File-dumps subagent me, conclusions main thread me. Code vs memory conflict = code wins, phir memory fix. Architecture change = same session me AGENTS.md `## Current State` + memory write-back.
...
1. **Discover (A1/A2 style):** parallel Grep/Glob se SAARE touch-points — definition, callers, routes (`@router`/`@app`), templates/JS, tests. Ek miss = regression. Intent confirm karo (git log/AGENTS.md/tests) — "toota hai" assume nahi.
...
5. **Evidence:** `/verify` green (prod_check + targeted tests + import) + claim-ka-artifact. Frontend/page change = live-browser bhi: `cd frontend && python -m http.server 8123` + Codex-in-chrome se drive (2026-07-05 office-upgrade pattern).
...
App = **systemd unit `leadgen`** (host uvicorn from `/opt/leadgen/.venv`, `EnvironmentFile=/opt/leadgen/.env`, `Restart=always`) — NOT a container; there is no `leadgen_app` (topology corrected 2026-09-15). Loop: prod_check → changed-file tests → Windows git push → VPS `bash scripts/deploy_vps.sh` (canonical parent: fetch → candidate worktree → runtime-data guard → build → `prod_check --deployment` → live pull → write `.env:APP_VERSION` → compose rollout of worker/scheduler/worker-heavy/worker-video → alembic → `systemctl restart leadgen` → `/health` verify). An app-code change needs `git pull` + `systemctl restart leadgen` and **no rebuild**; `app/`+`frontend/`+`.Codex/skills/` image me BAKED into `Dockerfile.lock` for the **Celery workers**, so a worker-code change DOES need `build` + `up -d --no-deps <service>`. `data/`+`logs/` bind-mount. CI gate-only (`DEPLOY_ENABLED` unset). **Live deploy = explicit user-auth, infer mat karo (A10).**
...
- Ambiguous product decision → git/AGENTS.md intent → warna 1 focused sawaal (A4). Ambiguous strategy/go-no-go → `llm-council-decision`.
