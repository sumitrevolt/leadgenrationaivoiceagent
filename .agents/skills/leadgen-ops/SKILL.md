...
- **Code change hai?** Sirf `./data`/`./logs` (bind-mount) badla → rebuild NAHI, sidha recreate/koi action nahi. Code/`app/`/`frontend/`/`.Codex/skills/` badla → full rebuild zaroori (image me BAKED).
...
- **Safety:** secrets sirf `/opt/leadgen/.env` (gitignored) — `scripts/check_secrets.py` step-3 me; never in committed file/`.bat`/AGENTS.md. Code/skill change = rebuild (BAKED); data-only (`./data`/`./logs`) = no rebuild.
