...
4. **Secrets** — only in `.env` (gitignored). NEVER in code/AGENTS.md/scripts/commits. New secret? Confirm it's `.env`-only + `.env.example` documents the NAME (not value). Check `git diff` for accidental secret leak.
...
- **Fail-CLOSED non-negotiables (bypass = reject, legal/financial risk):**
  ...
  - **Secrets** — sirf `.env` (gitignored), `.env.example` me NAME-only. Committed file/AGENTS.md/script me KABHI nahi.
