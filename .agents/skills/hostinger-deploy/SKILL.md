...
5. **App is in the Docker image** — `app/` + `frontend/` + `.Codex/skills/` are BAKED into `Dockerfile.lock`. Code/skill change = **`docker compose build app` + `up -d --no-deps app`** (NOT git-pull-only restart). Only `./data` + `./logs` are bind-mounts (data-only change = no rebuild).
