...
- [ ] Constraint confirm: bottleneck kahan hai? **Current sprint constraint canonical context se padho** (`AGENTS.md` → Current State sprint goal, `docs/context/`) — ye change hota rehta hai, kisi specific surface ko hardcode mat karo. Naya top-funnel loop tab tak waste hai jab tak mid-funnel constraint clear na ho.
- [ ] Overlap check: existing skill/capability grep (`.Codex/skills/`), duplicate route grep (`duplicate-route-guard`) — pehle se hai to EXTEND, rebuild nahi.
