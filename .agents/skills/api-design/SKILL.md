...
1. **GREP FIRST (mandatory, non-negotiable).** Before adding ANY route/module, search for an existing one:
   `grep -rn "@router\|@app.get\|@app.post" app/api/<area>.py` and `grep -rn "def <likely_name>" app/`. FastAPI is first-route-wins → a duplicate path silently SHADOWS prod. This project has lost hours to duplicate festival/review/route modules. If it exists, EXTEND it — don't rebuild.
2. **Additive + gated.** New behavior defaults to today's behavior. Risky/new = behind an env flag (default OFF) or `Depends(require_admin)`. Zero behaviour change unless explicitly enabled.
