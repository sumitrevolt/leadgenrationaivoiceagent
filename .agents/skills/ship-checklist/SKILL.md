...
4. **Deploy to VPS** (Git's ssh `C:\PROGRA~1\Git\usr\bin\ssh.exe`, PowerShell for clean quoting): `cd /opt/leadgen && git fetch && git merge --ff-only origin/main && docker compose -f docker-compose.vps.yml build app && up -d app`.
