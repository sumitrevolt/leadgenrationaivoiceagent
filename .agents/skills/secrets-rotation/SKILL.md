...
> Enterprise audit skill. Rule of house: **secrets SIRF `.env`** (gitignored), kabhi committed file/AGENTS.md/scripts me nahi. `scripts/check_secrets.py` `/verify` step-4 me wired (false-positive = line pe `nosecret`). Pehle `context-first`.
