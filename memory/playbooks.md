...
## OpenClaw LOCAL desktop gateway — 24/7 run + recovery (2026-08-02)
Prod OpenClaw (Stage A, in-app) is unrelated to this: it rides the `leadgen_app` container, `restart=unless-stopped`, `OPENCLAW_ENABLED=1` / `OPENCLAW_ALLOW_RED_ACTIONS=0` / gateway IPs `127.0.0.1,::1` — verified 24/7 on 2026-08-02.
**Local install:** openclaw `2026.7.1-2` at `C:\oc` (bundled node `C:\oc\node-v24.16.0-win-x64\node.exe` — system node 24.9.0 is NOT WAL-reset-safe, openclaw refuses it). Config/state `~/.openclaw`.
...
**Provider = Groq, NOT Gemini.** All 8 keys in the old `GEMINI_API_KEYs.txt` are DEAD (HTTP 400 on `models.list`); the only live Gemini key is the voice-scoped one in `.env` and sharing it would eat voice quota. Groq/Cerebras/NVIDIA all returned HTTP 200 — Groq chosen.
...
**Gotcha:** the legacy `OpenClaw Gateway` scheduled task cannot be re-registered without admin (`Register-ScheduledTask` = Access Denied 0x80070005). Do NOT disable the Startup VBS expecting the task to take over — that leaves nothing starting the gateway. Startup VBS + Watchdog task is the working no-admin combo; the legacy task stays Disabled on purpose.
...
Layout now: `_archive_2026-08-02` (worktrees + backups + loop27/28 patch), `_trading` (MT5/crypto), `_personal` (docx), `_secrets_DO_NOT_COMMIT` (API-key txt — deliberately OUTSIDE the repo, never commit).
...
## TypeSafe credential state + local activation (2026-09-18)

- **Truth check (key value KABHI print nahi hoti):** `python scripts/typesafe_status.py` (offline state) · `--probe` (live `POST /v1/systemone`) · `--json`. States: `PRESENT` · `ABSENT` · `INVALID` · `ROTATION_REQUIRED`; exit codes 0 / 2 / 3 / 4.
- **Local activation (ek owner action, hidden input, shell history me nahi):** `python scripts/typesafe_status.py --set-key-stdin` → repo `.env` me `TYPESAFE_API_KEY` likhta hai (gitignored, backup `.env.bak_typesafe_<ts>`, mode 0o600) aur turant probe karta hai.
- **App visibility:** `app/main.py` me `load_dotenv` NAHI hai, isliye `.env` me pada key `os.getenv` ko nahi dikhta. Dev run: `uvicorn app.main:app --reload --env-file .env`. VPS/systemd me key `EnvironmentFile`/compose env se aati hai — wahan koi code change nahi chahiye.
- **Module read order:** `TYPESAFE_API_KEY` → `TYPEsafe_API_KEY` (legacy) → `""` = INERT (fail-closed, kabhi silently authenticated nahi). Source me fallback literal KABHI nahi — 7317f990 ne live key aise leak ki thi, 979c2229 ne hatayi.
- **VPS write alag:** `scripts/env_set.py` (owner-gated). `--set-key-stdin` sirf repo-local file likhta hai; outside-repo ya `*_ENV=production` wala target REFUSE karta hai.
- **Compromised trip-wire:** `COMPROMISED_FINGERPRINTS` (`app/platform/typesafe_integration.py` — single source of truth; script wahin se padhta hai) me sirf `sha256[:12]` hashes hain (values nahi). Match hone par state `ROTATION_REQUIRED` + exit 4 — us key ko wapas arm mat karo, dashboard se naya key mint karo.
- **Silent-INERT visibility (2026-09-18):** TypeSafe call sites code me unconditional hain, isliye `automation_health.wiring_gaps()` ab credential gap report karta hai — key ABSENT ho ya exposed fingerprint par. Default ON; deliberate silence ke liye `TYPESAFE_ENABLED=0`. Check config-only hai (network call nahi) — isliye Mission Control load pe safe; live validity ke liye alag se `--probe` chalao.
- **Full local proof sequence (cold start):** `env -u TYPESAFE_API_KEY python scripts/typesafe_status.py --probe` (`.env` se) → phir `uvicorn.config.Config('app.main:app', env_file='.env')` (yani `--env-file` wala path) → `TYPESAFE_API_KEY: PRESENT` + `integration enabled: True`. Dono skip karke "kaam kar raha hai" mat kehna.

## TypeSafe admin triage — decision engine ka canonical path (2026-09-19)

- **Kab chalao:** jab bhi admin/owner ko evidence-backed findings me se *kaam ka order* choose karna ho. `python scripts/typesafe_admin_triage.py` (network) · `--json` (machine-readable) · `--dry-run` (judge karo, trace mat likho) · `--record-outcome <task_id> --outcome "..."`.
- **Input:** `docs/coordination/ADMIN_FINDINGS.json` — **tracked** source input (`.gitignore` ka blanket `*.json` isse ignore kar raha tha, isliye explicit `!docs/coordination/ADMIN_FINDINGS.json` negation hai; hataao to triage tool `REFUSED: findings registry not found` dega). Sirf `status: "open"` items judge hote hain. Har finding me `evidence` (command + observed output) mandatory hai — evidence ke bina judgment honest nahi hoti.
- **Ek request, kai independent judgments:** per-finding `Noul` (genuine risk?) + global `Score` (revenue impact) + `Choice` (next action) + `Noul` (owner-gate?). Skill ka rule: independent questions ek saath, kyunki wo ek doosre ka answer nahi dekhte.
- **Live wire contract (jev-1.13.0, verify kiya hua):** Noul `{"type":"noul","noul":0.94}` · Choice `{"type":"choice","choice":"B-2","confidence":0.89,"probabilities":{...}}` · Score `{"type":"score","score":3.0,"confidence":1.0,"legend":{"3":"..."},"probabilities":{...}}` — **`score` level ka INDEX hai aur `legend` usse naam deta hai** (string nahi). Isi liye `score_level()` legend se description nikalta hai; parser me string-expect karne se severity chup-chaap 1 par gir jati thi.
- **Fail-closed semantics:** credential ABSENT/INVALID ya HTTP error → **exit 3, koi decision record NAHI likha**. "Priority invent karna" allowed nahi. Exit codes: 0 ok · 1 usage/registry missing · 3 fail-closed.
- **Trace:** `data/typesafe_decisions.jsonl` (gitignored, append-only) — `kind: decision` record me task_id/state_hash/evidence_refs/requested_model/resolved_model/latency/raw answers/decision/downstream_action/`outcome: null`; baad me `kind: outcome` record `task_id` se judta hai. Decision record **rewrite nahi hota** (WORM-ish). Key value trace me kabhi nahi jaati (test se pinned).
