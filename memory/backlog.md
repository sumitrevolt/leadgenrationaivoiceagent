...
[2026-07-06] **Public-page perf quick wins (web-performance skill baseline)** — homepage HTML 79KB UNCOMPRESSED (content-encoding absent despite gzip/br Accept; `server: uvicorn` — Caddy `encode zstd gzip` directive missing?) + TTFB 1.46s vs /pricing 0.86s (homepage handler heavier?); mobile-4G conversion lever | Caddyfile `encode` + reload (infra change = user-auth), homepage hot-path profile, Lighthouse/PSI mobile baseline (anonymous quota 429 tha).
