...
[2026-07-07] **RL flywheel signal lopsided — ARCHIVED 2026-09-05 after evidence re-triage** — prod probe: 3860 rewards (funnel 3856, outreach 4, voice 0). Voice-0 ROOT CAUSE: live `vobiz_stream._auto_qualify` + legacy `call_manager` qual writers had NO reward hook (only post_call_hooks did) → FIXED (ref=call_id dedupe; regression tests pin all writers). Outreach low = channel_experiments rarely fires (usage, not wiring). Thompson Phase-1 note stands: voice arms need ~200 samples post-deploy; smartflo-path qualification missing (new gap); funnel self_improve cadence stalled since 08-25 (separate observation).
