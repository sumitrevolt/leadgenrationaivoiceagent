...
[2026-07-07] **RL flywheel signal lopsided — ARCHIVED 2026-09-05/06 after evidence re-triage** — prod probe: 3860 rewards (funnel 3856, outreach 4, voice 0). Voice-0 ROOT CAUSE: live `vobiz_stream._auto_qualify` + legacy `call_manager` qual writers had NO reward hook (only post_call_hooks did) → FIXED (ref=call_id dedupe; regression tests pin all writers). Outreach low = channel_experiments rarely fires (usage, not wiring). **Funnel "stall" since 08-25 = NOT a bug (2026-09-06 probe): self-improve loop ALIVE (`last_tick` today, 374 revives/48h) but `status=approval_pending` — governance gate awaiting OWNER approval (agents UNARMED, rollout held); funnel rewards resume only on approval.** Thompson Phase-1 note stands: voice arms need ~200 samples post-deploy; smartflo-path qualification missing (new gap).
...
[2026-09-06] **OmniRoute seed script `DB_PATH` default footgun** — `scripts/seed_omniroute_14combos.py` default `DB_PATH="/root/.omniroute/storage.sqlite"`, but 2026-09-05 handoff/DIRECT_HOST evidence says the LIVE gateway DB is actually at `/app/data/storage.sqlite` (old seeds targeted `/root/.omniroute/` = "wrong path, never landed"). Compose mounts `omniroute_data:/root/.omniroute` (holds provider/config state) but storage.sqlite lives under `/app/data`. RISK: running `seed_omniroute_14combos.py` WITHOUT `OMNIROUTE_DB_PATH=/app/data/storage.sqlite` silently seeds a DB the gateway ignores → "14 combos seeded OK" but gateway still serves stale/67-combo state. **Why not auto-fixed:** container runtime not verifiable from this sandbox (no docker / py3.12 / pytest); also the script's destructive `DELETE FROM combos WHERE ... NOT IN (...)` is backup-claiming but `run_seed()` has NO actual file-backup step (comment-only). **Do before next seed:** (a) confirm live path via `docker exec leadgen_omniroute ls -la /app/data/*.sqlite /root/.omniroute/*.sqlite` (b) export `OMNIROUTE_DB_PATH` to the live path (c) add a real non-destructive fs copy backup step before the DELETE. unblocks: safe OmniRoute reseeding + combo verify.

[2026-09-06] **telephony_readiness `compliance_flags` dekhta HAI GALAT flag (false-alarm + missed-risk)** — `app/telephony/telephony_readiness.py` L192-197: `os.environ.get("WHATSAPP_AUTO_SEND","0") != "1"` scores "ban-risky auto-send OFF". Problem: `WHATSAPP_AUTO_SEND` (with POST_CALL_WHATSAPP/VOICE_CLOSE_WHATSAPP) is the **owner-armed Swara-interested post-call WA inside an established conversation** — legit, not cold bulk. Prod has all three =1 (per CURRENT_STATE §Calling posture), so readiness falsely loses 5 points on an authorized feature. Meanwhile the ACTUAL cold/bulk ban-risk flag `SALES_AUTOPILOT_WHATSAPP_ENABLED` (=0 in prod, the real invariant in CLAUDE.md §5 ban-safety) is **NOT checked at all** — so true risk goes undetected while a safe path is penalized. **Why not auto-fixed:** affects a compliance-posture metric; changing it needs the telephony readiness tests run (py3.12+pytest unavailable in this sandbox); §6 DoD requires green tests before "done". **Do:** switch check to `SALES_AUTOPILOT_WHATSAPP_ENABLED != "1"` (weight 5), keep wording "cold/bulk auto-send OFF", update `tests/test_telephony_readiness_run_checks.py`, run targeted tests. unblocks: honest readiness score + correct ban-risk signal.
