...
**ADR-188 - FASTAPI_MCP_TOKEN = admin service-JWT + Hermes Desktop MCP wiring (2026-08-23).** Owner ask: "Hermes Desktop GUI me sab dikhe, work as admin" (₹5L/7d sprint cockpit). **Gap found (R1 evidence):** ops tools (ADR-186) /mcp pe exposed the par externally UNCALLEABLE — auth double-layer conflict: /mcp middleware Bearer=FASTAPI_MCP_TOKEN maangta hai, inner `require_admin` user-JWT maangta hai; ek hi Authorization header dono ko satisfy nahi kar sakta ("Invalid token: Not enough segments" 401 probe-se proven). fastapi-mcp 0.4.0 `_forward_headers=["authorization"]` default se inner call ko wahi header forward hota hai. **Fix (zero code change):** VPS pe app-container ke andar `create_access_token()` se owner super_admin (`73c80fb3…`) ka 1825-din service-JWT mint kiya, `.env` me FASTAPI_MCP_TOKEN replace kiya (backup `.env.bak-mcpgw-20260823_065853`), app-only recreate SAME image `5919c379` pe (no code deploy). Ab ek token = middleware gate ✓ + require_admin ✓ (auditable identity). **Verify:** REST `/api/ops/revenue-summary` 200 + `/api/ops/hotqueue` 200 via Caddy; `/health` version=`5919c379` environment=production; Hermes gateway restart ke baad log: `MCP server 'leadgen' (HTTP): registered 54 tool(s)` (SSE transport, headers auth). Hermes `config.yaml` mcp_servers me `leadgen:` block added (url=https://leadsgenai.in/mcp, transport=sse, headers.Authorization=Bearer <JWT>; backup `config.yaml.bak-leadgenmcp-20260823_123327`). **Caveats:** (1) owner password-reset → `revoke_all_for_user` epoch is JWT ko maar dega → re-mint zaroori (procedure isi ADR me); (2) JWT = full-admin equivalent, .env ke blast-radius ke barabar; (3) purana random-token consumers koi nahi tha (grep clean); (4) Hermes one-shot `-z` LLM-turn timeout aaya (OmniRoute/Groq provider-side) — MCP wiring ka issue NAHI. **Re-mint runbook:** container me `python -c "from app.utils.auth import create_access_token; ..."` → sed .env line → `APP_VERSION=<running-tag> docker compose -f docker-compose.vps.yml up -d --no-deps app` → Hermes config.yaml header update → gateway restart.

## ADR-189 (2026-08-26) ? WSL removed from OmniRoute/dev lane; gateway + Redis moved to Docker [LOCAL, dev-only]

**Context:** Owner asked to remove the WSL dependency for local dev ("wsl hata do"), since Docker is present. The only WSL usage was the optional OmniRoute free-LLM gateway (WSL Ubuntu-24.04 + tmux + nvm Node 22) + a WSL Redis broker. OmniRoute now runs as a Docker container; the Hermes Desktop app should bring it up on open.

**Decision:** Move OmniRoute to a Docker container (`deploy/compose/docker-compose.omniroute.yml` + `Dockerfile.omniroute`; Node 22, pinned `omniroute@3.8.46`, volume `omniroute_data` -> `/root/.omniroute`, loopback 20128/20129). Redis broker -> `leadgen_redis` container (`redis:7-alpine`, loopback 6379). Rewrote all WSL/tmux launchers to Docker: `start-omniroute.ps1`, `omniroute-check.ps1`, `start-leadgen-dev.ps1`, `_leadgen_dev_up.sh`, `omniroute-tmux.sh`, `omniroute_ensure_running.sh`, `omniroute-healthguard.sh`, `omniroute_debug_capture.sh`. New `scripts/start-hermes-omniroute.ps1` ensures the gateway is up, then launches Hermes Desktop. Contract tests pin the Docker design.

**Memory (RAM) fix:** the old hard-coded heap (2048 -> 4096 after a 2026-08-23 HTTP 503 under heap pressure) is now adaptive. The launcher auto-sizes `OMNIROUTE_MEMORY_MB` to ~half of host RAM, clamped `[1024, 4096]`, and sets a container `mem_limit` (`OMNIROUTE_MEM_LIMIT_MB` = heap*2, a safe backstop) so a runaway gateway cannot starve Docker Desktop and GC pressure is never an OOM tripwire; respects an explicit override. `.env.example` documents both.

**Security unchanged:** gateway loopback-only (0.0.0.0 never exposed); OmniRoute stays absent from the VPS stack (ADR-111). Graceful degrade -> `app/voice_agent/free_ai.py` fallback; no customer/revenue path depends on OmniRoute.

**Verification:** `docker compose -f deploy/compose/docker-compose.omniroute.yml config --quiet` EXIT=0; PowerShell AST parse OK (4 .ps1); 30 WSL-removal marker assertions PASS. Full pytest could NOT run in this session (the sandbox denies pytest tmpdir `os.listdir` on the basetemp dir — environment-only, not a code failure). Owner first-run action: build/start, then re-establish provider/OAuth in the `omniroute_data` volume (or seed from the old WSL `/root/.omniroute`), then `omniroute doctor`.

**Rejected/NOT done:** keeping WSL as a dependency; exposing 20128 beyond loopback; touching the VPS stack / `.env`; commit/push/deploy (user gate). Rollback = `docker compose ... down` + unset local endpoint; a hard WSL-return requires git-history restore.

## ADR-OWNER-1 (2026-08-27) — Daily Hot-Queue Owner Action Pack (9 AM IST CSV+MD+nfty)

**Decision:** New scheduled job `hot_queue_owner_pack` runs **daily 09:00 IST** and turns `reply_agent.hot_queue(scope="boss")` into `data/hot_queue_for_owner_<date>.csv` + `.md`, then pushes a high-priority ntfy to the owner topic (`leadgen-d6b984bd`). The CSV has the wa.me link column with the pre-written UPI deep-link embedded. The MD is the top-15 with clickable wa.me + draft preview.

**Context:** Owner Hot Queue had 42 `calling_flagged` / `interested` cards un-actioned (₹84K potential at 100% close, ₹10-20K realistic). The existing `hot_queue_brief` (8:15 AM, narrative audio) already mentioned the count but didn't make each card a single tap-to-send. `/app/inbox` UI exists but the owner reads on phone — 1-click wa.me is the natural close surface.

**Alternatives rejected:**
- (a) Auto-send cold WA from `calling_flagged` → blocked by `SALES_AUTOPILOT_WHATSAPP_ENABLED=0` (ban-safety, owner mandate).
- (b) `inbox.html` UI rebuild — high effort, owner still needs to open it.
- (c) Email digest — owner reads ntfy push; email gets buried.
...
- The canonical decision path is `scripts/typesafe_admin_triage.py`: findings registry → ONE System One request with independent Noul/Score/Choice judgments → ranking + chosen next action → append-only trace in `data/typesafe_decisions.jsonl` (`--record-outcome` closes the loop). Fail-closed: INERT/failed request writes NO decision.
