...
Remaining honest limits after the second pass: (a) the image was built FROM `ghcr.io/…:latest` because **no image exists for the PR sha** — CI publishes on merge, so same-sha coupling is enforced by the compose overlay + the deploy skew check and is verified statically, not by this build; (b) **RESOLVED 2026-08-01 closure pass** — the 3 approval-by-content-hash suites (`test_video_approval_saga`, `test_video_approval_principal`, `test_video_preview_identity::test_matching_hash_approves`) now run **68 passed / 0 failed**. Earlier they failed identically at the pre-change commit `ddc6442` while passing in CI, so "local state, not a branch regression" was correct — but the PRECISE cause is worth recording because it will recur: those fixtures redirect `video_pipeline.output_root`, the video-ad store, approvals and the content queue to `tmp_path`, but they do **not** redirect `video_media_paths.approved_media_dir()`, which resolves to the REPO's `data/video_ads/_approved`. So every run leaks its immutable approval snapshot + `_txn` transaction records into a shared, persistent directory, and a later run replaying the same fixture creative id hits an existing terminal transaction and gets a correct 409. CI is green because a fresh runner starts with that directory empty. Clearing the leaked `_approved/_txn` state (moved aside, never deleted — no real customer artifact was touched) restored a clean pass. **No production code is implicated and no assertion was weakened**; the residual hermeticity gap is in those fixtures, not in the approval logic, and is left as a separate follow-up rather than smuggled into this video PR; (c) the in-container render is ~17× slower than host under Docker Desktop's software-rendering VM (≈35 min vs ≈2 min) — fine for a Linux worker with a real CPU allocation, but it means `CREATIVE_VIDEO_SOFT_TIME_LIMIT_S` must be re-measured on the VPS before the flag is ever enabled. Related: [[ADR-142]] video decisions, [[ADR-097]] `:latest` provenance.
