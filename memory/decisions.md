...
**Consequence:** ⚠️ **jiya has ZERO consented visual assets registered**, so the canary contains no customer photography — it is brand-accurate (real name/colours/tagline/city from `clients_store`) but photo-free, and the template renders a designed typographic panel rather than a grey placeholder or an unlicensed stock photo. Do NOT describe it as a full photo canary. The tenant also has no `services[].price_inr`, so ANY price in copy is refused by the existing `brief.unverified_prices` gate — the canary copy therefore quotes no figure. Arming preconditions deliberately NOT met: `CREATIVE_OS_ENABLED` is still unset in prod (so the whole OS, not just this provider, is inert), `CREATIVE_PROVIDER_HYPERFRAMES_ENABLED=0`, and `CREATIVE_HYPERFRAMES_CANARY_TENANTS` empty = **no tenant** (fail-closed; empty deliberately does not mean "everyone"). ~~`Dockerfile.video` was authored and its compose merge validated, but the image was never built.~~ **SUPERSEDED by the 2026-08-01 update above — image built, hadolint clean, in-container render proven.** ~~Templates 2 and 3 are NOT built.~~ **SUPERSEDED — both shipped.**

Remaining honest limits after the second pass: (a) the image was built FROM `ghcr.io/…:latest` because **no image exists for the PR sha** — CI publishes on merge, so same-sha coupling is enforced by the compose overlay + the deploy skew check and is verified statically, not by this build; (b) **RESOLVED 2026-08-01 closure pass — and the cause was NOT what I first wrote.** The 3 approval-by-content-hash suites (`test_video_approval_saga`, `test_video_approval_principal`, `test_video_preview_identity::test_matching_hash_approves`) now run **68 passed / 0 failed**, twice in a row, in a disposable clean-room worktree at the frozen head.

**Actual cause: a missing method on the in-memory Redis fallback.** `app/api/customer_dashboard.py:2633` checks token revocation with `await redis_client.exists(...)`, and the customer-auth path is what these approval routes authenticate through. `InMemoryCache` — the fallback used whenever no Redis is running, i.e. on this dev box — had **no `exists()`**, so the call raised and the request failed with the confusing 409 / `KeyError: 'approval_txn_state'`. **PR #205 added `InMemoryCache.exists()`** (`app/cache/__init__.py`); merging current `origin/main` into this branch is what fixed the suites. CI never saw it because CI runs a real Redis (`harness real-redis integration`). This is a genuine environment dependency, fully explaining failure at `ddc6442`, the pass after the sync, and CI being green throughout.

**Retracted:** an earlier revision of this ADR attributed the failures to those fixtures leaking approval snapshots into the repo's `data/video_ads/_approved`. The LEAK IS REAL and confirmed (a clean-room run does create `_approved/_txn` + `fixture-tenant-p` there), but it is **not** the cause — two back-to-back clean-room runs both passed, so the "a later run hits a stale terminal transaction and 409s" step was an unproven causal claim, exactly the failure mode §CAUSAL-CLAIM DISCIPLINE warns about. Recorded as a separate hermeticity follow-up, not as this defect. **No production code was changed for this and no hash/approval assertion was weakened**; (c) the in-container render is ~17× slower than host under Docker Desktop's software-rendering VM (≈35 min vs ≈2 min) — fine for a Linux worker with a real CPU allocation, but it means `CREATIVE_VIDEO_SOFT_TIME_LIMIT_S` must be re-measured on the VPS before the flag is ever enabled. Related: [[ADR-142]] video decisions, [[ADR-097]] `:latest` provenance.
