...
- **platform_dial = TEST-MODE (RE-ENABLED by owner 2026-07-31)** — original USER-MANDATE (2026-07-05) "HARD OFF" ko Sumit ne hi re-enabled kiya after V2 score go-live: `data/platform_dial.json enabled:true limit:10` + test-mode allowlist + bot/IVR detection active + bounded cap 10/day (note in data file). `PLATFORM_DIAL_DAILY=10`. Koi live call nahi hua (no_call_log). Re-enable condition (go-ahead + allowlist + bot/IVR) met thi. Calling window + DND gates call path me abhi bhi ACTIVE.
...
- DLT cold-outbound: **APPROVED (user-confirmed 2026-07-14)** — `DLT_APPROVED=1` prod me correct hai. Cold outbound `platform_dial` TEST-MODE (cap 10/day, allowlist) se gated hai (§5), DLT se nahi.
...
**Ops facts (hot):** **Prod `/health`=`48f0577`** (2026-08-01 deploy via PR #207, canonical deploy_vps.sh; direct HTTPS probe, never browser). `origin/main` = `48f0577` (EQUAL to prod). All 5 app-image services on `48f0577` — **no per-container skew** (re-probed 2026-08-01). Rollback ref = `48f0577` until the next deploy · **sales autopilot EXECUTING hourly, REAL** (owner mandate 2026-08-01 "sab on karo"): `SALES_AUTOPILOT_ENABLED=1` · `SALES_AUTOPILOT_DRY_RUN=0` · `SALES_AUTOPILOT_EMAIL_ENABLED=1` · `SALES_AUTOPILOT_WHATSAPP_ENABLED=0` · `WHATSAPP_AUTO_SEND=0` · **Boss autonomy**: `OPENCLAW_REQUIRE_APPROVAL_FOR_AMBER=0` (prod still GREEN-only structural — Stage A strip in `allowed_commands()`; RED catalogue always refused) · scheduler routes to email channel when WhatsApp off (PR #207 `_primary_channel`) · last tick 2026-08-01T14:55Z `dry_run:false` processed 0 (single prospect `converted`) · WhatsApp stays 1-click human, dial test-mode cap 10 (both legal/ban gates — do NOT flip) · **platform_dial TEST-MODE**: `PLATFORM_DIAL_DAILY=10`, `data/platform_dial.json enabled:true limit:10` (owner re-enabled 2026-07-31, allowlist + bot/IVR detection; NO live call fired) · **CREATIVE_OS_ENABLED=1** (was "OFF" in memory — probe 2026-08-01; state file absent = likely inert, confirm intent) · **31 agents** · calling HARD OFF except test-mode dial · self-improve loop ALIVE (120/day cap, hit daily) · DLQ=0, celery queue=0 · revenue snapshot MRR ₹1,999 / 1 active / 0 churn.
