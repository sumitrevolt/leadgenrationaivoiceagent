...
- 2026-07-23: **ADR-142 VIDEO DECISIONS** — Reject is terminal/no-regeneration, only Changes revises, exact-version approval refuses stale terminal ledgers, and design-system assets bypass stale SW cache. Local contract/browser-proven, not deployed.
- 2026-07-23: **ADR-141 VIDEO REVIEW** — media is tenant/path/version bound; Stage-3 cohort requires explicit customer allowlist; Chart.js is locally pinned. Local browser-proven, not deployed.
...
**Next action:** Owner-authorized review/commit/push/deploy of `codex/video-review-stage3`; then exact-SHA parity + one authenticated read-only Jiya Preview canary with only customer-review + Jiya allowlist enabled. Keep WhatsApp/publish/scheduler OFF. Parallel: GTM Hot Queue → 2nd paying customer. Detail = `docs/context/ACTIVE_WORK.md`.

**Ops facts (hot):** **Prod `/health`=`c7d5fa69`** (re-probe before future claims) · all five app-image containers matched that SHA at last live proof · queues celery=0/failed=0/dead=0/resolved=9 · customer review/WhatsApp review/social publish/video scheduler OFF · `PLATFORM_DIAL_DAILY=0` · shared context = `docs/context/` · Swara/voice FROZEN this wave.
