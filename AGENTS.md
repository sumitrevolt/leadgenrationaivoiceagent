...
- 2026-07-07: **ADR-032 IA nav cleanup, narrow scope — Customer Delivery OS MISSION COMPLETE (6/6 sub-projects).** Discovery found far more admin-nav sprawl than the backlog note implied (13 orphaned pages, 5-way/3-way/3-way overlapping-cockpit clusters) — `advisor()` said go narrow. Shipped only 3 additive fixes: disambiguated the two Command Centers' colliding `<title>`+badge, added a direct `/app/clients` nav link, regrouped `agent-tools.html` under new "Advanced" section as "(Dev)". Sprawl backlogged with evidence, not fixed.
...
**Next action:** ADR-034 admin IA batch DONE + DEPLOYED (`6ccec85` live at leadsgenai.in — 6 nav-groups, 7 dup cockpits demoted, 6 orphans surfaced, command-center dup merged→deleted 47→46 pages). Nothing IA-related pending. Open items: 6 HIDE orphan pages (agents/journeys/voice-keys/conversations/minisite-builder/dialer) kept reachable-by-URL — long-term fate TBD; optional customer Reports/Billing explicit nav; hot lead **+917498797259** 1-click WhatsApp still pending (mid-funnel money moment); deliverability ~2000-email/0-reply root is ops-level (separate sprint). Note: local `main` ref stale (deploy used origin/main correctly); VPS + origin/main both at `6ccec85`.
