...
- 2026-07-12: **Local dev setups — OmniRoute + Automations + Unity WebGL DONE.** OmniRoute doctor 7-OK/0-fail, Node v22.23.1, tmux gateway+3 lanes, WS 20129 loopback; Unity WebGL built/deployed. Reproducible bring-up: `scripts/start-leadgen-dev.ps1`.
...
**Next action:** (1) Human-work the 3 qualified Hot Queue replies (no autonomous send/call) (2) Capture Jiya's verified real email and run one allowlisted approval reminder — **ye AKELA launch blocker bacha hai** (3) `HOT_QUEUE_BRIEF_DAILY` prod me `=1` hai par CLAUDE.md `OFF` keh raha tha — decide: intentional rakho ya `=0` karo. Chrome smoke, WAHA connection+webhook port, Hot Queue noise, Explorer sync, DLT, monitoring alert-fatigue, disk = RESOLVED.

**Ops facts (hot):** Company inboxes = `admin@leadsgenai.in`, `sunny@leadsgenai.in` only · `SOCIAL_PREFS_HONOR` default OFF (draft prefs dormant until operator flip) · Scheduler = Celery durable (`RUN_IN_PROCESS_SCHEDULER=0`, rollback = `=1`+`WEB_CONCURRENCY=1`) · 25 staff jobs (incl. new `hot_queue_brief` 08:15 IST, dormant flag OFF) + dead-man trio alive · WhatsApp auto-send OFF (`WHATSAPP_AUTO_SEND=0`, `VOICE_CLOSE_WHATSAPP=0`; 1-click only) · UPI pay-info ARMED + `/api/upi/submit` LIVE (route POST-only — GET pe 404 normal hai, `POST`+auth = 401 correct) · **`HOT_QUEUE_BRIEF_DAILY=1` prod me LIVE** (app+worker dono; 2026-07-14 audit ne pakda — pehle doc `unset/OFF` keh raha tha) · `VOICE_GEMINI_PRIMARY=0` prod me (doc `=1` keh raha tha — §2 stale) · NOTIFY_EMAIL set · Sentry ARMED · offsite backup LIVE (restore proven) · MCP `/mcp` gated (`FASTAPI_MCP_TOKEN`/allowlist) · 250 skills in skill_pack · Slash commands: `/verify` `/ship` `/checkpoint` `/learn` `/compact-check` `/optimize` `/test-expand`.
