...
- 2026-07-06: **05-Jul call-batch learning → 4 voice upgrades SHIPPED (UNCOMMITTED, deploy pending — ADR-025):** (1) `_IVR_PATTERNS` widened + vobiz `_is_ivr_prompt` shared-RE; (2) in-call IVR strike→hangup (`IVR_HANGUP` default ON, `IVR_MAX_HITS=2`) — 167s IVR-discovery paisa-burn band; (3) `_is_junk` Whisper-hallucination repetition filter; (4) `ACK_TRIAL_CLOSE` default ON: value-statement + bare-ack ("Okay") → trial-close ask + dialed-path post-close `_on_close_signal` parity. Tests 34 new + 79 neighbour green. **HOT LEAD from good call f452cce6: +917498797259** (real human, agency-user) — call_log corrected (interested/hot/verified_human), deal `3e3ea3eb73eb` interested; owner ko wa.me 1-click draft diya (WA auto-send OFF hi hai). Deploy + `NOINPUT_POLICY=1` (0-turn dead-air fix) = USER go-ahead pending.
- 2026-07-05: **WhatsApp auto-send HARD OFF in prod** (`WHATSAPP_AUTO_SEND=0` + `VOICE_CLOSE_WHATSAPP=0`; 1-click human send only — ban-safety). Full postmortem → `docs/SESSION_LOG.md` (2026-07-06 archive).
- 2026-07-05: **platform_dial HARD OFF + dial_gate LIVE** (`ea36df4`) — promotional = allowlist-only fail-closed + bot/IVR + min-user-turns qualify gate; re-enable = user go-ahead only. Office HQ honesty + outreach hardening `9e18ef3` DEPLOYED. Detail → ADR-019/ADR-024 (`memory/decisions.md`) + `docs/SESSION_LOG.md` (2026-07-06).
