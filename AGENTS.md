...
- 2026-07-07: **ADR-024 (supersedes ADR-023) Customer Delivery Ledger reconciled onto the REAL `app/marketing/delivery_ledger.py`** — mid-build discovered `tmp-deploy-main` (checked out at main repo root, 140 files ahead of `main`) already had a more mature, reviewed jsonl-based ledger module (same 14 events, never wired to any call site). Dropped this session's own Postgres `delivery_events` table/model, re-pointed all 8 wiring call sites + admin `deliver-now`/`timeline` + customer `timeline` + all 3 dashboard-fork UI cards at the real module. SHIPPED LOCALLY (13 commits on `claude/priceless-shirley-4a2f83`, NOT pushed/deployed/merged — `tmp-deploy-main` itself untouched). 15/15 new + 29/29 regression tests green, `prod_check.py` PASS. Spec/plan `docs/superpowers/specs|plans/2026-07-06-customer-delivery-ledger*`; sub-projects 2-6 in `memory/backlog.md`.
