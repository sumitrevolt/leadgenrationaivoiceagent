...
- 2026-07-12: **Health score SLA fix — gate-vs-failure split + 24h rolling window (`17d305c`).** `_record_stuck` now logs `delivery_gated` for gate reasons (auto_delivery_off/sweep_auto_off/no_phone), `automation_failed` only for real failures. `failed_count` uses 24h rolling window (`failures_24h`) instead of all-time. Jiya-makeover score 29→64 within 24h (RED flag clears as old events age out). 10 new tests, all gates passed, deployed + verified.
- 2026-07-12: **Production cleanup — repo hygiene + deploy `acc3e8d`.** Committed 53 pending Unity scaffold files, deleted 27+19 stale branches, cleared stashes. Single `main` branch, clean tree.
- 2026-07-12: **Local dev setups (cowork) — OmniRoute + Automations + Unity WebGL ALL DONE.** OmniRoute doctor 7-OK/0-fail (encryption-key fix + fresh DB, gateway Node v22.23.1, tmux gateway+3 lanes, WS 20129 loopback). Redis broker up (WSL 127.0.0.1:6379); venv/prod_check PASS, 27 automation tests green, ~40 `staff-*` beat-jobs. **Unity WebGL build SUCCEEDED** (31.2MB, rc 0) + deployed to `frontend/office_unity/Build/LeadGenVirtualOffice.*` (~7.35MB compressed); prod_check routes 1099→1100 (office-unity mount active). **Real root cause was NOT antivirus** — DC-spawned process had empty `PROGRAMDATA`/`ALLUSERSPROFILE`, crashing UPM's `getLocalConfigFolder` (`path.join(undefined)`); fix = set `PROGRAMDATA=C:\ProgramData` before Unity.exe. Reproducible bring-up: `scripts/start-leadgen-dev.ps1`.
**Blockers / USER-action pending (env-unset = dormant, graceful skip):**
- Unity WebGL build (ADR-076): **RESOLVED 2026-07-12** — build SUCCEEDED + deployed to `frontend/office_unity/Build/LeadGenVirtualOffice.{loader.js,data.br,framework.js.br,wasm.br}`, StaticFiles mount active (routes 1100). Real cause was NOT AV — empty `PROGRAMDATA`/`ALLUSERSPROFILE` in the DC-spawned env crashed UPM's `getLocalConfigFolder`; **fix = `PROGRAMDATA=C:\ProgramData` before launching Unity.exe** (Defender exclusion was a red herring; user may remove it). CAVEAT: `WebGLBuild.cs` emits `Build.*` nested in `Build/Build/`; deploy must flatten+rename to `LeadGenVirtualOffice.*` (currently done manually — codify before next rebuild). 3D office still gated by `UNITY_VIRTUAL_OFFICE_ENABLED` (/app/office?mode=3d); real-browser smoke pending. `[MenuItem("LeadGen/Build WebGL")]` in WebGLBuild.cs (uncommitted).
...
**Next action:** (1) Customer notification for 18 pending approvals (email/dashboard banner — customer can't approve what they don't know exists) (2) WAHA QR scan (user-action) to fix WhatsApp send path (3) secret log-redaction + affected-key rotation (P0) (4) GTM sprint — Hot Queue / dialer for mid-funnel conversion. `HOT_QUEUE_BRIEF_DAILY`/`CELERY_VIDEO_QUEUE` remain OFF.

**Ops facts (hot):** Company inboxes = `admin@leadsgenai.in`, `sunny@leadsgenai.in` only · `SOCIAL_PREFS_HONOR` default OFF (draft prefs dormant until operator flip) · Scheduler = Celery durable (`RUN_IN_PROCESS_SCHEDULER=0`, rollback = `=1`+`WEB_CONCURRENCY=1`) · 25 staff jobs (incl. new `hot_queue_brief` 08:15 IST, dormant flag OFF) + dead-man trio alive · WhatsApp auto-send OFF (`WHATSAPP_AUTO_SEND=0`, `VOICE_CLOSE_WHATSAPP=0`; 1-click only) · UPI pay-info ARMED + `/api/upi/submit` LIVE (401 with auth = correct, 404 fixed 2026-07-10 `4aaf8040`) · `HOT_QUEUE_BRIEF_DAILY` unset in prod (OFF — worker/scheduler ab new image pe, flip-ready on explicit user go) · NOTIFY_EMAIL set · Sentry ARMED · offsite backup LIVE (restore proven) · MCP `/mcp` gated (`FASTAPI_MCP_TOKEN`/allowlist) · 250 skills in skill_pack · Slash commands: `/verify` `/ship` `/checkpoint` `/learn` `/compact-check` `/optimize` `/test-expand`.
