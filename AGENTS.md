...
- 2026-07-09: **ADR-058 Outreach pending recipients now have operator-review buckets (visibility only, no auto-block).** After ADR-057, remaining 130 were mixed-fit; broad category blocking was unsafe. Added `pending_review` buckets to `outreach_activity()` and rendered them in Admin Email Activity. Live buckets: 71 `Priority: local SMB`, 48 `Review: enterprise/edge`, 6 `Review: unknown fit`, 5 `Review: vendor/competitor`. Gate: 23 targeted tests green, ruff clean, admin JS syntax OK, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`2b6d476`): queue 0, `/app/admin` marker OK, public `/health`=`environment:production`, no fresh real errors.
...
**Next action:** No verified uncommitted production-ready batch is pending in main; latest live code commit is `40db6be` (Hot Queue historic noise guard). Dirty Claude worktrees still exist and must be audited one-by-one; do not merge blindly. Keep `EMAIL_WARMUP` paused; do not resume from `/api/growth/outreach/warmup/resume` until complaint-source/list-quality cleanup is done. Next high-leverage action is human/operator review of the exported 71 priority local SMBs, then one-by-one outreach only.
