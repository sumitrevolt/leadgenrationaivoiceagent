...
- 2026-07-09: **ADR-061 Priority outreach review decisions are now audit-only operator bookmarks.** Completed the half-pending review-decision batch: append-only `data/outreach_review_decisions.jsonl`, latest-state counts, admin-only `POST /api/platform/team/outreach-review-decision`, `GET /api/platform/team/outreach-review-decisions`, counts endpoint, and Admin Email Activity decision panel. It records reviewed/sent/skip/schedule/suppress-note decisions but does **not** send email, mutate warmup, or alter `email_unsub` suppression. Gate: 26 outreach tests green, ruff clean, admin JS syntax OK, `prod_check.py` PASS with API docs synced, secrets clean. **COMMITTED + DEPLOYED to production** (`acb66c0`): app/worker/worker-heavy/scheduler healthy, queue 0, review endpoints 401 unauthenticated, public `/health`=`environment:production`, no fresh real errors.
- 2026-07-09: **ADR-060 Hot Queue read-path now applies historic noise guard + operator blocklist.** Audited dirty `beautiful-dijkstra` worktree; most dependency/API-doc edits were stale or already superseded, but a real remaining gap existed: historic `reply_drafts.jsonl` rows bypassed `REPLY_SENDER_BLOCKLIST` and WA `status/@broadcast` read-path filtering. Added `_is_noise_row()` and wired it before Hot Queue dedupe; auto-ack filtering remains covered there too. Gate: 47 reply/hot-queue tests green, ruff clean, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`40db6be`): live Hot Queue 12, no `status/@broadcast`, no auto-ack subjects, queue 0, public `/health`=`environment:production`, no fresh real errors. `adityabirla.com` still appears as 1 deduped row because env blocklist is unset; code now supports blocking it without data mutation/env overwrite by agent.
...
**Next action:** No verified uncommitted production-ready batch is pending in main after ADR-062; latest live code commit is `026a9d7` (Social Setup Wizard prefs honored behind default-OFF flag). Dirty Claude worktrees still exist and must be audited one-by-one; do not merge blindly. Keep `EMAIL_WARMUP` paused; do not resume from `/api/growth/outreach/warmup/resume` until complaint-source/list-quality cleanup is done. Next high-leverage action is human/operator review of the exported 71 priority local SMBs, recording decisions before any one-by-one outreach.
