...
- 2026-07-06: **COUNCIL → scrape data-quality: phone-TYPE gate + self-learning DID blocklist DEPLOYED LIVE (`03dad80` — ADR-027; ADR-025 voice upgrades bhi isi batch me LIVE; backfill applied: 652 fixed-DID dial_block, backup `.bak-20260706_131706`):** prod audit ne 649 FIXED_LINE cloud-IVR DIDs "ready" pool me pakde (05-Jul junk dials SAB isi bucket se). `dial_gate.phone_quality` (`PHONE_TYPE_GATE` ON: promotional par fixed/tollfree/invalid BLOCK, mobile+FLOM pass) + NEW `call_feedback.py` (`CALL_FEEDBACK_LOOP` ON: IVR-confirm → number-block + prefix ≥3-distinct auto-block + prospect `dial_block` tag + audit jsonl = self-improving loop) + harvester `_valid_phone` is_mobile enforce + `phone_type` tag + `scripts/backfill_phone_type.py`. Tests 24 new + 85 regression green. VPS backfill --apply = deploy ke baad.
- 2026-07-06: **05-Jul call-batch learning → 4 voice upgrades SHIPPED (UNCOMMITTED, deploy pending — ADR-025):** IVR strike→hangup (`IVR_HANGUP` ON) · `_is_junk` hallucination filter · `ACK_TRIAL_CLOSE` ON (bare-ack → trial-close) · dialed-path close parity · `_IVR_PATTERNS` widened. Tests 34+79 green. **HOT LEAD f452cce6: +917498797259** (real human, agency-user) — call_log corrected, deal `3e3ea3eb73eb` interested; owner ko wa.me 1-click draft diya. Deploy + `NOINPUT_POLICY=1` = USER go-ahead pending.
- 2026-07-05: **WhatsApp auto-send HARD OFF** (`WHATSAPP_AUTO_SEND=0` + `VOICE_CLOSE_WHATSAPP=0`; 1-click only) · **platform_dial HARD OFF + dial_gate LIVE** (`ea36df4`; re-enable = user go-ahead only). Detail → ADR-019/ADR-024 + `docs/SESSION_LOG.md` (2026-07-06).
