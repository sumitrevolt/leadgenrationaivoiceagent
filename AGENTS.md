...
- 2026-07-09: **ADR-065/066 Automation-logs admin UI + social-setup ledger fix + per-customer job attribution.** Fixed `social_setup_completed` ledger event (bad `customer_visible=` kwarg → TypeError swallowed → milestone never logged; now records with idempotency key). Wired the ADR-064 DB-backed `/api/admin/automation-logs` into a NEW "Automation Runs" admin panel (status/job-type/customer/date filters) in `delivery_command_center.html` (was API-only = adhoora). `_run_job` finish-log now sets `output_summary`. Per-customer attribution: `client_report.run_monthly()` + `auto_content.run_daily_content()` write per-client `AutomationLog` rows (client_id + report-path proof) so the panel's customer filter works. Additive, no migration, no new route, nothing auto-enabled. Gate: 9/9 new tests green (related 26/27 — the 1 fail = pre-existing `test_seed_client_content`), `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`b8a5af9`, also carried pending docs `ecbfc53`): app/worker/worker-heavy/scheduler healthy, celery queue 0, `/api/admin/automation-logs` 401 unauth, internal+public `/health`=`environment:production`.
- 2026-07-09: **ADR-063 White-label client reports now include delivery-ledger proof counts.** Safely extracted the useful `funny-williamson` slice without merging its broad standalone deliverables branch. `client_report.py` now adds month-windowed `delivery` counts and `next_actions_hi`, renders an “AI team ne is mahine kya kiya” section, and preserves existing `ok/path/stats/emailed` keys plus the `weekly_report_generated` event. Gate: 34 targeted tests green, ruff clean, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`ac288e9`): app/worker/worker-heavy/scheduler healthy, queue 0, public `/health`=`environment:production`, live collector returns expected delivery keys.
- 2026-07-09: **ADR-062 Social Setup Wizard prefs are honored by draft generation only behind `SOCIAL_PREFS_HONOR=1`.** Safely extracted the useful `nervous-gould` slice without merging broad automation work. Saved cadence/channel/review prefs can now shape `run_daily_content()` drafts when the flag is enabled: `cadence=off` skips configured clients, weekly/3x-week cadences are respected, channels are stamped on draft items, and `approval_mode=review` submits to approval queue. Default remains OFF, no `SOCIAL_ENGINE`, no auto-post, no WhatsApp auto-send, no email warmup resume. Gate: 16 targeted tests green, ruff clean, customer-dashboard JS syntax OK, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`026a9d7`): app/worker/worker-heavy/scheduler healthy, queue 0, public `/health`=`environment:production`, live `_social_prefs()` default returns `{}`.
...
**Next action:** ADR-065/066 COMMITTED + DEPLOYED (`b8a5af9`) = latest live code commit. Automation Runs panel now surfaces the structured job log with a working per-customer filter; social-setup milestone records. Another session's `leads.py`/`product_one_delivery.py`/`test_api.py` still uncommitted in the main tree + dirty worktrees — audit one-by-one, never `git add -A`. Deeper-logs part 2/3 (Proof column from meta_json + retry_count threading) DONE this session. `evidence_url` proof column (migration 014) DONE + verified (full 001→014 alembic chain green locally; prod at `013`, Postgres transactional DDL). Still deferred: DLQ-sourced retry/next_retry visibility. Keep `EMAIL_WARMUP` paused. `SOCIAL_ENGINE`/Postiz still off — connect for real `post_published` proof. Operator review of the 71 priority local SMBs still pending.
