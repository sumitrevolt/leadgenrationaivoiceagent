...
- 2026-07-10: **ADR-073 Hybrid Flagship Control Plane Phases 2-6 + enterprise invariant gate, default-OFF.** Fixed a real Phase-2 bug (routing never surfaced an unconfigured flagship → gateway skip-path dead + its own pytest was failing); split into planner(`route_preview`: ideal order + honest `effective_provider`)/enforcer(gateway). Added `DevTaskUsage`+migration 016 usage/cost ledger; draft-only `runner.py`+`locks.py`+`tasks/dev_worker.py` (REVIEW-ONLY proposal, `apply_patch` REFUSES); `reconcile.py` (DB-truth lease reclaim); `deploy.py` (fail-closed human approval token, code NEVER deploys); `delivery.py` (evidence + per-customer AutomationLog + ban-safe draft notify); `/api/dev-tasks/*` lifecycle + `/app/dev-control` cockpit; `scripts/dev_control_gate.py` (8 invariants) in `prod_check.py`; 4 new gate flags. Verified on a hermetic Linux venv: 29/29 dev-control tests, Alembic 001→016, gate OK, 14-check ASGI router smoke. Owed: full `prod_check.py`/`run_tests.bat` under the project image (sandbox lacked heavy deps). All OFF by default; no commit/deploy.
- 2026-07-10: **ADR-072 Provider-pinned engineering gateway added, default-safe.** `app.dev_control.gateway.invoke()` now centralizes local-first sensitive routing, configured-provider filtering, bounded fallback, task/daily budget admission, and structured usage/cost evidence for future workers. No external flagship alias, credential, shell/worktree mutation, patch, commit, customer action, or deploy is enabled.
- 2026-07-10: **ADR-071 Hybrid Flagship Agent Control Plane Phase 1 added, default-OFF.** A DB-backed `DevTask` ledger, configuration-driven model catalog, local-first cost admission, admin `/api/dev-tasks` contract, claim/heartbeat/report lifecycle, and migration 015 were added without enabling workers, patch application, or deployment. Verification is pending a compatible project Python runtime because the current `.venv` launcher target and bundled compiled `pydantic_core` are unavailable.
- 2026-07-09: **ADR-070 Legacy `LLMBrain` now falls through to `free_ai` for free-provider model names.** After live `DEFAULT_LLM=mistral-small-latest`, legacy `LLMBrain()` call-sites could raise `Unknown LLM model`; `IntentDetector` now uses `free_ai.chat` directly and `LLMBrain` maps unknown/free model names to `provider="free_ai"` for `_generate()`/`_generate_chat()`. OpenAI/Claude/Gemini/local explicit paths unchanged. Gate: 51 focused AI/voice tests green, `prod_check.py` PASS, secrets clean; VPS app/worker/worker-heavy/scheduler healthy; monkeypatched prod diag returned `free_ai`, fresh logs had no `Unknown LLM model`/Claude/Anthropic hits.
...
**Next action:** ADR-073 dev-control Phases 2-6 + gate landed UNCOMMITTED in main tree (new files `app/dev_control/{usage,runner,locks,reconcile,deploy,delivery}.py`, `app/models/dev_usage.py`, `alembic/versions/016_add_dev_task_usage.py`, `app/tasks/dev_worker.py`, `frontend/dev_control.html`, `scripts/dev_control_{gate,status}.py`, 7 `tests/test_dev_*.py`; edits to `registry.py`/`dev_tasks.py`/`models/__init__.py`/`worker.py`/`main.py`/`automation_flags.py`/`prod_check.py`). Verified on a hermetic Linux venv (29/29 tests, Alembic 001→016, gate OK, ASGI smoke) — but OWED before any deploy: full `prod_check.py` + `run_tests.bat` + `check_secrets.py` under the project's Python 3.11/3.12 image (`.venv` still broken; sandbox lacked heavy whisper/onnx/qdrant deps). Stale-mount lesson reconfirmed: author code via a sandbox-native path, Windows tools are truth. Everything OFF by default (`DEV_ORCHESTRATOR`/`DEV_WORKER_ENABLED`/`DEV_DEPLOY_APPROVAL_TOKEN` unset). Do NOT `git add -A` (other sessions' `leads.py`/`product_one_delivery.py`/`test_api.py` still dirty). Keep `EMAIL_WARMUP` paused.
