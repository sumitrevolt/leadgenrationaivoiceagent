...
**Next action:** ADR-065/066 COMMITTED + DEPLOYED (`b8a5af9`) = latest live code commit. Automation Runs panel now surfaces the structured job log with a working per-customer filter; social-setup milestone records. Another session's `leads.py`/`product_one_delivery.py`/`test_api.py` still uncommitted in the main tree + dirty worktrees — audit one-by-one, never `git add -A`. Deeper-logs part 2/3 (Proof column from meta_json + retry_count threading) DONE this session. Still deferred: `evidence_url` DB column (migration 014) — prod `DB_CREATE_ALL=0` so it needs a separate vetted in-container-alembic batch; plus DLQ-sourced retry/next_retry visibility. Keep `EMAIL_WARMUP` paused. `SOCIAL_ENGINE`/Postiz still off — connect for real `post_published` proof. Operator review of the 71 priority local SMBs still pending.
