...
- 2026-07-09: **ADR-053 Hot Queue now hides uncontactable WhatsApp IDs and saved auto-ack replies; deliverability auto-pause is live.** Live audit after ADR-052 found top Hot Queue rows were Meta/status numeric IDs, not real phones, and old auto-ack emails still appeared as "interested." Shipped stricter Indian-mobile normalization (`10`, `91+10`, `0+10` only), removed WA Reply action for invalid IDs, hid invalid WhatsApp rows from `hot_queue()`, and filtered saved auto-ack rows on read. Gate: 18 hot-queue tests green, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`1b54e43`, `04631af`, `dd8c118`): live queue 20→12, bad WhatsApp rows 0, auto-ack "Thank you..." rows 0, `/app/inbox` 200, queue 0, app/worker/worker-heavy/scheduler healthy, public/internal `/health`=`environment:production`. Also found `EMAIL_WARMUP` is currently **paused**: complaint rate 0.449% (7/1559) >= 0.25%; do not resume or increase outreach until list quality/suppression is cleaned.
...
**Next action:** No verified uncommitted production-ready batch is pending; latest live code commit is `dd8c118` (Hot Queue contactability/auto-ack cleanup). Stale/dirty Claude worktrees still exist, but council decision remains **do not merge blindly**. DB read-path migration should wait until `db_audit.read_path_ready` is clean on real customers. Highest-leverage GTM work is now **deliverability recovery/list quality** before more email volume: `EMAIL_WARMUP` paused itself on complaint rate 0.449% (7/1559 in 7d). Do not resume from `/api/growth/outreach/warmup/resume` until suppression/list cleanup is done. `/app/inbox` Hot Queue is cleaner (12 rows, bad WhatsApp 0, auto-ack 0) but remaining rows are mostly vendor/internal ops replies, not clear buyer leads.
