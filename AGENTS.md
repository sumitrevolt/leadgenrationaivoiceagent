...
- 2026-07-19: **ADR-125 LIVE `716bed84`** — `auto_content.generate_poster_pack(target=4)` branded_posters top-up (real SVG, self-guarding, wired into `generate_content`). Jiya poster 2→4, branded_posters=done, **pct 80→90** (both ids). `proof` = honest last 10% (needs real publish — blocked, NOT faked). Also live: dbc4c86 (identity extension → billing/dashboard/studio) `820e36ea`.
- 2026-07-19: **ADR-124 LIVE `ca98ece4`** — GBP + review-reply generators (`generate_gbp_pack`/`generate_review_reply_pack`, wired into `generate_content`). Jiya gbp+review=done, pct 60→80.
- 2026-07-19: **ADR-123 LIVE `670f5793`** — Jiya client-identity split-brain fix: portal + `customer_delivery_status` billing/login id → marketing id canonicalize (billing/invoice RAW). Portal 10%→60%. (Older → `memory/decisions.md`.)
