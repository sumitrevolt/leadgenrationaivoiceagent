...
- 2026-07-20: **ADR-OPENCLAW-OWNER-COPILOT** — OpenClaw = Owner Copilot edge only; Owner OS sole authority; `OPENCLAW_ENABLED=0` default; Stage A read-only.
...
**Next action:** (0) OpenClaw Copilot — commit/PR `feat/openclaw-owner-copilot` when asked; prod Stage A only after deploy auth. (1) Jiya `proof` last 10% HONEST-blocked (Meta/approvals). (2) GTM Hot Queue → 2nd paying customer. Detail = `docs/context/ACTIVE_WORK.md`.

**Ops facts (hot):** **Prod `/health`=`ef5e8b4b`** (re-probe before claiming) · **`OPENCLAW_ENABLED` unset/0 on prod** · **`AGENT_RUNTIME=1` CANARY** (rollback=unset + recreate) · shared context = `docs/context/` · Company inboxes = `admin@leadsgenai.in`, `sunny@leadsgenai.in` only · `reply_auto_send=enabled_all` in Redis · `SOCIAL_PREFS_HONOR=1` · Scheduler Celery durable (`RUN_IN_PROCESS_SCHEDULER=0`) · WhatsApp auto-send OFF · **`HOT_QUEUE_BRIEF_DAILY=1`** · Swara/voice FROZEN this wave.
