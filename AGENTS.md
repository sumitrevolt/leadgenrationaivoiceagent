...
- 2026-07-10: **ADR-074 Health-gated Hot Queue Revenue Brief, default-OFF.** Council ranked health-gated revenue brief above brief-only and watchdog-only. Existing `office_briefing` + `/app/inbox` are reused at 08:15 IST; no duplicate queue or auto-contact channel. `HOT_QUEUE_BRIEF_DAILY` is fail-closed on unknown queue/recent job failure, cross-process idempotent, retry/DLQ + dual boot-grace + Celery/in-process parity wired. FastAPI 0.139 lazy-route gate fixed; missing UPI `BaseModel` import restored locally. 36 focused tests + `prod_check.py` + secrets green. Uncommitted/not deployed; live `/api/upi/submit` still 404.
...
- LIVE billing debt: `/api/upi/submit` = 404 because deployed main lacks the restored `BaseModel` import; local fix verified, selective commit/deploy needs explicit user authorization.
...
**Next action:** Shared main tree has ADR-073 + ADR-074 + ADR-075 uncommitted alongside other sessions' edits. Current `prod_check.py` PASS (1065 routes/0 gaps), 96 targeted tests green (38 dev-control + 58 delivery/billing), secrets clean; full `run_tests.bat` remains owed. Highest priority is user-authorized selective commit/deploy of the live UPI 404 fix + route-gate compatibility + dormant Hot Queue automation + ADR-075 control-plane files, never `git add -A`; then verify app/worker/scheduler and only explicitly enable `HOT_QUEUE_BRIEF_DAILY`. Keep `EMAIL_WARMUP`, WhatsApp auto-send, voice-close WhatsApp, and platform dial OFF.

**Ops facts (hot):** Company inboxes = `admin@leadsgenai.in`, `sunny@leadsgenai.in` only · `SOCIAL_PREFS_HONOR` default OFF (draft prefs dormant until operator flip) · Scheduler = Celery durable (`RUN_IN_PROCESS_SCHEDULER=0`, rollback = `=1`+`WEB_CONCURRENCY=1`) · 24 staff jobs + dead-man trio alive · WhatsApp auto-send OFF (`WHATSAPP_AUTO_SEND=0`, `VOICE_CLOSE_WHATSAPP=0`; 1-click only) · UPI pay-info ARMED but live submit route 404 pending deploy · NOTIFY_EMAIL set · Sentry ARMED · offsite backup LIVE (restore proven) · MCP `/mcp` gated (`FASTAPI_MCP_TOKEN`/allowlist) · 250 skills in skill_pack · Slash commands: `/verify` `/ship` `/checkpoint` `/learn` `/compact-check` `/optimize` `/test-expand`.
