...
- 2026-07-14: **`recipient_not_on_whatsapp` ab integration failure NAHI (ADR-096) + `app:8080` port-trap fix.** WAHA ka "is number pe WhatsApp nahi hai" = correct API answer, fault nahi — usko fail count karne se synthetic test numbers ne rolling-24h snapshot ko `whatsapp fail=72 ok=2 rate=0.973` pe pahuncha diya, threshold(3) cross hua, aur REAL customer jiya ke ledger me ROZ jhootha `integration_failed: WhatsApp integration failing` likha (07-11→07-14) jabki WhatsApp chal raha tha (`waha:3000/api/sessions/default → 200`, ECONNREFUSED/2h = 0). Real faults (not_configured/wrong_linked_number/transport) ab bhi record hote hain. Saath: `infra_handler._check_ready()` ab `app:8080` pehle try karta — live proof `app:8000 → 000`, `app:8080 → 200` (poora purana URL list worker se fail tha). Prod `.env` ka `SELF_HEALTH_URL` abhi bhi `app:8000` = ab harmless, par operator 8080 kar de.
