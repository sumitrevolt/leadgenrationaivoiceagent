...
- 2026-07-09: **ADR-054 Outreach suppression is now sendable-candidate gating + warmup pause is visible.** Added never-raise `email_unsub.suppressed_emails()` / `list_suppressed()`, bulk-loaded suppression before initial outreach and follow-up candidate selection, and made `outreach_stats()`/`outreach_activity()` expose `pending_total`, `pending_sendable`, `suppressed`, complaint rate, and pause reason. No warmup resume, no data deletion, no broad-domain block. Gate: 31 targeted tests green, ruff clean, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`c5d9a76`): app/worker/worker-heavy/scheduler healthy, queue 0, `/app/inbox` 200, public/internal `/health`=`environment:production`; live outreach headline now says `Email warmup PAUSED: complaint rate 0.449% >= 0.25% (7/1559 in 7d)`, with 7 suppressed and 151 sendable pending.
- 2026-07-09: **ADR-053 Hot Queue now hides uncontactable WhatsApp IDs and saved auto-ack replies; deliverability auto-pause is live.** Live audit after ADR-052 found top Hot Queue rows were Meta/status numeric IDs, not real phones, and old auto-ack emails still appeared as "interested." Shipped stricter Indian-mobile normalization (`10`, `91+10`, `0+10` only), removed WA Reply action for invalid IDs, hid invalid WhatsApp rows from `hot_queue()`, and filtered saved auto-ack rows on read. Gate: 18 hot-queue tests green, `prod_check.py` PASS, secrets clean. **COMMITTED + DEPLOYED to production** (`1b54e43`, `04631af`, `dd8c118`): live queue 20→12, bad WhatsApp rows 0, auto-ack "Thank you..." rows 0, `/app/inbox` 200, queue 0, app/worker/worker-heavy/scheduler healthy, public/internal `/health`=`environment:production`.
- 2026-07-08: **ADR-052 Hot Queue web surfaces now open WhatsApp with the AI reply draft prefilled (human-send only).** `reply_agent.hot_queue()` now emits a safe `wa_link` for phone-backed hot replies using the existing draft text; `/app/inbox` and `/app/office` show an explicit "WhatsApp draft" action while keeping copy/email/call/done controls. This closes the web-surface parity gap: ntfy push actions already had prefilled WA, but the operator inbox still opened a blank chat. Gate: 15 hot-queue tests + 15 Office frontend tests green, inline script syntax check green, `prod_check.py` PASS, secrets clean, duplicate hot-queue route grep clean. **COMMITTED + DEPLOYED to production** (`62fe0e1`): app/worker/worker-heavy/scheduler healthy, `/app/inbox` 200, hot-queue API 401 auth-gated, live HTML marker present, queue 0, public/internal `/health`=`environment:production`.
...
**Next action:** No verified uncommitted production-ready batch is pending; latest live code commit is `c5d9a76` (outreach suppression/sendable gating). Stale/dirty Claude worktrees still exist, but council decision remains **do not merge blindly**. Keep `EMAIL_WARMUP` paused; do not resume from `/api/growth/outreach/warmup/resume` until complaint-source/list-quality cleanup is done. Next high-leverage loop: classify the 151 sendable pending prospects by source/domain/niche quality and add precise suppression/segmentation rules only where evidence supports them; avoid broad Gmail-style blocks.
