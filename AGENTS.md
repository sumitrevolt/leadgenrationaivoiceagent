...
- 2026-07-09: **ADR-059 Priority local-SMB outreach review can now be exported as admin CSV.** Built admin-only `/api/platform/team/outreach-pending-review?bucket=priority_local_smb&limit=100` plus Admin Email Activity “Priority CSV” button. Export returns deduped, valid, unsuppressed pending recipients only; it does not send, suppress, or resume warmup. Gate: 24 targeted tests green, ruff clean, admin JS syntax OK, `prod_check.py` PASS with API docs synced, secrets clean. **COMMITTED + DEPLOYED to production** (`4d4a622`): live export count 71, endpoint 401 without admin, queue 0, public `/health`=`environment:production`, no fresh real errors.
