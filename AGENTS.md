...
- 2026-07-07: **ADR-024 (supersedes ADR-023) Customer Delivery Ledger reconciled onto the REAL `app/marketing/delivery_ledger.py`** — mid-build discovered `tmp-deploy-main` (checked out at main repo root, 140 files ahead of `main`) already had a more mature, reviewed jsonl-based ledger module (same 14 events, part of the `8683774`/`e5178f1` Gemini-SDK+delivery-ledger commit below, never wired to any call site). Dropped this session's own Postgres `delivery_events` table/model, re-pointed all 8 wiring call sites + admin `deliver-now`/`timeline` + customer `timeline` + all 3 dashboard-fork UI cards at the real module, then merged this branch with `tmp-deploy-main` (both bodies of work, user-authorized). 15/15 new + 29/29 regression tests green, `prod_check.py` PASS. Spec/plan `docs/superpowers/specs|plans/2026-07-06-customer-delivery-ledger*`; sub-projects 2-6 in `memory/backlog.md`.
- 2026-07-06: **Gemini SDK migration + realtime chain + trainer timeout (commit `8683774`/`e5178f1`):** `telecaller_brain.py` `google.generativeai`→`google.genai` SDK, `free_ai.py` realtime chain `groq→cerebras→mistral`, `team_scheduler.py` trainer timeout 900→480s, `.env.example` 20+ new flags — merged into this branch above.
- 2026-07-06: **Full Waves 1-4 DEPLOYED LIVE (commit `c5761b4`/`96fe590`):** 22 shipped items (scheduler hardening/LLM cache/Prometheus metrics/KB integrity/security gates/product delivery/Office HQ trends/voice upgrades). CI hard-gate blocking. 2FA fail-closed.
- 2026-07-05: **WhatsApp auto-send HARD OFF in prod** + **platform_dial HARD OFF** (user mandate — ban-risk/IVR false-positives/paisa burn) — both remain OFF through this merge; `dial_gate`/`call_qualifier` enforcement LIVE in prod.
...
**Next action:** User-authorized merge of this branch + `tmp-deploy-main` IN PROGRESS (both bodies of work combined, then push origin/main + VPS deploy) — resolves the old "separate go-ahead" pending item below. After deploy: (1) hot lead **+917498797259** — 1-click WhatsApp (mid-funnel money moment). (2) Fix D DONE (`REPLY_AGENT=1` live + IMAP OK); deliverability SPF/DKIM/DMARC all valid — ~2000-email/0-reply root is ops-level (warmup/content/list-hygiene), separate sprint. (3) Post-deploy `.env` flag flips (NOINPUT_POLICY/PROMETHEUS_JOB_METRICS/DIGEST_NTFY/OPS_ALERTS) + Caddy `/metrics`+`/health/deep` block + VPS data ops (backfill_phone_type/purge_junk) = user-gated.
