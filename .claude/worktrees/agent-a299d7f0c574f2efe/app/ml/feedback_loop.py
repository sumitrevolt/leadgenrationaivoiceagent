...
class ResponsePattern:
    ...
    def update_metrics(self, outcome: CallOutcome):
        ...
        if self.times_used > 0:
            self.success_rate = (self.success_count + self.partial_count * 0.5) / self.times_used
...
class ObjectionHandler:
    ...
    def add_response(self, pattern: ResponsePattern):
        ...
        existing = next(
            (p for p in self.response_variants if p.response_text == pattern.response_text), None
        )
        if existing:
...
class IndustryInsights:
    ...
    best_value_prop: str | None = None
    best_closing: str | None = None
...
class FeedbackLoop:
    ...
    async def _update_pattern(
        ...
        existing = next(
            (p for p in self.patterns[pattern_key] if p.response_text == response_text), None
        )
    ...
    async def _update_industry_insights(
        self, industry: str, outcome: CallOutcome, responses_used: list[dict]
    ):
    ...
    async def _update_objection_handler(
        self, objection_type: str, response_text: str, industry: str, outcome: CallOutcome
    ):
        ...
        pattern = next(
            (p for p in handler.response_variants if p.response_text == response_text), None
        )
    ...
    def get_best_response(
        self, response_type: str, industry: str, language: str = "hinglish", fallback: str = None
    ) -> str | None:
        """
    ...
    def get_best_objection_response(self, objection_type: str, fallback: str = None) -> str | None:
    ...
    def get_industry_insights(self, industry: str) -> IndustryInsights | None:
        """Get learned insights for an industry"""
        return self.industry_insights.get(industry)

    def get_response_options(
        self, response_type: str, industry: str, top_k: int = 3
        ...
        sorted_patterns = sorted(valid, key=lambda p: p.success_rate * p.confidence, reverse=True)

        return [(p.response_text, p.success_rate) for p in sorted_patterns[:top_k]]
    ...
    async def export_learnings(self, output_file: str):
        ...
        export_data = {
            "patterns": {k: [p.to_dict() for p in v] for k, v in self.patterns.items()},
            "industry_insights": {k: asdict(v) for k, v in self.industry_insights.items()},
            "objection_handlers": {
                k: {
