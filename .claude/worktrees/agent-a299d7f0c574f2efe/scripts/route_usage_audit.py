...
"""
import re, glob, os, argparse, pathlib, sys
...
def extract_routes():
    ...
    if mp.exists():
        ...
        for mm in re.finditer(r'@app\.(get|post|put|delete|patch)\(\s*["\']([^"\']+)["\']',g):
            routes.append((mm.group(1).upper(), mm.group(2), f"{mm.group(1).upper():6} {mm.group(2)}   [main.py]"))
    return routes

def to_regex(path):
    parts=["[^/]+" if (s.startswith("{") and s.endswith("}")) else re.escape(s) for s in path.split("/")]
...
def main():
    ...
    a=ap.parse_args()
    if not os.path.exists(a.access_log): sys.exit(f"access log not found: {a.access_log}")
    routes=extract_routes()
    comp=[(to_regex(p),d) for _,p,d in routes]
    hits={d:0 for _,_,d in routes}
    for p in log_paths(a.access_log):
        for rx,d in comp:
            if rx.match(p): hits[d]+=1; break
