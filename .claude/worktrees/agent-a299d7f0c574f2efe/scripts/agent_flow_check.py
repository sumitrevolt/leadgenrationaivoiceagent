...
def _team():
    ...
    for m in members:
        ...
        print(f"   - {m.get('key'):10} {st:9} last_active={m.get('last_active_mins')}m  product={m.get('product','')}")
    return ts
...
def _hermes():
    ...
    try:
        ...
        loop = asyncio.new_event_loop(); snap = loop.run_until_complete(infra_handler.snapshot()); loop.close()
...
def _llm():
    ...
    print(f"  total_calls={st.get('total_calls')} fallback/fail_rate={st.get('fallback_or_fail_rate')}")
    provs = st.get("providers") or {}
    for p, v in list(provs.items())[:12]:
        if isinstance(v, dict):
            print(f"   - {p:14} ok={v.get('ok_rate', v.get('ok','?'))} calls={v.get('calls','?')} last_err={str(v.get('last_error',''))[:40]}")
    return st
...
def _flags():
    ...
    for f in flags:
        ...
        (on if v in ("1", "true", "yes") or (f in ("SEARXNG_URL","NTFY_URL","NTFY_TOPIC") and v) else off).append(f)
...
def _pulse():
    ...
    print(f"  prospects total={pr.get('total')} ready={pr.get('ready')} client={pr.get('client')} with_email={pr.get('with_email')}")
    print(f"  inquiries={p.get('inquiries')} blog={p.get('blog_articles')} content={p.get('content_items')} clients_active={p.get('marketing_clients_active')}")
    if p.get("best_niche"):
...
def _loops():
    try:
        ...
        print(f"  process_engine: total={len(runs)} running={len(running)} waiting-breakpoint={len(waiting)}")
    except Exception as e:
...
def _events():
    ...
    for e in evs:
        print(f"   {str(e.get('at',''))[:19]}  {e.get('agent','?'):10} {e.get('action','?'):22} {str(e.get('detail',''))[:50]}")
    if not evs:
