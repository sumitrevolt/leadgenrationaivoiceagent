...
async def main() -> None:
    ...
    print("KEY_SET:", bool(settings.google_maps_api_key) and "your-" not in settings.google_maps_api_key)
    ...
    if leads:
        l = leads[0]
        print("SAMPLE:", getattr(l, "name", "?"), "| ph:", bool(getattr(l, "phone", None)),
              "| rating:", getattr(l, "rating", None), "| reviews:", getattr(l, "reviews_count", None))

    # personalized pitch
    from app.platform import prospector
    if hasattr(prospector, "build_personalized_pitch"):
        p = prospector.build_personalized_pitch("Test Cafe", "restaurant_cafe", "Pune", 3.9, 12, False)
        print("PITCH:", p[:120])
