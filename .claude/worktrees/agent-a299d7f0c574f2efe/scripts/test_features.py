...
async def main():
    try:
        ...
        rec("generate_post", bool(p.get("caption")), f"provider={p.get('provider')} cap={len(p.get('caption',''))}ch tags={len(p.get('hashtags',[]))} img={'image_url' in p}")
    except Exception as e:
        rec("generate_post", False, repr(e))

    try:
        from app.marketing import ai_image
        img = await ai_image.marketing_image("Test Cafe", "restaurant_cafe", occasion="Diwali", offer="20% off")
        rec("ai_image", img.get("url", "").startswith("http"), f"prompt='{img.get('prompt','')[:60]}'")
        rec("logo", ai_image.logo_url("Test Cafe", "restaurant_cafe").startswith("http"), "url built")
    except Exception as e:
        rec("ai_image", False, repr(e))

    try:
        from app.marketing import post_generator
        vs = await asyncio.gather(*[post_generator.generate_post("Test Cafe", "restaurant_cafe") for _ in range(2)])
        rec("variations", all(v.get("caption") for v in vs), f"{len(vs)} variants, providers={[v.get('provider') for v in vs]}")
    except Exception as e:
    ...
    try:
        ...
        rec("chatbot", bool(r.get("answer")), f"ask_contact={r.get('ask_contact')} sources={r.get('sources')} ans='{r.get('answer','')[:60]}'")
    except Exception as e:
        rec("chatbot", False, repr(e))

    try:
        from app.marketing import sentiment
        s = await sentiment.analyze(["bahut accha service tha", "delivery slow thi", "staff helpful hai"])
        rec("sentiment", s.get("count") == 3, f"breakdown={s.get('breakdown')} score={s.get('score')} summary={bool(s.get('summary'))}")
    except Exception as e:
    ...
    try:
        ...
        rec("hashtags", len(h.get("hashtags", [])) > 0, f"{len(h.get('hashtags',[]))} tags, {len(h.get('best_times',[]))} times")
    except Exception as e:
    ...
    try:
        ...
        rec("ops_watchdog", ("ok" in w) or ("skipped" in w), f"ok={w.get('ok')} issues={[i.get('key') for i in w.get('issues',[])]}")
    except Exception as e:
...
def http_post(path, body):
    req = urllib.request.Request("http://127.0.0.1:8000" + path, data=json.dumps(body).encode(),
                                 method="POST", headers={"Content-Type": "application/json", "User-Agent": "selftest"})
    try:
        ...
        return repr(e)
for ep in ["/api/marketing/ai-image", "/api/marketing/complete-post", "/api/marketing/post-variations",
           "/api/marketing/chatbot", "/api/marketing/sentiment", "/api/marketing/hashtags", "/api/marketing/brand-logo"]:
