...
description: Deploy / fix / manage the LeadGen AI platform on the Hostinger KVM VPS (systemd host uvicorn for the web app + Docker for the Celery workers). Use when the user mentions VPS, Hostinger, deploy, server, leadsgenai.in, SSH, Caddy, "site is down", SSL, 502, 404, Docker, or wants to update the live server. Captures the exact gotchas hit during deploys so they are never repeated.
---

# Hostinger VPS Deploy & Ops (LeadGen AI)


> ## ⚠️ TOPOLOGY CORRECTION — 2026-09-15 (READ FIRST)
> This skill was written for the **pre-migration container era**; several lines below were
> stale and one of them was actively dangerous. Verified production topology:
> - **The web app is a systemd unit, not a container.** `/etc/systemd/system/leadgen.service`
>   (`is-enabled`=enabled, `is-active`=active) runs
>   `/opt/leadgen/.venv/bin/python -m uvicorn app.main:app --host 127.0.0.1 --port 8000 --workers 2`
>   with `EnvironmentFile=/opt/leadgen/.env` and `WorkingDirectory=/opt/leadgen`.
> - **There is NO `leadgen_app` container.** `docker exec leadgen_app` → `No such container`.
>   `docker ps -aq --filter "label=com.docker.compose.service=app"` returns two **orphans**
>   (`leadgen_app_vobiz`, `leadgen_app_staging`) which will silently mislead any
>   compose-based check into comparing the wrong `APP_VERSION`.
> - **`systemctl restart leadgen` IS the app deploy step** — the opposite of what this skill
>   previously claimed.
> - **`EnvironmentFile` freezes `APP_VERSION` at process start** and `app/main.py` never
>   re-reads `.env`, so `.env:APP_VERSION` must be updated AND the unit restarted, or
>   `/health` keeps reporting the previous version.
> - The **Celery workers ARE containers** and do need `docker compose build` + `up -d`.
> - Canonical release path = **`scripts/deploy_vps.sh`**. Do not hand-roll docker commands
>   for the web app.
...
- App dir on VPS: **/opt/leadgen**. **App = systemd unit `leadgen`** — a HOST uvicorn process on `127.0.0.1:8000` (`EnvironmentFile=/opt/leadgen/.env`, `WorkingDirectory=/opt/leadgen`, `--workers 2`), `enabled` + `active`. **There is NO `leadgen_app` container.** The `app` compose service still declares `127.0.0.1:8000:8080` but can never bind while the unit holds the port, so `app` must never be rolled via compose. **⇒ `systemctl restart leadgen` IS the app deploy step.** (Corrected 2026-09-15: this line previously claimed the opposite and told operators NOT to run the one command that is required.)
...
- Scheduler = **Celery durable (LIVE)**: `leadgen_worker` + `leadgen_worker_heavy` + `leadgen_scheduler` containers (`--profile celery`). (2026-07-05 verified; container set corrected 2026-09-15) Full set: worker · worker_heavy · worker_video · scheduler · dsh_worker · redis · redis-cache · db · pgbouncer · qdrant · **postiz** (social publisher) · **waha** (WhatsApp HTTP API) + obs containers. **`app` is NOT in this set** — the web app is the systemd unit `leadgen`.
...
5. **Who serves what (corrected 2026-09-15).** The **web app is NOT containerised** — systemd `leadgen` runs `/opt/leadgen/.venv/bin/python -m uvicorn app.main:app` straight from the host checkout, so a code change goes live after **`git pull` + `systemctl restart leadgen`** (no rebuild). `app/` + `frontend/` + `.claude/skills/` are still BAKED into `Dockerfile.lock` for the **Celery workers** (`worker`/`worker_heavy`/`worker_video`/`scheduler`/`dsh_worker`), and THOSE do need `docker compose build` + `up -d --no-deps <service>`. Only `./data` + `./logs` are bind-mounts.

6. **New `@app.get` page-route** = picked up when the **systemd unit restarts** (the host process reads the checkout directly). ALWAYS curl-verify the new route post-deploy (200 + real content).
...
- App logs: **`journalctl -u leadgen -n 60 --no-pager`** — the app is the systemd unit; `docker logs leadgen_app` fails with `No such container`.
- **Deploy / restart after code change**:
  **STEP 0 — DRIFT CHECK (MANDATORY, kabhi skip mat karo):** VPS tree chronically dirty rehta (live hotfixes + docker-cp drift); blind `reset --hard` ne 2026-07-01/02 me kaam KHOYA. Reset se PEHLE:
  ...
  systemctl status leadgen --no-pager | head -5   # app = the systemd unit, not a container
  ```
  ...
  ```
  bash scripts/deploy_vps.sh        # CANONICAL parent — do not hand-roll this chain
  # manual equivalent, ONLY if the parent is unavailable:
  #   git -C /opt/leadgen fetch --all -q && git -C /opt/leadgen reset --hard origin/main -q
  #   bash /opt/leadgen/scripts/migration_preflight.sh
  #   sed -i "s/^APP_VERSION=.*/APP_VERSION=<sha>/" /opt/leadgen/.env
...
Edit `/opt/leadgen/.env` (NO inline comments!), then restart the app: `systemctl restart leadgen` (systemd re-reads `EnvironmentFile` on start; there is no `leadgen_app` container since 2026-09-15). Verify with `journalctl -u leadgen -n 20 --no-pager` for "Application startup complete".

## LLM quota note
LLM chain is FREE multi-provider (`app/voice_agent/free_ai.py`): **Mistral `mistral-small-latest` PRIMARY** → Groq `llama-3.1-8b-instant` → Cerebras `gpt-oss-120b` (429-prone, NOT primary) → Ollama floor → Gemini → SambaNova → OpenRouter. Escalating circuit-breaker handles 429s. If bot suddenly rule-based / "[echo / test-mode]", check `journalctl -u leadgen -n 200 --no-pager | grep -iE "429|quota|ResourceExhausted"` — usually a provider cooldown that self-recovers; Gemini is only a late fallback, not the default.
...
- **Change-risk tier:** any live-VPS action is **High-risk** — it locks: app code change = **`git pull` + `systemctl restart leadgen`** (no rebuild — the app is not containerised); worker/scheduler code change = `build` + `up -d --no-deps <service>`, curl-verify every new route, named rollback ready. `worker-heavy` (hyphen) wrong service name = whole `up` ABORT → verify service names first.
...
- **Observability:** `journalctl -u leadgen -n 60 --no-pager` ("Application startup complete") · `docker ps`/`docker stats --no-stream` (CPU 0% + hang = event-loop freeze → `prod-incident-triage`) · `journalctl -u caddy -n 30` · `redis-cli llen celery` (>500 → `del celery`) · `dlq:failed_tasks`.
