...
- LLM brain: `llm_brain.py` (`LLMBrain`). **Model id must be real** (e.g. `gemini-2.5-flash`); the bare value `gemini` is invalid. Provider routing keys on substrings ("gemini","gpt","claude","vertex","local").
