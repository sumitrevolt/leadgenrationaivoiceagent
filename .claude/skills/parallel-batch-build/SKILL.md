...
2. **File ownership matrix LIKHO prompt me** — ek file = ek hi owner. Shared files (main.py, growth.py, marketing.py, public_site.py, team_scheduler.py, worker.py, compose, requirements) **KOI agent touch nahi karta** — wiring main session sequentially karta hai (mounts, page routes, AUTOMATION_FLAGS, scheduler hooks best-effort try/except).
...
5. Wiring → `python scripts/prod_check.py` → targeted pytest suites (full suite ke 6 pre-existing env-fails ignore) → commit → deploy → **smoke me naye PUBLIC endpoints zaroor hit karo** → health re-check.
