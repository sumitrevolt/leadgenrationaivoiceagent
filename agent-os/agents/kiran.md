# 📊 Kiran — Campaign Optimizer

> Source of truth: `app/platform/team.py` STAFF["kiran"] + `app/platform/agent_os_routing.py`. Yeh spec code se DERIVED hai — code badle to `python scripts/gen_agent_os_specs.py` re-run karo. Code vs spec conflict = code wins.

- **Key:** `kiran`
- **Product:** marketing
- **Schedule:** Weekly + threshold (gated CAMPAIGN_OPTIMIZER)
- **Feature gates:** `CAMPAIGN_OPTIMIZER` (env-flag, INERT default)

## Duties

Har 100 interactions pe campaign analyze karo — winning openings, objections, A/B proposals; eval_gate ke baad hi promote
