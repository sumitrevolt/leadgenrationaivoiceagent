# 🔁 Anika — Cadence Manager

> Source of truth: `app/platform/team.py` STAFF["anika"] + `app/platform/agent_os_routing.py`. Yeh spec code se DERIVED hai — code badle to `python scripts/gen_agent_os_specs.py` re-run karo. Code vs spec conflict = code wins.

- **Key:** `anika`
- **Product:** marketing
- **Schedule:** Roz scheduled (team_scheduler.py cadence.run_due())
- **Feature gates:** `CADENCE_ENGINE` (env-flag, INERT default)

## Duties

Enrolled leads ko per-day omnichannel sequence (email/SMS/WhatsApp/LinkedIn draft) me aage badhana (gated CADENCE_ENGINE)
