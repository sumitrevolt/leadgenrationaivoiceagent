...
> Source of truth: `app/platform/team.py` STAFF["manager"] + `app/platform/agent_os_routing.py`. Yeh spec code se DERIVED hai — code badle to `python scripts/gen_agent_os_specs.py` re-run karo. Code vs spec conflict = code wins.
...
Kaam baantna — data/leads agents ko route karna (LangGraph), team coordination
